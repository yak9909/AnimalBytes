...
BMPImage *UpSampleUntilItsEnough(BMPImage *img, int maxX, int maxY)
{
  ...
  if( temp->Width() < static_cast<u32>(maxX) ||
      temp->Height() < static_cast<u32>(maxY) )
...
{
  if( width >= height ) return ((float)width / maxX);
...
BMPImage *PostProcess(BMPImage *img, int maxX, int maxY)
{
  ...
  if( width == maxX && height == maxY ) return img;
...
void Preferences::LoadSettings(void)
{
  ...
  if( OpenConfigFile(settings, header) == 0 ) {
    ...
    Flags = header.flags;
    memcpy(reinterpret_cast<void *>(Backlights), &header.lcdbacklights,
           sizeof(Backlights));
  }

  // Check for hotkeys to be valid
  if( MenuHotkeys == 0 ) MenuHotkeys = Key::Select;
}
...
void Preferences::LoadBackgrounds(void)
{
  ...
  Task task([](void *arg UNUSED) {
    ...
    if( topBackgroundImage == nullptr &&
        File::Exists(source + "TopBackground.bmp") ) {
    ...
    if( bottomBackgroundImage == nullptr &&
        File::Exists(source + "BottomBackground.bmp") ) {
...
{
  if( bottomBackgroundImage || topBackgroundImage ) _bmpCanBeLoaded = true;
...
void Preferences::WriteSettings(void)
{
  ...
  File settings;
  int mode =
    File::READ | File::WRITE | File::CREATE | File::TRUNCATE | File::SYNC;
  Header header = {0};
...
void Preferences::Initialize(void) { LoadBackgrounds(); }
}  // namespace CTRPluginFramework
