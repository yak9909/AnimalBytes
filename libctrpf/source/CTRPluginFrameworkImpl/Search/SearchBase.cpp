    ...
    _previous(previous)
{
  ...
  if( Directory::Open(dir, "Search", true) == 0 ) {
    // Open file
    int res = File::Open(
      _file, path, File::READ | File::WRITE | File::CREATE | File::TRUNCATE);
...
cancel:
  ...
  if( isRegionFinished || Canceled ) {
    ...
    if( CheckNextRegion() || Canceled ) {
      ...
      _indexRegion = 0;
      if( _previous != nullptr ) _previous->_indexRegion = 0;
...
const Header &Search::GetHeader(void) const { return (_header); }
...
void Search::ExtractPreviousHits(void *data, u32 index, u32 structSize,
                                 u32 &nbItem, bool forced)
{
  if( !forced ) {
    RegionOnFile &region = _header.regions[_indexRegion];

    if( index > region.nbResults || !nbItem ) return;
    ...
    if( nbItem + index > region.nbResults ) nbItem = region.nbResults - index;
    ...
    // Check nbItem
    if( regionIndex == _header.nbRegions - 1 &&
        index + nbItem > region.nbResults )
...
void Search::WriteHeader(void)
{
  ...
  if( offset > 0 ) _file.Seek(offset, File::SET);
}

void Search::WriteResults(void)
{
  if( !_resultsInPool ) return;
...
void Search::UpdateProgress(void)
{
  ...
  Progress =
    100.0f * (float)(_achievedRegionSize + excedent) / (float)_totalRegionSize;
}
...
bool Search::CheckNextRegion(void)
{
  ...
  if( IsFirstSearch(_flags) ) {
    ...
    if( _indexRegion == _header.nbRegions - 1 ) return (true);
  ...
  {
    // If we've done the last region, return true
    if( _indexRegion >= _header.nbRegions ) return (true);
    ...
    // Increment achieved size for progress calculation
    _achievedRegionSize +=
      _previous->_header.regions[_previous->_indexRegion].nbResults;
    ...
    if( previousIndex >= previousHeader.nbRegions ) return (true);
    ...
    svcFlushProcessDataCache(
      ProcessImpl::ProcessHandle, _startRegion,
      _header.regions[_indexRegion].endAddress - _startRegion);
...
u32 Search::GetRegionIndex(u32 index)
{
  ...
  for( RegionIndex &rindex : _indexTable ) {
    if( index >= rindex.startIndex && index <= rindex.endIndex ) return (i);
    i++;
  }
