  ...
  : Search(parameters.previous)
{
  ...
  if( Heap::SpaceFree() <
      0xB0000 )  /// 0x60000 (for pool) + 0x50000 (for Storage)
  {
    // Unload bmp
  ...
  _resultSize = IsFirstSearch(_flags)
                  ? (IsUnknownSearch(_flags) ? sizeof(u32) : sizeof(Results32))
                  : sizeof(Results32WithOld);
  ...
  if( !parameters.fullMemory ) {
    ...
    _totalRegionSize =
      _header.regions[0].endAddress - _header.regions[0].startAddress;

    _startRegion = parameters.ranges[0].startAddress;
    _endRegion = parameters.ranges[0].endAddress - 1;

    // Flush memory
    svcFlushProcessDataCache(ProcessImpl::ProcessHandle, _startRegion,
    ...
    for( Region &region : parameters.ranges ) {
      ...
      if( Process::CheckRegion(region.startAddress, size, MEMPERM_READ) ) {
        ...
        if( size < regionSize ) {
          ...
          _header.regions[_header.nbRegions] = {
            region.startAddress, region.startAddress + size, 0, 0};
          _header.nbRegions++;
          ...
          // Region is okay, add it to header
          _header.regions[_header.nbRegions] = {region.startAddress,
                                                region.endAddress, 0, 0};
          _header.nbRegions++;
  ...
  // If it's not the first search, _totalRegionSearch must be total results from
  // previousSearch
...
void Search32::ReadResults(u32 index, StringVector &addr, StringVector &newVal,
                           StringVector &oldVal)
{
  addr.clear();
  newVal.clear();
  ...
  if( index >= ResultsCount ) return;

  if( nbItem + index >= ResultsCount ) nbItem = ResultsCount - index;
  ...
  if( IsFirstSearch(_flags) ) {
    ...
    if( IsSpecifiedSearch(_flags) ) {
      ...
      ExtractPreviousHits(previousHits.data(), index, sizeof(Results32), nbItem,
                          true);
  ...
  // Subsidiary search
  {
    ...
    // Extract previous hits
    ExtractPreviousHits(previousHits.data(), index, sizeof(Results32WithOld),
                        nbItem, true);
...
{
  if( _currentAddress < _startRegion ) _currentAddress = _startRegion;

  u32 endAddress = std::min(_currentAddress + 0x40000, _endRegion);
  Results32 *result = reinterpret_cast<Results32 *>(
    (u32)_pool + (sizeof(Results32) * _resultsInPool));
  SearchFlags compare = (SearchFlags)CompareFlags(_flags);
  SearchFlags type = (SearchFlags)TypeFlags(_flags);

  if( type == SearchFlags::U8 )
    FirstSearchSpecifiedU8(endAddress, compare, result);
  else if( type == SearchFlags::U16 )
    FirstSearchSpecifiedU16(endAddress, compare, result);
  else if( type == SearchFlags::U32 )
    FirstSearchSpecifiedU32(endAddress, compare, result);
  else if( type == SearchFlags::Float )
    FirstSearchSpecifiedFloat(endAddress, compare, result);
  else
    return (true);  ///< An error occured, skip region

  // Check if region is finished
  if( _currentAddress >= _endRegion ) return (true);
...
bool Search32::SecondSearchSpecified(void)
{
  ...
  Results32WithOld *result = reinterpret_cast<Results32WithOld *>(
    (u32)_pool + (sizeof(Results32WithOld) * _resultsInPool));

  // Extract previous hits
  _previous->ExtractPreviousHits(previousHits.data(), _currentAddress,
                                 sizeof(Results32), nbItem);
  ...
  if( type == SearchFlags::U8 )
    SecondSearchSpecifiedU8(previousHits, compare, result);
  else if( type == SearchFlags::U16 )
    SecondSearchSpecifiedU16(previousHits, compare, result);
  else if( type == SearchFlags::U32 )
    SecondSearchSpecifiedU32(previousHits, compare, result);
  else if( type == SearchFlags::Float )
    SecondSearchSpecifiedFloat(previousHits, compare, result);
  else
    return (true);  ///< An error occured, skip region
...
bool Search32::SecondSearchUnknown(void)
{
  ...
  // Do the comparison on previous hits
  if( type == SearchFlags::U8 )
    SecondSearchUnknownU8(previousHits, compare, result);
  else if( type == SearchFlags::U16 )
    SecondSearchUnknownU16(previousHits, compare, result);
  else if( type == SearchFlags::U32 )
    SecondSearchUnknownU32(previousHits, compare, result);
  else if( type == SearchFlags::Float )
    SecondSearchUnknownFloat(previousHits, compare, result);
  else
    return (true);  ///< An error occured, skip region
...
bool Search32::SubsidiarySearchSpecified(void)
{
  ...
  // Extract previous hits
  _previous->ExtractPreviousHits(previousHits.data(), _currentAddress,
                                 sizeof(Results32WithOld), nbItem);
  ...
  // Do the comparison on previous hits
  if( type == SearchFlags::U8 )
    SubsidiarySearchSpecifiedU8(previousHits, compare, result);
  else if( type == SearchFlags::U16 )
    SubsidiarySearchSpecifiedU16(previousHits, compare, result);
  else if( type == SearchFlags::U32 )
    SubsidiarySearchSpecifiedU32(previousHits, compare, result);
  else if( type == SearchFlags::Float )
    SubsidiarySearchSpecifiedFloat(previousHits, compare, result);
  else
    return (true);  ///< An error occured, skip region
...
bool Search32::SubsidiarySearchUnknown(void)
{
  ...
  // Do the comparison on previous hits
  if( type == SearchFlags::U8 )
    SubsidiarySearchUnknownU8(previousHits, compare, result);
  else if( type == SearchFlags::U16 )
    SubsidiarySearchUnknownU16(previousHits, compare, result);
  else if( type == SearchFlags::U32 )
    SubsidiarySearchUnknownU32(previousHits, compare, result);
  else if( type == SearchFlags::Float )
    SubsidiarySearchUnknownFloat(previousHits, compare, result);
  else
    return (true);  ///< An error occured, skip region
