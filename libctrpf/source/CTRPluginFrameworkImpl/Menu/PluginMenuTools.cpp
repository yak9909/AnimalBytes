...
#define TAG_VERSION \
  CTRPF_VERSION_MAJOR "." CTRPF_VERSION_MINOR "." CTRPF_VERSION_BUILD
...
static void MenuHotkeyModifier(void)
{
  ...
  (HotkeysModifier(keys,
                   "Select the hotkeys you'd like to use to open the menu."))();

  if( keys != 0 ) Preferences::MenuHotkeys = keys;
}
...
void PluginMenuTools::UpdateSettings(void)
{
  ...
  if( Preferences::IsEnabled(Preferences::ShowTopFps) )
    (*item++)->AsMenuEntryTools().Enable();
  else
    (*item++)->AsMenuEntryTools().Disable();

  if( Preferences::IsEnabled(Preferences::ShowBottomFps) )
    (*item)->AsMenuEntryTools().Enable();
  else
    (*item)->AsMenuEntryTools().Disable();
}
...
enum HookFilesMode { NONE = 0, OSD = 1, FILE = 2 };
...
static u32 FindNearestSTMFD(u32 addr)
{
  for( u32 i = 0; i < 1024; i++ ) {
    addr -= 4;
    if( *(u16 *)(addr + 2) == 0xE92D ) return addr;
  }
  return (0);
}
...
{
  const u8 tryOpenFilePat1[] = {0x0D, 0x10, 0xA0, 0xE1, 0x00, 0xC0, 0x90, 0xE5,
                                0x04, 0x00, 0xA0, 0xE1, 0x3C, 0xFF, 0x2F, 0xE1};
  const u8 tryOpenFilePat2[] = {0x10, 0x10, 0x8D, 0xE2, 0x00, 0xC0, 0x90, 0xE5,
                                0x05, 0x00, 0xA0, 0xE1, 0x3C, 0xFF, 0x2F, 0xE1};

  u32 *addr = (u32 *)0x00100000;
  u32 *maxAddress = (u32 *)(Process::GetTextSize() + 0x00100000);

  while( addr < maxAddress ) {
    if( !memcmp(addr, tryOpenFilePat1, sizeof(tryOpenFilePat1)) ||
        !memcmp(addr, tryOpenFilePat2, sizeof(tryOpenFilePat2)) ) {
...
static bool InitFsTryOpenFileHook(void)
{
  ...
  if( FsTryOpenFileAddress != 0 ) {
    ...
    // Initialize the hook
    g_FsTryOpenFileHook.InitializeForMitm(FsTryOpenFileAddress,
                                          (u32)FsTryOpenFileCallback);
...
static void _DisplayLoadedFiles(MenuEntryTools *entry)
{
  ...
  if( entry->WasJustActivated() ) {
    // Initialize hook
    if( !InitFsTryOpenFileHook() ) entry->Disable();  ///< Hook failed
  ...
  if( !entry->IsActivated() ) {
    ...
    if( g_HookMode == 0 ) g_FsTryOpenFileHook.Disable();
  }
}
...
static void _WriteLoadedFiles(MenuEntryTools *entry)
{
  ...
  if( entry->WasJustActivated() ) {
    ...
    std::string filename =
      Utils::Format("[%016llX] - LoadedFiles.txt", Process::GetTitleID());

    if( File::Open(g_hookExportFile, filename, mode) != 0 ) {
      OSD::Notify(
        std::string("Error: couldn't open \"").append(filename).append("\""),
...
static void GetScreenShotMode(void)
{
  Keyboard kb(Color::LimeGreen << "Screenshot settings\x18\n\n Which screen(s) "
              ...
              {"Top screen", "Bottom screen", "Both screens"});

  int mode = kb.Open();
  if( mode != -1 ) Screenshot::Screens = mode + 1;
}
...
static void UpdateScreenshotText(void)
{
  ...
  g_screenshotEntry->name = ("Screenshot: " << Color::LimeGreen)
                              .append(KeysToString(Screenshot::Hotkeys));
  g_screenshotEntry->name += "\x18, " << Color::Orange;
  g_screenshotEntry->name.append(
    screens[(Screenshot::Screens & SCREENSHOT_BOTH) - 1]);
...
{
  Keyboard kb(Color::LimeGreen
                << "Screenshot settings\x18\n\nWhat do you want to change ?",
              {"Screens", "Hotkeys", "Timer", "Name", "Directory"});
  ...
  do {
    ...
    switch( choice ) {
      ...
      case 1:  ///< Hotkeys
      {
        ...
        if( keys != 0 ) Screenshot::Hotkeys = keys;
        break;
      }
      ...
      case 2:  ///< Timer
      {
        ...
        Keyboard keyboard(
          Color::LimeGreen
          << "Screenshot timer\x18\n\n Enter the amount of seconds you would "
             "like\n to continuously take screenshots.\n\n Enter 0 to disable "
             "the timer.\n\n"
          << Color::Orange
          << " Note that during that time, you can't\n access the menu.");
      ...
      case 3:  ///< Name
      {
        Keyboard nameKb(Color::LimeGreen
      ...
      case 4:  ///< Directory
      {
        ...
        if( Utils::DirectoryPicker(out) == -1 ) break;
...
static void EditBacklight(MenuEntryTools *entry)
{
  ...
  while( true ) {
    ...
    message =
      ...
      "  - Top screen: " +
      std::to_string(Preferences::Backlights[0].value) + ", " +
      (Preferences::Backlights[0].isEnabled
         ? (std::string)Color::LimeGreen + "Enabled\n"
         : (std::string)Color::Red + "Disabled\n") +
      ResetColor();

    message +=
      "  - Bottom screen: " + std::to_string(Preferences::Backlights[1].value) +
      ", " +
      (Preferences::Backlights[1].isEnabled
         ? (std::string)Color::LimeGreen + "Enabled\n"
         : (std::string)Color::Red + "Disabled\n") +
      ResetColor();
    message +=
...
{
  // Main menu
  _mainMenu.Append(new MenuEntryTools(
    "About", [] { g_mode = ABOUT; }, Icon::DrawAbout));

  _hexEditorEntry = new MenuEntryTools(
    "Hex Editor", [] { g_mode = HEXEDITOR; }, Icon::DrawGrid);
  _mainMenu.Append(_hexEditorEntry);
  _mainMenu.Append(new MenuEntryTools(
    "Gateway RAM Dumper", [] { g_mode = GWRAMDUMP; }, Icon::DrawRAM));
  _mainMenu.Append(new MenuEntryTools("Screenshots", nullptr,
                                      Icon::DrawUnsplash, new u32(SCREENSHOT)));
  _mainMenu.Append(new MenuEntryTools("Miscellaneous", nullptr, Icon::DrawMore,
                                      new u32(MISCELLANEOUS)));
  _mainMenu.Append(
    new MenuEntryTools("Settings", nullptr, Icon::DrawSettings, this));
  _mainMenu.Append(
    new MenuEntryTools("Shutdown the 3DS", Shutdown, Icon::DrawShutdown));
  _mainMenu.Append(
    new MenuEntryTools("Reboot the 3DS", Reboot, Icon::DrawRestart));

  // Screenshots menu
  _screenshotMenu.Append(new MenuEntryTools(
    "Change screenshot settings", ScreenshotMenuCallback, Icon::DrawSettings));
  _screenshotMenu.Append(
    (g_screenshotEntry = new MenuEntryTools(
       "Screenshot: " << Color::LimeGreen << KeysToString(Screenshot::Hotkeys)
                      << "\x18, " << Color::Orange << "Both screens",
       Screenshot_Enabler, true)));

  // Miscellaneous menu
  _miscellaneousMenu.Append(
    new MenuEntryTools("Display loaded files", _DisplayLoadedFiles, true));
  _miscellaneousMenu.Append(
    new MenuEntryTools("Write loaded files to file", _WriteLoadedFiles, true));
  _miscellaneousMenu.Append(new MenuEntryTools(
    "Display touch cursor",
    [] { Preferences::Toggle(Preferences::DrawTouchCursor); }, true,
    Preferences::IsEnabled(Preferences::DrawTouchCursor)));
  _miscellaneousMenu.Append(new MenuEntryTools(
    "Display touch coordinates",
    [] { Preferences::Toggle(Preferences::DrawTouchPosition); }, true,
    Preferences::IsEnabled(Preferences::DrawTouchPosition)));
  _miscellaneousMenu.Append(new MenuEntryTools(
    "Display top screen's fps",
    [] { Preferences::Toggle(Preferences::ShowTopFps); }, true,
    Preferences::IsEnabled(Preferences::ShowTopFps)));
  _miscellaneousMenu.Append(new MenuEntryTools(
    "Display bottom screen's fps",
    [] { Preferences::Toggle(Preferences::ShowBottomFps); }, true,
    Preferences::IsEnabled(Preferences::ShowBottomFps)));

  // Settings menu
  _settingsMenu.Append(new MenuEntryTools(
    "Change menu hotkeys", MenuHotkeyModifier, Icon::DrawGameController));

  _settingsMenu.Append(new MenuEntryTools("Set backlight (Experimental)",
                                          EditBacklight, false, false));
  _settingsMenu.Append(new MenuEntryTools(
    "Use floating button",
    [] { Preferences::Toggle(Preferences::UseFloatingBtn); }, true,
    Preferences::IsEnabled(Preferences::UseFloatingBtn)));

  _settingsMenu.Append(new MenuEntryTools(
    "Auto save enabled cheats",
    [] { Preferences::Toggle(Preferences::AutoSaveCheats); }, true,
    Preferences::IsEnabled(Preferences::AutoSaveCheats)));
  _settingsMenu.Append(new MenuEntryTools(
    "Auto save favorites",
    [] { Preferences::Toggle(Preferences::AutoSaveFavorites); }, true,
    Preferences::IsEnabled(Preferences::AutoSaveFavorites)));
  _settingsMenu.Append(new MenuEntryTools(
    "Auto load enabled cheats at starts",
    [] { Preferences::Toggle(Preferences::AutoLoadCheats); }, true,
    Preferences::IsEnabled(Preferences::AutoLoadCheats)));
  _settingsMenu.Append(new MenuEntryTools(
    "Auto load favorites at starts",
    [] { Preferences::Toggle(Preferences::AutoLoadFavorites); }, true,
    Preferences::IsEnabled(Preferences::AutoSaveFavorites)));
  _settingsMenu.Append(new MenuEntryTools(
    "Load enabled cheats now", [] { Preferences::LoadSavedEnabledCheats(); },
    nullptr));
  _settingsMenu.Append(new MenuEntryTools(
    "Load favorites now", [] { Preferences::LoadSavedFavorites(); }, nullptr));
}

bool PluginMenuTools::operator()(EventList &eventList, Time &delta)
{
  if( g_mode == HEXEDITOR ) {
    if( _hexEditor(eventList) ) g_mode = NORMAL;
    return (false);
  }
  ...
  // Process Event
  for( size_t i = 0; i < eventList.size(); i++ ) _ProcessEvent(eventList[i]);
...
void PluginMenuTools::_RenderBottom(void)
{
  ...
  // Draw Framework version
  {
    ...
    Renderer::DrawLine(30, posY, 12 * 6, blank);
    posY += 10;
