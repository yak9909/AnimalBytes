...
KeyboardImpl::~KeyboardImpl(void)
{
  ...
  //    iter->Clear();
  for( TouchKeyString *tks : _strKeys ) delete tks;
}
...
void KeyboardImpl::SetHexadecimal(bool isHex) { _isHex = isHex; }

bool KeyboardImpl::IsHexadecimal(void) const { return (_isHex); }

void KeyboardImpl::SetMaxInput(u32 max) { _max = max; }

void KeyboardImpl::CanAbort(bool canAbort) { _canAbort = canAbort; }

std::string &KeyboardImpl::GetInput(void) { return (_userInput); }

std::string &KeyboardImpl::GetMessage(void) { return (_text); }

std::string &KeyboardImpl::GetTitle(void) { return _title; }
...
{
  if( !_customKeyboard ) return;
  if( !_strKeys.size() || entry >= static_cast<int>(_strKeys.size()) ||
      !_strKeys[entry]->CanUse() )
    entry = -1;
  ...
  if( entry != -1 && _displayScrollbar ) {
    ...
    if( keyRow > positionRow + 6 - 2 ) {
      ...
      _scrollSize =
        ((positionRow * 36.01f) + 15) / _scrollJump - _scrollPosition;
...
{
  bool mustReset =
    (_strKeys.size() != input.size()) || resetScroll || _isIconKeyboard;

  int count = input.size();

  if( mustReset ) _ChangeManualKey(0, false);
  ...
  int posY =
    (count < 6) ? (20 + (200 - ((30 * count) + 6 * (count - 1))) / 2) : 30;
  ...
  for( const std::string &str : input ) {
    ...
    if( !mustReset ) tks->SetPosition(0xFFFF, origPosY[i++]);
...
void KeyboardImpl::Populate(const std::vector<CustomIcon> &input,
                            bool resetScroll)
{
  bool mustReset =
    (_strKeys.size() != input.size()) || resetScroll || _customKeyboard;
  ...
  for( const CustomIcon &ico : input ) {
    ...
    if( ico.sizeX != 30 || ico.sizeY != 30 )
      tks = new TouchKeyString(
        CustomIcon(Icon::DefaultCustomIcon.pixArray,
                   Icon::DefaultCustomIcon.sizeX, Icon::DefaultCustomIcon.sizeY,
                   ico.isEnabled),
        box, true);
    else
      tks = new TouchKeyString(ico, box, true);

    if( !mustReset ) tks->SetPosition(0xFFFF, origPosY[j++]);
...
int KeyboardImpl::Run(void)
{
  ...
  EventManager manager(EventManager::EventGroups::GROUP_KEYS |
                       EventManager::EventGroups::GROUP_TOUCH);
  Clock clock;
  ...
  while( _isOpen ) {
    ...
    if( !_customKeyboard ) {
      ...
      if( _errorMessage && inputChanged ) _errorMessage = false;
...
exit:
  ...
  if( menu && !menu->IsOpen() && ret != SLEEP_ABORT ) ScreenImpl::Clean();
  if( _mustRelease ) ProcessImpl::Play(false);
  return (ret);
}

void KeyboardImpl::Close(void) { _isOpen = false; }
...
void KeyboardImpl::_RenderTop(void)
{
  ...
  Renderer::DrawSysStringReturn(reinterpret_cast<const u8 *>(_text.c_str()),
                                posX, posY, maxX,
                                Preferences::Settings.MainTextColor, maxY);

  // IF error
  if( _errorMessage && !_error.empty() ) {
    if( posY < 120 ) posY += 48;
    Renderer::DrawSysStringReturn(reinterpret_cast<const u8 *>(_error.c_str()),
                                  posX, posY, maxX, red, maxY);
  }
}
...
void KeyboardImpl::_RenderBottom(void)
{
  ...
  if( !_customKeyboard ) {
    ...
    // Draw input
    Renderer::DrawSysString(_userInput.c_str(), posX, posY, 300, theme.Input,
                            _offset);

    // Draw cursor
    if( _showCursor && _blinkingClock.GetElapsedTime() < Seconds(0.5f) )
      Renderer::DrawLine(_cursorPositionOnScreen + posX, 21, 1, theme.Cursor,
                         16);
  ...
  // Draw custom keyboard
    ...
    Renderer::DrawRect2(background, theme.BackgroundMain,
...
void KeyboardImpl::_ProcessEvent(Event &event)
{
  ...
  if( event.type == Event::KeyPressed ) {
    ...
    if( event.key.code == X && !_customKeyboard && _layout != QWERTY &&
        _canChangeLayout ) {
      keyPressIntended = true;
      _userInput.clear();
      _ClearKeyboardEvent();
    ...
    if( _customKeyboard && (event.key.code & (Key::Down | Key::Up | Key::Left |
                                              Key::Right | Key::A)) ) {
      keyPressIntended = true;
  ...
  if( event.type == Event::KeyDown ) {
    ...
    if( _customKeyboard && inputPassedTime ) {
      if( event.key.code &
          (Key::Down | Key::Up | Key::Left | Key::Right | Key::A) ) {
        keyPressIntended = true;
  ...
  if( !keyPressIntended && _onKeyboardEvent != nullptr && _owner != nullptr &&
      (event.type == Event::KeyDown || event.type == Event::KeyPressed ||
...
void KeyboardImpl::_UpdateScroll(float delta, bool ignoreTouch)
{
  ...
  IntVector touchPos(ignoreTouch ? IntVector(0, 0)
                                 : (IntVector)Touch::GetPosition());
  ...
  if( _displayScrollbar ) {
    ...
    _currentPosition =
      (_scrollPosition * _scrollJump) / 36;  //(_scrollPosition / 36);
    ...
    for( TouchKeyString *tks : _strKeys ) tks->Update(isTouchDown, touchPos);
  }
}
...
void KeyboardImpl::_QwertyLowCase(void)
{
  ...
  /*low case*/
  _QwertyKeys.emplace_back('q', pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back('w', pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back('e', pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back('r', pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back('t', pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back('y', pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back('u', pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back('i', pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back('o', pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back('p', pos);
  pos.leftTop.x += 25;
  ...
  _QwertyKeys.emplace_back('a', pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back('s', pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back('d', pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back('f', pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back('g', pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back('h', pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back('j', pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back('k', pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back('l', pos);
  pos.leftTop.x += 25;
  ...
  _QwertyKeys.emplace_back(KEY_CAPS, Icon::DrawCapsLockOn, pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back('z', pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back('x', pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back('c', pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back('v', pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back('b', pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back('n', pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back('m', pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back(',', pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back('.', pos);
  ...
  pos.size.x = 40;
  _QwertyKeys.emplace_back("+=@", pos, KEY_SYMBOLS);
  pos.leftTop.x += 40;
  _QwertyKeys.emplace_back("\uE008", pos, KEY_SMILEY);
  pos.leftTop.x += 40;
  pos.size.x = 120;
...
void KeyboardImpl::_QwertySymbols(void)
{
  ...
  _QwertyKeys.emplace_back('&', pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back('1', pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back('2', pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back('3', pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back(KEY_BACKSPACE, Icon::DrawClearSymbol, pos);
  ...
  _QwertyKeys.emplace_back('-', pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back('_', pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back('=', pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back("\u00F7", pos);
  ...
  _QwertyKeys.emplace_back('+', pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back('4', pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back('5', pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back('6', pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back(KEY_ENTER, Icon::DrawEnterKey, pos);
  ...
  _QwertyKeys.emplace_back("\u2192", pos, KEY_SYMBOLS_PAGE);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back('\\', pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back(';', pos);
  ...
  _QwertyKeys.emplace_back('/', pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back('7', pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back('8', pos);
  pos.leftTop.x += 25;
  ...
  pos.leftTop.x += 120;
  pos.size.x = 25;
  _QwertyKeys.emplace_back('0', pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back('.', pos);
  ...
  _QwertyKeys.emplace_back("\u00A5", pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back("\u00B5", pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back("\u00A7", pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back('1', pos);
...
void KeyboardImpl::_QwertyNintendo()
{
  ...
  _QwertyKeys.emplace_back("\uE005" /* R */, pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back("\uE054" /* ZL */, pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back("\uE055" /* ZR */, pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back("\uE006" /* DPAD */, pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back("\uE041" /* DPAD Wii*/, pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back(KEY_BACKSPACE, Icon::DrawClearSymbol, pos);
  ...
  _QwertyKeys.emplace_back("\uE053" /* r */, pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back("\uE050" /* L Stick */, pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back("\uE051" /* R Stick */, pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back("\uE042" /* A Wii */, pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back("\uE043" /* B Wii */, pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back(KEY_ENTER, Icon::DrawEnterKey, pos);
  ...
  _QwertyKeys.emplace_back("\u2192", pos, KEY_NINTENDO_PAGE);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back("\uE040" /* Power */, pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back("\uE044" /* Home */, pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back("\uE045" /* + */, pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back("\uE046" /* - */, pos);
  ...
  _QwertyKeys.emplace_back("\uE048" /* 2 */, pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back("\uE049" /* Stick */, pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back("\uE04A" /* C */, pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back("\uE04B" /* Z */, pos);
  ...
  /*page 2*/
  _QwertyKeys.emplace_back("\uE079" /* DPAD UP */, pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back("\uE07B" /* DPAD DOWN */, pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back("\uE07C" /* DPAD LEFT */, pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back("\uE07D" /* DPAD RIGHT */, pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back("\uE07E" /* DPAD UP&DOWN */, pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back("\uE07F" /* DPAD LEFT&RIGHT */, pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back("\uE077" /* Wii Stick */, pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back("\uE078" /* Wii Power */, pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back("\uE056" /* Enter */, pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back("\uE057" /* Space*/, pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back(KEY_BACKSPACE, Icon::DrawClearSymbol, pos);
  ...
  _QwertyKeys.emplace_back("\uE007" /* Clock */, pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back("\uE008" /* Happy */, pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back("\uE009" /* Angry */, pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back("\uE00A" /* Sad */, pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back("\uE00B" /* ExpressionLess */, pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back("\uE00C" /* Sun */, pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back("\uE00D" /* Cloud */, pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back("\uE00E" /* Umbrella */, pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back("\uE00F" /* Snowman */, pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back("\uE06B" /* ? */, pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back(KEY_ENTER, Icon::DrawEnterKey, pos);
  ...
  _QwertyKeys.emplace_back("\uE018" /*  */, pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back("\uE019" /* Arrow Right */, pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back("\uE01A" /* Arrow Left */, pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back("\uE01B" /* Arrow Up */, pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back("\uE01C" /* Arow Down */, pos);
  pos.leftTop.x += 25;
  _QwertyKeys.emplace_back("\uE01E" /* Camera */, pos);
  pos.leftTop.x += 25;
...
void KeyboardImpl::_Decimal(void)
{
  ...
  for( ; iter != end; ++iter ) (*iter).Enable(false);
}
...
void KeyboardImpl::_Hexadecimal(void)
{
  ...
  if( !_HexaDecimalKeys.empty() ) return;
...
static int UnitsToAdvance(const char *str, u32 target)
{
  ...
  // Skip UTF8 sig
  if( s[0] == 0xEF && s[1] == 0xBB && s[2] == 0xBF ) s += 3;
...
void KeyboardImpl::_ScrollUp(void)
{
  ...
  // If input is empty
  if( strLength == 0 ) goto empty;

  {
    // Get units to advance to go to the next char
    const int units = UnitsToNextChar(cstr + _cursorPositionInString,
                                      strLength - _cursorPositionInString);
...
void KeyboardImpl::_ScrollDown(void)
{
...

  {
    ...
    const int units = UnitsToPreviousChar(cstr + _cursorPositionInString,
                                          _cursorPositionInString);
  ...
  if( _cursorPositionInString < 0 ) _cursorPositionInString = 0;
...
enum { CLEAR_NOT_PRESSED = 0, CLEAR_JUST_PRESSED, CLEAR_QUICK_MODE };
    ...
    if( ret != -1 ) {
      ...
      if( ret == KEY_BACKSPACE ) {
        ...
        if( backspaceFastMode == CLEAR_JUST_PRESSED ) {
          if( !backspacetimer.HasTimePassed(FastModeWaitTime) ) return (false);
          backspaceFastMode = CLEAR_QUICK_MODE;
        }
        if( backspaceFastMode == CLEAR_QUICK_MODE ) {
          if( !backspacetimer.HasTimePassed(FastClearingFrame) ) return (false);
        }
        backspacetimer.Restart();
      ...
      if( ret == KEY_ENTER ) {
        ...
        return false;
               ...
               (!_max || Utils::GetSize(_userInput) < _max) ) {
        ...
        _useSymbols = false;
        _useCaps = false;
        if( !_useNintendo ) _nintendoPage = 0;
        _symbolsPage = 0;
      }
        ...
        if( _layout == DECIMAL && _userInput.length() >= 18 ) return (false);
...
{
  if( _layout == QWERTY ) {
    if( _compare != nullptr ) return (_compare((void *)&_userInput, _error));
    return (true);
  }

  // In case there's no convert function, always consider input as valid
  if( _convert == nullptr ) return (true);
  ...
  if( _compare == nullptr ) return (true);
  ...
  if( _isIconKeyboard ) {
    if( _manualKey == -1 ) {
      if( key & (Key::Down | Key::Left) ) {
        ...
        if( !_strKeys[tempKey]->CanUse() ) tempKey = 0;
        _ChangeManualKey(tempKey);
      }
      else if( key & (Key::Up | Key::Right) ) {
        int tempKey = _displayScrollbar ? std::min((int)_strKeys.size() - 1,
                                                   _currentPosition + 20)
                                        : _strKeys.size() - 1;
        if( !_strKeys[tempKey]->CanUse() ) tempKey = (int)_strKeys.size() - 1;
        _ChangeManualKey(tempKey);
      }
      else
        return;
    ...
    if( key & (Key::Down | Key::Up | Key::Left | Key::Right) ) {
      if( key & Key::Down ) {
        ...
        do {
          tempKey += 4;
        } while( tempKey < static_cast<int>(_strKeys.size()) &&
                 !_strKeys[tempKey]->CanUse() && tempKey - orig < 16 );

        if( tempKey >= static_cast<int>(_strKeys.size()) ||
            tempKey - orig >= 16 )
          tempKey = orig;
        ...
        do {
          tempKey -= 4;
        } while( tempKey > 0 && !_strKeys[tempKey]->CanUse() &&
                 orig - tempKey < 16 );

        if( tempKey < 0 || orig - tempKey >= 16 ) tempKey = orig;
        ...
        do {
          tempKey++;
                 ...
                 ((u32)tempKey & 3) != 0 && !_strKeys[tempKey]->CanUse() );
        ...
        do {
          tempKey--;
        } while( tempKey > 0 && ((u32)tempKey & 3) != 3 &&
                 !_strKeys[tempKey]->CanUse() );

        if( tempKey < 0 || ((u32)tempKey & 3) == 3 ) tempKey = orig;
      ...
      if( key & Key::Down ) {
        ...
        do {
          tempKey++;
                 ...
                 !_strKeys[tempKey]->CanUse() && tempKey - orig < 4 );
        ...
        if( tempKey < 0 || orig - tempKey >= 4 ) tempKey = orig;
...
void KeyboardImpl::_ChangeManualKey(int newVal, bool playSound)
{
  ...
  if( _manualKey != _prevManualKey && !preventRecursion && playSound )
    if( _onKeyboardEvent != nullptr && _owner != nullptr &&
        _manualKey != _prevManualKey && !preventRecursion ) {
...
bool KeyboardImpl::operator()(int &out)
{
  ...
  if( !_CheckKeys() ) return (false);

  if( _userInput.size() ) {
    out = (int)_userInput[0];
    if( out >= '0' && out <= '9' ) out -= '0';
    if( out >= 'A' && out <= 'F' ) out = 10 + out - 'A';
    _userInput.pop_back();
    return (true);
  }
  return (false);
