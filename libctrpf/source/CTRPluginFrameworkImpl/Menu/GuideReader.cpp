...
static MenuFolderImpl *CreateFolder(std::string path)
{
  ...
  if( !directories.empty() ) {
    for( size_t i = 0; i < directories.size(); i++ ) {
      ...
      if( subMFolder != nullptr ) mFolder->Append(subMFolder);
    }
  }
  directories.clear();
  // List all files
  ...
  if( !files.empty() ) {
    for( size_t i = 0; i < files.size(); i++ ) {
      ...
      std::string fname =
        fpos != std::string::npos ? files[i].substr(0, fpos) : files[i];
...
GuideReader::GuideReader(void)
  : _isOpen(false),
    _menu(CreateFolder("Guide"), Icon::DrawFile),
...
void GuideReader::_LoadBMP(void)
{
  ...
  _image =
    new BMPImage(_currentDirectory.GetFullName() + "/" + _bmpList[_currentBMP]);
  _image = PostProcess(_image, 280, 200);
}
...
bool GuideReader::_ProcessEvent(Event &event)
{
  ...
  if( _guideTB.IsOpen() ) {
    ...
    // If folder changed
      ...
      if( item != nullptr &&
          Directory::Open(_currentDirectory, item->note) == 0 ) {
        _currentDirectory.ListFiles(_bmpList, ".bmp");
        if( !_bmpList.empty() ) {
          _currentBMP = 0;
    ...
    // If an file entry was selected by user
      ...
      if( entry != _last ) {
        ...
        file.Rewind();
        _text.clear();
        if( _text.capacity() < size ) _text.reserve(size);
    ...
    if( event.type == Event::KeyPressed ) {
      if( event.key.code == Key::L && _currentBMP > 0 ) {
        ...
        _LoadBMP();
               ...
               static_cast<size_t>(_currentBMP) < _bmpList.size() - 1 ) {
        _currentBMP++;
        _LoadBMP();
      }
    }
    else if( event.type == Event::TouchSwipped ) {
      if( event.swip.direction == Event::SwipDirection::Left &&
          _currentBMP > 0 ) {
        _currentBMP--;
        _LoadBMP();
      }
      else if( event.swip.direction == Event::SwipDirection::Right &&
               static_cast<size_t>(_currentBMP) < _bmpList.size() - 1 ) {
        _currentBMP++;
        _LoadBMP();
      }
...
void GuideReader::Open(void) { _isOpen = true; }

void GuideReader::Close(void) { _isOpen = false; }

bool GuideReader::IsOpen(void) { return (_isOpen); }
}  // namespace CTRPluginFramework