...
PluginMenuActionReplay::PluginMenuActionReplay()
  : _topMenu{"ActionReplay"},
    _noteBtn(Button::Icon | Button::Toggle, IntRect(90, 30, 25, 25),
             Icon::DrawInfo),
...
bool PluginMenuActionReplay::operator()(EventList &eventList, const Time &delta)
{
  ...
  if( _noteBtn() ) {
    if( !_topMenu.IsNoteOpen() ) {
      if( !_topMenu.ShowNote() ) _noteBtn.Disable();
    }
    else
      _topMenu.CloseNote();
  }
...
void PluginMenuActionReplay::_ProcessEvent(EventList &eventList)
{
  for( Event &event : eventList ) {
    ...
    if( action == MenuEvent::MenuClose ) Window::BottomWindow.Close();
  }
}
...
void PluginMenuActionReplay::_EditorBtn_OnClick(void)
{
  ...
  if( !item ) return;
  if( !item->IsFolder() ) options.push_back("Code");
...
void PluginMenuActionReplay::_NewBtn_OnClick(void)
{
  ...
  if( !ActionReplay_GetInput(name) ) return;
...
void PluginMenuActionReplay::_DuplicateBtn_OnClick(void)
{
  ...
  MenuEntryActionReplay *old =
    reinterpret_cast<MenuEntryActionReplay *>(current);
  MenuEntryActionReplay *dup =
    new MenuEntryActionReplay(current->name, current->note);
...
void PluginMenuActionReplay::_OpenFileBtn_OnClick(void)
{
  ...
  if( Utils::FilePicker(newPath, ".txt") == -1 ) return;
...
void PluginMenuActionReplay::SaveCodes(void)
{
  ...
  // If we're here, it means that everything went fine, so delete old file and
