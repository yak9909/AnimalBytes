...
void MenuItem::NoteChanged(void) { Flags.noteChanged = true; }

bool MenuItem::HasNoteChanged(void) const { return (Flags.noteChanged); }

void MenuItem::HandledNoteChanges(void) { Flags.noteChanged = false; }
...
void MenuItem::_DisableFolder(MenuFolderImpl *folder)
{
  ...
  for( ; iter != folder->_items.end(); ++iter ) {
    ...
    if( item->_IsStarred() ) PluginMenuImpl::UnStar(item);
...
{
  if( Flags.isVisible == false ) return;

  Flags.isVisible = false;

  // If the item is starred
  if( Flags.isStarred ) PluginMenuImpl::UnStar(this);

  // If it's an entry, disable it
  if( _type == MenuType::Entry ) {
    MenuEntryImpl *e = reinterpret_cast<MenuEntryImpl *>(this);

    if( e != nullptr ) e->Disable();
  }
  // Else refresh the menu to be sure that we're not currently in a hidden
  // folder
...
{
  if( Flags.isVisible == true ) return;
