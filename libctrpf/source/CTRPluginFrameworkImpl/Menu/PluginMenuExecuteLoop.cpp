...
PluginMenuExecuteLoop::PluginMenuExecuteLoop(void) { _firstInstance = this; }

void PluginMenuExecuteLoop::WriteEnabledCheatsToFile(
  Preferences::Header &header, File &file)
{
  if( _firstInstance == nullptr ) return;
  ...
  while( count ) {
    ...
    if( file.Write(uids.data(), sizeof(u32) * uids.size()) != 0 ) goto error;
...
{
  if( _firstInstance == nullptr || entry == nullptr ) return;
  ...
  if( entry->_flags.isRadio && id != -1 ) {
    ...
    vector.erase(
      std::remove_if(
        ...
        [](MenuEntryImpl *e) {
          if( e->_flags.state && e->_flags.isRadio &&
              e->_radioId == g_radioId ) {
...
void PluginMenuExecuteLoop::Remove(MenuEntryImpl *entry)
{
  ...
  if( _firstInstance->_builtinEnabledList.empty() ) return;
...
void PluginMenuExecuteLoop::RemoveAR(MenuEntryActionReplay *entry)
{
  ...
  if( entry->context.hooks != nullptr ) {
    for( Hook &hook : *entry->context.hooks ) hook.Disable();
...
void PluginMenuExecuteLoop::ExecuteAR(void)
{
  ...
  for( MenuEntryActionReplay *ar : _firstInstance->_arEnabledList ) {
    if( ar->GameFunc != nullptr )
      ar->GameFunc((MenuEntry *)(ar));  ///< cast is just for the warning,
                                        ///< properly handled on the callback
  }
}
...
bool PluginMenuExecuteLoop::operator()(void)
{
  ...
  if( _builtinEnabledList.empty() ) return false;
  ...
  for( MenuEntryImpl *entry : _builtinEnabledList ) {
    ...
    if( entry->IsEntry() &&
        entry->_Execute() )  ///< _Execute returns true if the entry have to be
    ...
    {
      MenuEntryActionReplay *ar = reinterpret_cast<MenuEntryActionReplay
    *>(entry);
  ...
  if( needToRemove ) {
    _builtinEnabledList.erase(
      std::remove_if(_builtinEnabledList.begin(), _builtinEnabledList.end(),
                     [](MenuEntryImpl *e) { return e->_MustBeRemoved(); }),
