...
MenuEntryImpl::MenuEntryImpl(const std::string &name, const std::string &note,
                             MenuEntry *owner)
  : MenuItem(MenuType::Entry)
{
  this->name = name;
  this->firstName = name;
  this->note = note;
...
MenuEntryImpl::MenuEntryImpl(const std::string &name, FuncPointer func,
                             const std::string &note, MenuEntry *owner)
  : MenuItem(MenuType::Entry)
{
  this->name = name;
  this->firstName = name;
...
void MenuEntryImpl::SetArg(void *arg) { _arg = arg; }

void *MenuEntryImpl::GetArg(void) const { return (_arg); }
...
bool MenuEntryImpl::IsActivated(void) const { return (_flags.state); }

MenuEntry *MenuEntryImpl::AsMenuEntry(void) const { return (_owner); }

//##############################################################

std::string &MenuEntryImpl::GetNote()
{
  if( _owner == nullptr || _owner->Hotkeys.Count() == 0 ) return (note);
...
{
  if( _flags.state ) return (false);
  if( _flags.justChanged ) return (false);
  return (true);
}
...
bool MenuEntryImpl::_Execute(void)
{
  ...
  if( GameFunc != nullptr ) GameFunc(_owner);
