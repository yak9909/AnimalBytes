...
MenuFolderImpl::MenuFolderImpl(MenuFolder *owner, const std::string &name,
...
u32 MenuFolderImpl::ItemsCount(void) const { return (_items.size()); }
...
std::vector<MenuFolder *> MenuFolderImpl::GetFolderList(void) const
{
  ...
  for( MenuItem *item : _items ) {
    if( item->IsFolder() ) {
      ...
      if( folder != nullptr ) folders.push_back(folder);
    }
  }
...
{
  if( start >= _items.size() ) return;

  if( start + count >= _items.size() ) count = _items.size() - start;

  auto begin = _items.begin() + start;
  auto end = begin + count;

  if( destroy ) {
    for( auto b = begin, e = end; b != e; ++b ) delete *b;
  }
