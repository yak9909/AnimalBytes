...
static const Key ktable[14] = {ZL,       L,     DPadUp, DPadLeft, DPadRight,
                               DPadDown, Start, ZR,     R,        X,
                               Y,        A,     B,      Select};

static const char *stable[14] = {FONT_ZL, FONT_L,  FONT_DU, FONT_DL, FONT_DR,
                                 FONT_DD, "Start", FONT_ZR, FONT_R,  FONT_X,
                                 FONT_Y,  FONT_A,  FONT_B,  "Select"};

static int GetIndex(int code)
{
  for( int i = 0; i < 14; ++i )
    if( ktable[i] == code ) return (i);
    ...
    _message(message)
{
  for( int i = 0, posY = 32; i < 7; ++i, posY += 25 ) {
    Button b(Button::Icon | Button::Toggle, IntRect(30, posY, 20, 20),
             Icon::DrawCheckBox);
    _checkboxs.push_back(b);
  }
  for( int i = 0, posY = 32; i < 7; ++i, posY += 25 ) {
    Button b(Button::Icon | Button::Toggle, IntRect(200, posY, 20, 20),
             Icon::DrawCheckBox);
    _checkboxs.push_back(b);
  }
  for( int i = 0; i < 16; ++i ) {
...
#define DPADY (Key::DPadDown | Key::DPadUp)
    ...
    for( int i = 0; i < 14; i++ ) {
      if( _checkboxs[i].GetState() ) _keys |= ktable[i];
    }
...
void HotkeysModifier::_DrawTop(void) const
{
  ...
  int posY = 61;
  Renderer::DrawSysStringReturn((const u8 *)_message.c_str(), 40, posY, 300,
...
void HotkeysModifier::_DrawBottom(void)
{
  ...
  for( int i = 0 + skip, posY = 32 + (skip * 25); i < 7; ++i, posY += 9 )
    Renderer::DrawSysString(stable[i], 50, posY, 290,
                            Preferences::Settings.MainTextColor);
  for( int i = 7 + skip, posY = 32 + (skip * 25); i < 14; ++i, posY += 9 )
    Renderer::DrawSysString(stable[i], 220, posY, 290,
                            Preferences::Settings.MainTextColor);
}
