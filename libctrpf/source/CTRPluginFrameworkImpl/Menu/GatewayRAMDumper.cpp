...
bool GatewayRAMDumper::operator()(void)
{
  ...
  if( size ) buffer = Heap::Alloc(size);
  ...
  for( Region &region : _regions ) {
    ...
    while( _currentAddress < _endAddress ) {
      ...
      while( ssize-- > 0 ) *pool++ = *reinterpret_cast<u8 *>(_currentAddress++);
...
static u32 HowManyAreSelected(MenuFolderImpl &folder)
{
  ...
  for( u32 i = 0; i < folder.ItemsCount(); i++ ) {
    if( folder[i]->AsMenuEntryImpl().IsActivated() ) count++;
  }
  return (count);
}
...
bool GatewayRAMDumper::_SelectRegion(void)
{
  Menu menu("Gateway RAM Dumper",
            "Select the region(s) to dump.\n\n"
            "Key:\n"
            "    " FONT_A
            ": (De)Select the current region\n"
  ...
  for( Region &region : _regions ) {
    std::string &&name =
      Utils::Format("%08X - %08X", region.startAddress, region.endAddress);
...
void GatewayRAMDumper::_OpenFile(void)
{
  ...
  if( msgBox() ) {
    ...
    if( keyboard.Open(_fileName) == -1 ) return;
  }
  else {
    // Default name
  ...
  *buffer++ = 0;
  ...
  for( Region &region : _regions ) {
    ...
    *buffer++ = regionSize;
    ...
    // Flush region
    svcFlushProcessDataCache(ProcessImpl::ProcessHandle, region.startAddress,
                             regionSize);
  }
