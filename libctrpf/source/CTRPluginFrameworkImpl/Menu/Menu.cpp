...
Menu::Menu(const std::string &title, const std::string &footer,
...
MenuFolderImpl *Menu::Open(MenuFolderImpl *folder, int selector)
{
  ...
  _selector =
    std::max((int)0, (int)std::min(selector, (int)(folder->ItemsCount() - 1)));
  _folder = folder;
...
Menu::~Menu(void) { delete _folder; }

void Menu::Append(MenuItem *item) const { _folder->Append(item); }

void Menu::Remove(MenuItem *item)
{
  for( auto iter = _folder->_items.begin(); iter < _folder->_items.end();
       ++iter ) {
    if( *iter == item ) {
...
MenuFolderImpl *Menu::GetFolder(void) const { return (_folder); }

MenuFolderImpl *Menu::GetRootFolder() const { return (_root); }
...
MenuItem *Menu::GetSelectedItem()
{
  ...
  if( count > 0 ) {
    if( _selector >= count ) _selector = std::max(0, (int)count - 1);
...
void Menu::Draw(void) const
{
  ...
  // Draw title
  int width =
    Renderer::DrawSysString(_folder->name.c_str(), posX, posY, XMAX, title);
  ...
  if( _iconCallback != nullptr ) {
    ...
    for( ; i < max; i++ ) {
      ...
      if( item->_type == MenuType::Entry ) {
        Renderer::DrawSysCheckBox(item->name.c_str(), posX, posY, XMAX, c,
                                  item->AsMenuEntryImpl().IsActivated());
        // Renderer::DrawSysString(item->name.c_str(), posX + 20, posY, XMAX,
        // c);
      }
      // MenuEntryTools
        ...
        if( e->UseCheckBox ) {
          ...
          if( e->Icon != nullptr ) e->Icon(posX, posY);
          Renderer::DrawSysString(item->name.c_str(), posX + 20, posY, XMAX, c);
        }
      }
      // MenuEntryFreeCheat
  ...
  if( drawFooter ) {
    ...
    if( !_folder->GetNote().empty() ) {
      int posY = 40;
      Renderer::DrawSysStringReturn((u8 *)_folder->GetNote().c_str(), 35, posY,
                                    295, Preferences::Settings.MainTextColor,
                                    190);
    }
  }
}
...
int Menu::ProcessEvent(Event &event, MenuItem **userchoice)
{
  ...
  if( _folder->ItemsCount() == 0 ) {
    if( event.type == Event::KeyPressed && event.key.code == Key::B ) {
      ...
      if( p != nullptr ) {
        _folder = p;
        if( userchoice ) *userchoice = reinterpret_cast<MenuItem *>(p);
    ...
    switch( event.key.code ) {
      case A: {
        ...
        // MenuEntryTools
          ...
          if( e->UseCheckBox ) e->TriggerState();
          if( e->Func != nullptr ) e->Func();
          if( e->FuncArg != nullptr ) e->FuncArg(e);
        ...
        // MenuEntryActionReplay
          ...
          MenuEntryActionReplay *e =
            reinterpret_cast<MenuEntryActionReplay *>(item);
...
{
  if( !_input.HasTimePassed(Milliseconds(200)) ) return;
  while( _selector > 0 && step-- > 0 ) _selector--;
  if( step > 0 )
...
void Menu::Update(const Time &delta)
{
  ...
  if( _lastSelectedItem != item ) {
    ...
    if( namesize < 280.f ) _selectedNameSize = 0;
  ...
  if( !_selectedNameSize ||
      (!_reverseFlow && !_scrollClock.HasTimePassed(Seconds(2))) ||
      (_reverseFlow && !_scrollClock.HasTimePassed(Seconds(1))) )
    return;

  float deltaAsSeconds = delta.AsSeconds();

  if( deltaAsSeconds > 0.5f ) return;
...
void Menu::CloseNote(void) { _noteTB.Close(); }

bool Menu::IsNoteOpen(void) { return (_noteTB.IsOpen()); }

void Menu::Insert(MenuItem *item)
{
  if( !item || !_folder ) return;
...
{
  if( !_folder->ItemsCount() ) return nullptr;
