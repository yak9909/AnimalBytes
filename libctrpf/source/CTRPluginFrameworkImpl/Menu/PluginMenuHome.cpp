...
PluginMenuHome::PluginMenuHome(std::string &name, bool showNoteBottom)
  :
    ...
    _noteTB(
      "", "",
      showNoteBottom ? IntRect(20, 46, 280, 124) : IntRect(40, 30, 320, 180)),

    _showStarredBtn(Button::Toggle | Button::Sysfont | Button::Rounded,
                    "Favorite", IntRect(30, 70, 120, 30), Icon::DrawFavorite),
    _hidMapperBtn(Button::Sysfont | Button::Rounded, "Mapper",
                  IntRect(165, 70, 120, 30), Icon::DrawController),
    _gameGuideBtn(Button::Sysfont | Button::Rounded, "Game Guide",
                  IntRect(30, 105, 120, 30), Icon::DrawGuide),
    _searchBtn(Button::Sysfont | Button::Rounded, "Search",
               IntRect(165, 105, 120, 30), Icon::DrawSearch),
    _arBtn(Button::Sysfont | Button::Rounded, "ActionReplay",
           IntRect(30, 140, 120, 30)),
    _toolsBtn(
      Button::Sysfont | Button::Rounded, "Tools",
      showNoteBottom ? IntRect(99, 172, 120, 30) : IntRect(165, 140, 120, 30),
      Icon::DrawTools),

    // _closeBtn(*this, nullptr, IntRect(275, 24, 20, 20), Icon::DrawClose),
    _keyboardBtn(Button::Icon, IntRect(130, 30, 25, 25), Icon::DrawKeyboard),
    _controllerBtn(Button::Icon, IntRect(170, 30, 25, 25),
                   Icon::DrawGameController25),

    _AddFavoriteBtn(Button::Icon | Button::Toggle, IntRect(50, 30, 25, 25),
                    Icon::DrawAddFavorite),
    _InfoBtn(Button::Icon | Button::Toggle, IntRect(90, 30, 25, 25),
             Icon::DrawInfo)
{
  ...
  if( !Preferences::Settings.AllowActionReplay ) _arBtn.Lock();
  if( !Preferences::Settings.AllowSearchEngine ) _searchBtn.Lock();
}
...
void PluginMenuHome::Append(MenuItem *item) const { _folder->Append(item); }
...
void PluginMenuHome::Refresh(void)
{
  ...
  do {
    // If the currently selected folder is root
    // Nothing to do
    if( _starred->_container == nullptr ) break;
  ...
  if( _selector >= static_cast<int>(folder->ItemsCount()) ) _selector = 0;
}
...
static void ScrollUp(int &selector, MenuFolderImpl &folder, int step)
{
  ...
  if( !SelectableEntryCount(folder) ) return;
...
void PluginMenuHome::_ProcessEvent(Event &event)
{
  ...
  *********************************/
  if( folder->ItemsCount() > 0 && event.key.code != Key::Touchpad &&
      (event.type < Event::TouchBegan || event.type > Event::TouchSwipped) ) {
...
void PluginMenuHome::_RenderTop(void)
{
  ...
  int posYbak = posY;
  int width = Renderer::DrawSysString(folder->name.c_str(), posX, posY,
                                      maxWidth, maintext);
  Renderer::DrawLine(posX, posY, width, maintext);
  posY += 7;

  if( _showVersion && !_starMode && !folder->HasParent() )
    Renderer::DrawSysString(_versionStr.c_str(), _versionPosX, posYbak, 360,
                            maintext);
  ...
  for( ; i < max; i++ ) {
    ...
    if( item->_type == MenuType::Entry ) {
      ...
      if( entry->GameFunc != nullptr )
        Renderer::DrawSysCheckBox(name, posX, posY, 350, fg,
                                  entry->IsActivated(), offset);
      else {
  ...
  *************/
  ...
  if( folder && (*folder)[_selector] ) {
    ...
    // If selected object is a folder
      ...
      MenuFolderImpl *e =
        reinterpret_cast<MenuFolderImpl *>((*folder)[_selector]);
    ...
    // If selected object is an entry
      ...
      MenuEntryImpl *e =
        reinterpret_cast<MenuEntryImpl *>((*folder)[_selector]);
...
void PluginMenuHome::_TriggerEntry(void)
{
  ...
  if( _selector >= static_cast<int>(folder->ItemsCount()) ) return;
...
void PluginMenuHome::_keyboardBtn_OnClick(void)
{
  ...
  if( e->MenuFunc != nullptr ) e->MenuFunc(e->_owner);
}

void PluginMenuHome::_actionReplayBtn_OnClick() { _mode = 4; }

void PluginMenuHome::_gameGuideBtn_OnClick(void) { _mode = 2; }

void PluginMenuHome::_searchBtn_OnClick(void) { _mode = 3; }

void PluginMenuHome::_toolsBtn_OnClick(void) { _mode = 5; }
...
void PluginMenuHome::AddPluginVersion(u32 version)
{
  ...
  sprintf(buffer, "[%d.%d.%d]", static_cast<int>(version & 0xFF),
...
{
  if( folder != _root ) {
    if( _folder == folder ) _folder = _folder->_Close(_selector, false);
    if( _starred == folder ) _starred = _starred->_Close(_selector, true);
  }
}

void PluginMenuHome::UpdateNote(void)
{
  if( !ShowNoteBottom ) return;

  MenuFolderImpl *folder = _starMode ? _starred : _folder;

  if( !folder || !((*folder)[_selector]) ) return;
