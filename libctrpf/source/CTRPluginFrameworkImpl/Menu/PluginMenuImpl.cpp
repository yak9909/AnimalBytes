...
void PluginMenuImpl::Append(MenuItem *item) const { _home->Append(item); }
...
void PluginMenuImpl::Callback(CallbackPointer callback)
{
  // If the callback is going to be added, make sure it's not in the trash bin
  {
    ...
    if( _callbacksTrashBin.size() ) {
      auto it = std::find(_callbacksTrashBin.begin(), _callbacksTrashBin.end(),
                          callback);
      if( it != _callbacksTrashBin.end() ) _callbacksTrashBin.erase(it);
    }
  }

  if( std::find(_callbacks.begin(), _callbacks.end(), callback) ==
      _callbacks.end() )
...
void PluginMenuImpl::RemoveCallback(CallbackPointer callback)
{
  ...
  if( std::find(_callbacksTrashBin.begin(), _callbacksTrashBin.end(),
                callback) == _callbacksTrashBin.end() )
...
int PluginMenuImpl::Run(void)
{
  ...
  EventManager openManager(EventManager::EventGroups::GROUP_KEYS |
                           EventManager::EventGroups::GROUP_TOUCH_AND_SWIPE);
  Clock clock;
  Clock inputClock;
  int mode = 0;
  ...
  if( Controller::IsKeyPressed(Key::Start) ||
      Controller::IsKeyDown(Key::Start) )
  ...
  if( Preferences::Settings.AllowSearchEngine ) search.RestoreSearchState();

  // Load AR Cheats
  if( Preferences::Settings.AllowActionReplay ) ar.Initialize();

  if( _showMsg ) OSD::Notify("Plugin ready!", Color::White, Color());

  // Main loop
  while( _pluginRun ) {
    // Check Event
    eventList.clear();
    while( (_isOpen && openManager.PollEvent(event)) ||
           (!_isOpen && closedManager.PollEvent(event)) || _forceOpen ) {
      bool isHotkeysDown = false;

      // If it's a KeyPressed event
      if( event.type == Event::KeyPressed &&
          inputClock.HasTimePassed(Milliseconds(500)) &&
          (!Preferences::IsEnabled(Preferences::UseFloatingBtn) || _isOpen) &&
      ...
      if( _forceOpen || isHotkeysDown ) {
        ...
        else  ///< Open menu
        {
          ...
          if( OnOpening != nullptr ) continueOpening = OnOpening();
    ...
    if( _isOpen ) {
      if( mode == 0 ) { /* Home */
        if( OnFrame != nullptr ) OnFrame(delta);
      ...
      */
      else if( mode == 2 ) { /* Guide */
        if( guide(eventList, delta) ) mode = 0;
      }
      else if( mode == 3 ) { /* Search */
        if( search(eventList, delta) ) mode = 0;
        goto __skip;
      }
      else if( mode == 4 ) { /* ActionReplay  */
        if( ar(eventList, delta) ) mode = 0;
      }
      else if( mode == 5 ) { /* Tools  */
        if( tools(eventList, delta) ) mode = 0;
      }
      _aboutToOpen = false;
      // End frame
    ...
    else  // menu is closed
    {
        ...
        if( _callbacksTrashBin.size() ) {
          _callbacks.erase(
            std::remove_if(_callbacks.begin(), _callbacks.end(),
                           [](CallbackPointer cb) {
                             auto &trashbin =
                               _runningInstance->_callbacksTrashBin;
                             auto foundIter =
                               std::find(trashbin.begin(), trashbin.end(), cb);
      ...
      // static KeySequenceImpl konamicode({ DPadUp, DPadUp, DPadDown, DPadDown,
      // DPadLeft, DPadRight, DPadLeft, DPadRight, B, A, B, A });

      // if (konamicode())
      //     OSDImpl::MessColors = !OSDImpl::MessColors;

      if( _wasOpened ) _wasOpened = false;
    }
  }
...
{
  if( _runningInstance == nullptr ) return;
...
void PluginMenuImpl::LoadEnabledCheatsFromFile(
  const Preferences::Header &header, File &settings)
{
  if( _runningInstance == nullptr ) return;
  ...
  if( settings.Read(uids.data(), sizeof(u32) * header.enabledCheatsCount) ==
      0 ) {
    for( u32 &uid : uids ) {
...
void PluginMenuImpl::LoadFavoritesFromFile(const Preferences::Header &header,
                                           File &settings)
{
  if( _runningInstance == nullptr ) return;
...
void PluginMenuImpl::LoadHotkeysFromFile(const Preferences::Header &header,
                                         File &settings)
{
  if( _runningInstance == nullptr || header.hotkeysCount == 0 ) return;
  ...
  for( size_t count = 0; count < header.hotkeysCount; count++ ) {
    if( settings.Read(buffer, sizeof(u32) * 2) == 0 ) {
      if( settings.Read(&buffer[2], sizeof(u32) * buffer[1]) == 0 ) {
        ...
        if( item == nullptr || !item->IsEntry() )
          return;  ///< An error occurred, abort operation
        ...
        if( entry->Hotkeys.Count() == buffer[1] ) {
          for( u32 i = 0; i < buffer[1]; i++ ) {
            ...
            if( callback != nullptr ) callback(entry, i);
          }
...
void PluginMenuImpl::WriteEnabledCheatsToFile(Preferences::Header &header,
                                              File &settings)
{
  if( _runningInstance == nullptr ) return;
  ...
  for( MenuItem *item : folder->_items ) {
    if( item->IsEntry() &&
        reinterpret_cast<MenuEntryImpl *>(item)->IsActivated() )
...
void PluginMenuImpl::WriteFavoritesToFile(Preferences::Header &header,
                                          File &settings)
{
  if( _runningInstance == nullptr ) return;
...
void PluginMenuImpl::ExtractHotkeys(HotkeysVector &hotkeys,
                                    MenuFolderImpl *folder, u32 &size)
{
...
void PluginMenuImpl::WriteHotkeysToFile(Preferences::Header &header,
                                        File &settings)
{
  if( _runningInstance != nullptr ) {
    ...
    if( size ) {
      ...
      for( Preferences::HotkeysInfos &hkInfos : hotkeys ) {
        ...
        for( u32 keys : hkInfos.hotkeys ) buffer[i++] = keys;
      }
...
{
  if( _runningInstance != nullptr ) _runningInstance->_pluginRun = false;
}

void PluginMenuImpl::ForceOpen(void)
{
  if( _runningInstance != nullptr ) _runningInstance->_forceOpen = true;
}
...
bool PluginMenuImpl::GetHexEditorState() const { return _hexEditorState; }

void PluginMenuImpl::ShowWelcomeMessage(bool showMsg) { _showMsg = showMsg; }

MenuFolderImpl *PluginMenuImpl::GetRoot() const { return (_home->_root); }

bool PluginMenuImpl::IsOpen(void) const { return (_isOpen); }

bool PluginMenuImpl::WasOpened(void) const { return (_wasOpened); }
