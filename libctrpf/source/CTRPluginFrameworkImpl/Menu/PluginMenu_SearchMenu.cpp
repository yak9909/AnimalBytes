...
SearchMenu::SearchMenu(Search *&curSearch, HexEditor &hexEditor, bool &inEditor,
                       bool &useHexInput)
  : _hexEditor(hexEditor),
    _currentSearch(curSearch),
    _submenu{
      {"Show game"}},  //"Play 1 frame", "Play 2 frames", "Play 5 frames" }},
    _inEditor(inEditor),
...
bool SearchMenu::ProcessEvent(EventList &eventList, Time &delta)
{
  ...
  for( size_t i = 0; i < eventList.size(); i++ ) {
    ...
    if( event.type == Event::EventType::KeyPressed ) {
      // if (_currentSearch != nullptr)
      {
        switch( event.key.code ) {
          ...
          case Key::CPadDown: {
            _selector =
              std::min((int)(_selector + 5), (int)(_resultsAddress.size() - 1));
            _startFastScroll.Restart();
            break;
          }
          case Key::CPadUp: {
          ...
          case Key::DPadDown: {
            _selector =
              std::min((int)(_selector + 1), (int)(_resultsAddress.size() - 1));
            _startFastScroll.Restart();
            break;
          }
          case Key::DPadLeft: {
          ...
          case Key::DPadRight: {
            _index = std::min((int)(_index + _selector + 500),
                              (int)(_currentSearch->ResultsCount / 500 * 500));
            _selector = 0;
            _startFastScroll.Restart();
            Update();
            break;
    ...
    // Hold
      ...
      if( _currentSearch != nullptr &&
          _startFastScroll.HasTimePassed(Seconds(0.5f)) &&
          _fastScroll.HasTimePassed(Seconds(0.1f)) ) {
        switch( event.key.code ) {
          case Key::CPadDown: {
            _selector =
              std::min((int)(_selector + 5), (int)(_resultsAddress.size() - 1));
...
void SearchMenu::Draw(void)
{
  ...
  /**/ {
    ...
    /**/ posY += 10;
        ...
        if( _currentSearch == nullptr || _resultsAddress.size() == 0 ||
            _resultsNewValue.size() == 0 ) {
          _submenu.Draw();
          return;
        }
        ...
        for( u32 i = start; i < end; i++ ) {
          if( i >= _resultsAddress.size() ) return;
          ...
          // Address
          Renderer::DrawString((char *)_resultsAddress[i].c_str(), pos, posy,
                               black);
        ...
        std::string str =
          std::to_string(start) + "-" +
          std::to_string(
            std::min((u32)(start + 10), (u32)_currentSearch->ResultsCount)) +
...
void SearchMenu::Update(void)
{
  ...
  if( _currentSearch == nullptr ) {
    ...
    _index = 0;
    if( _submenu.OptionsCount() > 4 )
      _submenu.ChangeOptions({"Show game"});  //, "Play 1 frame", "Play 2
                                              // frames", "Play 5 frames" });
    return;
  }

  if( _submenu.OptionsCount() < 5 && !_currentSearch->IsFirstUnknownSearch() ) {
    _submenu.ChangeOptions(
      {"Edit", "Jump in editor", "New cheat", "Export", "Export all",
       "Converter",
       "Show Game"});  //, "Play 1 frame", "Play 2 frames", "Play 5 frames" });
  }
  ...
  _currentSearch->ReadResults(_index, _resultsAddress, _resultsNewValue,
                              _resultsOldValue);

  if( static_cast<size_t>(_selector) >= _resultsAddress.size() ) _selector = 0;
...
{
  if( _export.IsOpen() ) return;

  if( File::Open(_export, "ExportedAddresses.txt",
...
void SearchMenu::_Edit(void)
{
  ...
  switch( _currentSearch->GetType() ) {
    case SearchFlags::U8: {
      u8 value =
        *(u8 *)(address);  // strtoul(_resultsNewValue[_selector].c_str(),
                           // NULL, 16);

      int res = keyboard.Open(value, value);

      if( res != -1 ) {
        if( Process::CheckAddress(address) ) *(u8 *)(address) = value;
      }
      break;
    }
    case SearchFlags::U16: {
      u16 value =
        *(u16 *)(address);  // strtoul(_resultsNewValue[_selector].c_str(),
                            // NULL, 16);

      int res = keyboard.Open(value, value);

      if( res != -1 ) {
        if( Process::CheckAddress(address) ) *(u16 *)(address) = value;
      }
      break;
    }
    case SearchFlags::U32: {
      u32 value =
        *(u32 *)(address);  // strtoul(_resultsNewValue[_selector].c_str(),
                            // NULL, 16);

      int res = keyboard.Open(value, value);

      if( res != -1 ) {
        if( Process::CheckAddress(address) ) *(u32 *)(address) = value;
      }
      break;
    }
      /*  case SearchFlags::U64:
        {
            u64 value = *(u64
        *)(address);//strtoull(_resultsNewValue[_selector].c_str(), NULL, 16);
    ...
    case SearchFlags::Float: {
      float value =
        *(float
            *)(address);  // strtof(_resultsNewValue[_selector].c_str(), NULL);

      int res = keyboard.Open(value, value);

      if( res != -1 ) {
        if( Process::CheckAddress(address) ) *(float *)(address) = value;
      }
      break;
    }
      /* case SearchFlags::Double:
       {
           double value = *(double
       *)(address);//strtod(_resultsNewValue[_selector].c_str(), NULL);
...
{
  if( !PluginMenuImpl::GetRunningInstance()->GetHexEditorState() ) return;
  u32 address = strtoul(_resultsAddress[_selector].c_str(), NULL, 16);
...
{
  if( !_alreadyExported ) {
    if( !_export.IsOpen() ) _OpenExportFile();
    _export.WriteLine("");
    time_t t = time(NULL);
    char *ct = ctime(&t);
  ...
  std::string str =
    _resultsAddress[_selector] + " : " + _resultsNewValue[_selector];
...
void SearchMenu::_ExportAll(void)
{
  ...
  for( int i = _selector; i < _selector + 10; i++ ) {
    if( i >= static_cast<int>(_resultsAddress.size()) ) break;
...
{
  MessageBox(Color::Green << "Info",
             "Press " FONT_B " to return to the menu.")();
