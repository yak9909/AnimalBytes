...
void WatchPointManager::Initialize(void)
{
  ...
  u32 (*K_GetCtxId)(void) = [](void) {
    u32 contextId;
#ifndef _MSC_VER
    __asm__ __volatile__("mrc p15, 0, %[val], c13, c0, 1"
                         : [val] "=r"(contextId));
#endif
    return contextId;
  };
...
void WatchPointManager::SetWatchPoint(u32 address, u32 size, u32 kind)
{
  ...
  if( manager._total >= 2 ) return;

  if( size == 0 || (offset + size) > 4 || kind == WatchPoint::Disabled ) return;

  u32 id = manager._watchPoints[0]._kind == WatchPoint::Disabled ? 0 : 1;

  if( manager._watchPoints[!id]._address == address ) return;  ///< No duplicate
  ...
  svcKernelSetState(id == 0 ? 0x10005 : 0x10006, address, WCR,
                    manager._contextId);  // set watchpoint
...
void WatchPointManager::HandleException(ERRF_ExceptionInfo* excep,
                                        CpuRegisters* regs)
{
  auto& manager = _singleton;
...

  {
    ...
    for( wpid = 0; wpid < 2; ++wpid ) {
      if( manager._watchPoints[wpid]._address == excep->far ) break;
    }

    if( wpid >= 2 ) return;  ///< Didn't found the watchpoint ???
