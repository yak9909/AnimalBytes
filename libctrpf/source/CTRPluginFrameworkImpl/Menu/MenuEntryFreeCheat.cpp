...
MenuEntryFreeCheat::MenuEntryFreeCheat(const std::string& text, u32 addr,
                                       u8 value)
  : MenuEntryImpl(text),
    Func(Write8),
    Address(addr)
...
/*MenuEntryFreeCheat::MenuEntryFreeCheat(const std::string& text, u32 addr, u64
value) : MenuEntryImpl(text), Func(Write64), Address(addr)
{
  _type = MenuType::FreeCheat;
  Type = Type_e::Bits64;
  Value.Bits64 = value;
...
/*MenuEntryFreeCheat::MenuEntryFreeCheat(const std::string& text, u32 addr,
double value) : MenuEntryImpl(text), Func(WriteDouble), Address(addr)
{
  _type = MenuType::FreeCheat;
  Type = Type_e::Double;
  Value.Double = value;
...
MenuEntryFreeCheat::MenuEntryFreeCheat(
  const Preferences::SavedCheats& savedCheats)
    ...
    Address(savedCheats.address)
{
  ...
  if( savedCheats.flags >> 8 && Preferences::AutoLoadCheats ) Enable();
...
bool MenuEntryFreeCheat::TriggerState(void) { return (_TriggerState()); }
