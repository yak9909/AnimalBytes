...
{
  if( _inEditor ) {
    if( _hexEditor(eventList) ) _inEditor = false;
    Renderer::EndFrame();
    return (false);
  }
  ...
  // Do search
  if( _inSearch ) {
    if( _cancelBtn() ) _cancelBtn_OnClick();
    ...
    if( finish ) {
      ...
      if( _step == 1 ) _PopulateSearchType(false);
      ...
      _cancelBtn.Disable();
      _resetBtn.Enable();
      if( _searchHistory.size() ) _undoBtn.Enable();
    }
    return (false);
  }
  ...
  // Check buttons
  if( Window::BottomWindow.MustClose() ) return (true);
  ...
  if( _memoryRegions() ) {
    ...
    Region reg;
    Region &region = (_memoryRegions.SelectedItem == 0)
                       ? reg
                       : _regionsList[_memoryRegions.SelectedItem - 1];
    ...
    switch( _memoryRegions.SelectedItem ) {
      case 0:
        ...
        break;
      default:
        startRange = region.startAddress;
        endRange = region.endAddress;
        isEnabled = true;
        break;
    }
  ...
  if( _searchSize() ) {
    switch( _searchSize.SelectedItem ) {
      case 0:
        _valueTextBox.ValueType = NumericTextBox::Type::Bits8;
        break;
      case 1:
        _valueTextBox.ValueType = NumericTextBox::Type::Bits16;
        break;
      case 2:
        _valueTextBox.ValueType = NumericTextBox::Type::Bits32;
        break;
        // case 3: _valueTextBox.ValueType = NumericTextBox::Type::Bits64;
        // break;
      case 3:
        _valueTextBox.ValueType = NumericTextBox::Type::Float;
        break;
        //  case 5: _valueTextBox.ValueType = NumericTextBox::Type::Double;
        //  break;
      default:
        break;
    }
...
void PluginMenuSearch::GetRegionsList(std::vector<Region> &list)
{
  ...
  for( Region &region : _regionsList ) list.push_back(region);
}
...
void PluginMenuSearch::RestoreSearchState(void)
{
  ...
  if( dir.IsOpen() && dir.ListFiles(filenames, "Step") ) {
    while( filenames.size() > 5 ) filenames.erase(filenames.begin());

    for( const std::string &fn : filenames ) {
      if( _currentSearch != nullptr ) _searchHistory.push_back(_currentSearch);
    ...
    if( _currentSearch != nullptr ) {
      ...
      // Set search size
      _searchSize.SelectedItem =
        __builtin_ffs(TypeFlags(_currentSearch->GetType())) - 1;
      if( _searchSize.SelectedItem == 4 ) _searchSize.SelectedItem = 3;
...
static void ClearSearchFolder(void)
{
  ...
  if( dir.ListFiles(files) > 0 ) {
    for( std::string &name : files ) File::Remove("Search/" + name);
  }
}
...
void PluginMenuSearch::_searchBtn_OnClick(void)
{
  ...
  if( _memoryRegions.SelectedItem == -1 ||
      _memoryRegions.SelectedItem > static_cast<int>(_regionsList.size()) )
    return;
  ...
  // If first search
  if( _currentSearch == nullptr ) parameters.flags = (u32)SearchFlags::First;
  ...
  switch( _searchSize.SelectedItem ) {
    case 0:
      parameters.flags |= (u32)SearchFlags::U8;
      break;
    case 1:
      parameters.flags |= (u32)SearchFlags::U16;
      break;
    case 2:
      parameters.flags |= (u32)SearchFlags::U32;
      break;
    // case 3: parameters.flags |= (u32)SearchFlags::U64;  break;
    case 3:
      parameters.flags |= (u32)SearchFlags::Float;
      break;
  ...
  // Set Search Flags
  parameters.flags |=
    _searchType.SelectedItem == 0 ? 0 : (u32)SearchFlags::Unknown;
...
void PluginMenuSearch::_cancelBtn_OnClick(void) { _currentSearch->Cancel(); }
...
void PluginMenuSearch::_resetBtn_OnClick(void)
{
  ...
  if( _currentSearch != nullptr ) delete _currentSearch;
...
void PluginMenuSearch::_undoBtn_OnClick(void)
{
  ...
  if( !_searchHistory.size() ) _undoBtn.Disable();
  _searchMenu.Update();
}
...
s32 PluginMenuSearch::_ShowProgressWindow(void *arg)
{
  ...
  while( currentSearch->IsInProgress ) {
    ...
    // Draw Result count
    sprintf(buf, "Hit(s): %u",
            static_cast<unsigned int>(currentSearch->ResultsCount));
  ...
  while( true ) {
    ...
    std::string res2 =
      "in " + std::to_string(currentSearch->SearchTime.AsSeconds()) + "s";
    ...
    Controller::Update();
    if( Controller::IsKeyPressed(Key::A) ) break;
  }
...
void PluginMenuSearch::_ListRegion(void)
{
  ...
  int i;
  Region bakRegion =
    (_memoryRegions.SelectedItem > 0 &&
     _regionsList.size() > static_cast<size_t>(_memoryRegions.SelectedItem))
      ? _regionsList[_memoryRegions.SelectedItem]
      : (Region){0};
  Result ret;
  ...
  svcQueryProcessMemory(&meminfo, &page_info, target, 0x00100000);
  {
    ...
    _memoryRegions.Add(
      Utils::Format("%08X-%08X", reg.startAddress, reg.endAddress));
  }
  ...
  while( save_addr < 0x50000000 ) {
    ...
    if( R_FAILED(ret) ) {
      if( meminfo.base_addr >= 0x50000000 ) break;
    ...
    if( meminfo.base_addr == 0x06000000 || meminfo.base_addr == 0x07000000 ||
        meminfo.base_addr == 0x07500000 ||
        meminfo.base_addr == __ctru_linear_heap )
      continue;
#endif
    if( meminfo.state != 0x0 && meminfo.state != 0x2 && meminfo.state != 0x3 &&
        meminfo.state != 0x6 ) {
      if( meminfo.perm & MEMPERM_READ ) {
        Region reg =
          (Region){meminfo.base_addr, meminfo.base_addr + meminfo.size};
        _regionsList.push_back(reg);
        _memoryRegions.Add(
          Utils::Format("%08X-%08X", reg.startAddress, reg.endAddress));
        i++;
