...
enum { JumpTo = 0, JumpRelative = 1, JumpToValue = 2 };
...
HexEditor::HexEditor(u32 target)
  : _submenu{{"New cheat", "Jump to", "Jump relative", "Jump to value",
              ...
              "Browse history",
              "Clear history"}},  //, "Play 1 Frame", "Play 2 frames", "Play 5
                                  // frames" }},
...
{
  for( u32 i = 0; i < 3; ++i ) delete _views[i];
}
...
bool HexEditor::operator()(EventList& eventList)
{
  ...
  for( size_t i = 0; i < eventList.size(); i++ ) {
    ...
    if( !isSubMenuOpen ) _ProcessEvent(eventList[i]);
  }
  ...
  // If submenu is opened, check for option
    ...
    else if( !subchoice-- )  ///< Converter
    {
      ...
      _ClearHistory();
    // Fix Play X frames!!!!!
    /*else if (!subchoice--)  { Process::Play(1);  _ctx._flags |= DirtySrc;
    return false; } else if (!subchoice--)  { Process::Play(2);  _ctx._flags |=
    DirtySrc; return false; } else if (!subchoice--)  { Process::Play(5);
    _ctx._flags |= DirtySrc; return false; }*/
  }
...
u32 HexEditor::GetCursorAddress(void) { return _ctx._cursorAddress; }
...
void HexEditor::_ProcessEvent(Event& event)
{
  ...
  if( event.type == Event::KeyPressed ) {
    switch( event.key.code ) {
      case Key::A: {
        if( flags & DirtyMemoryCache ) _ApplyChanges();
        break;
      }
...
s32 HexEditor::_RenderTop(void* arg)
{
  ...
  // Addresses
  for( size_t i = 0; i < _ctx._items.size();
       i += obj->_viewCurrent->ItemsPerLine ) {
...
void HexEditor::_Update(void)
{
  ...
  if( flags & DirtyCursorCache ) {
    ...
    snprintf(_ctx._cursorAddressStr, 9, "%08X",
             static_cast<unsigned int>(_ctx._cursorAddress));
  }
}

void HexEditor::_CreateCheat(void)
{
  if( !Preferences::Settings.AllowActionReplay ) return;
...
void HexEditor::_MoveBackward(void)
{
  ...
  --_indexHistory;
  if( _indexHistory <= -1 ) _indexHistory = 0;
...
void HexEditor::_SaveThisAddress(void)
{
  ...
  if( _history.size() && _history.back() == address ) return;
...
void HexEditor::_BrowseHistory(void)
{
  ...
  for( u32 v : _history ) addresses.push_back(Utils::ToHex(v));
...
bool HexEditor::Goto(u32 address, bool updateCursor)
{
  ...
  if( region.base_addr == 0 ) return false;
...
void HexEditor::SetView(u32 view)
{
  ...
  if( flags & DirtyMemoryCache ) return;
...
void HexEditor::Refresh(void) { _ctx._flags |= DirtySrc; }

void HexEditor::_ApplyChanges(void)
{
  if( !(_ctx._flags & DirtyMemoryCache) || _ctx._flags & InvalidSrc ) return;

  for( Item& item : _ctx._items ) *(vu32*)item.address = item.value32;
...
{
  for( Item& item : _ctx._items ) item.value32 = *(vu32*)item.address;
...
{
  static const char* msg[2] = {"Enter the address to jump to:",
                               "Enter the offset to jump to:"};
...
void HexEditor::_JumpTo(int mode)
{
  ...
  if( mode == JumpToValue ) {
    ...
    if( !(MessageBox("Confirm",
                     Utils::Format("Do you want to jump to: 0x%08X ?", address),
...
{
  if( _ctx._flags & DirtyMemoryCache || _submenu.IsOpen() ) return;

  MemInfo currentRegion =
    ProcessImpl::GetMemRegion(_ctx._items.front().address);
...
HexEditor::Item& HexEditor::_GetSelectedItem(void)
{
  ...
  if( _viewCurrent->ItemsPerLine > 1 ) item += _ctx._cursorX >> 3;
...
bool HexEditor::Context::ScrollUp(u32 stride, bool updatecursor)
{
  ...
  if( target < 0x00100000 ) return false;
...
static inline s16 Min16(s16 left, s16 right) { return std::min(left, right); }

static inline s16 Max16(s16 left, s16 right) { return std::max(left, right); }
...
void HexEditor::IView::ProcessEventsCommon(Context& _ctx, Event& event,
                                           s16 maxX)
{
  // Key pressed
  if( event.type == Event::EventType::KeyPressed ||
      (event.type == Event::KeyDown &&
       _ctx._clock.HasTimePassed(_ctx._latency)) ) {
    _ctx._clock.Restart();
...
void HexEditor::ByteView::Draw(void)
{
  ...
  // Draw Cursor
  Renderer::DrawRect(_ctx._cursorPosX, _ctx._cursorPosY, 7, 10,
                     Color::DeepSkyBlue);
...
void HexEditor::ByteView::UpdateView(void)
{
  ...
  if( flags & DirtyCursor ) {
    ...
    _ctx._cursorY =
      target >> shiftY;  /// _ctx._cursorY = target / _viewCurrent->Stride
  }
...
void HexEditor::ByteView::EditValueAtCursor(u32 val)
{
  ...
  _ctx._flags |=
    DirtyMemoryCache | DirtyViewCache | DirtyCursorAddress | DirtyCursorPos;
}
...
void HexEditor::IntegerView::Draw(void)
{
  ...
  for( ; item != _ctx._items.end(); ) {
    ...
    for( int i = 0; i < 4; ++i, posX += 58 ) {
      ...
      // Values
      Renderer::DrawString(item++->valueCache.c_str(), posX, posYY,
                           Color::Black);
    }
    posY += 10;
  }
...
void HexEditor::IntegerView::UpdateView(void)
{
  ...
  if( flags & DirtyCursor ) {
    ...
    u32 targetX = (target - (target >> shift << shift))
                  << 1;  /// target % Stride * 2
  ...
  if( flags & DirtyCursorPos ) {
    ...
    // posX
    {
      ...
      _ctx._cursorPosX =
        startX + bytes * byteWidth + space + ((_ctx._cursorX & 1) ? 6 : 0);
    }
...
void HexEditor::AsmView::UpdateView(void)
{
  ...
  if( flags & DirtyCursorPos ) {
    ...
    // posX
    {
      ...
      _ctx._cursorPosX =
        startX + bytes * byteWidth + ((_ctx._cursorX & 1) ? 6 : 0);
    }
