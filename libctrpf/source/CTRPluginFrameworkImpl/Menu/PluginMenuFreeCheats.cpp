...
FreeCheats::FreeCheats(HexEditor &hexEditor)
  : _hexEditor(hexEditor),
    ...
    _saveBtn("Save", *this, &FreeCheats::_SaveBtn_OnClick,
             IntRect(40, 180, 73, 15)),
    _cancelBtn("Cancel", *this, &FreeCheats::_CancelBtn_OnClick,
               IntRect(123, 180, 73, 15)),
    _deleteBtn("Delete", *this, &FreeCheats::_DeleteBtn_OnClick,
               IntRect(206, 180, 73, 15)),
    _openInEditorBtn("Open in HexEditor", *this,
                     &FreeCheats::_OpenInEditorBtn_OnClick,
                     IntRect(40, 120, 240, 15)),
    _changeNameBtn("Change name", *this, &FreeCheats::_ChangeNameBtn_OnClick,
                   IntRect(40, 140, 240, 15)),
    _duplicateBtn("Duplicate", *this, &FreeCheats::_DuplicateBtn_OnClick,
                  IntRect(40, 160, 240, 15)),
    _u8CheckBox(60, 100),
...
FreeCheats *FreeCheats::GetInstance(void) { return (_instance); }
...
{
  if( _instance == nullptr || header.freeCheatsCount == 0 ) return;
  ...
  while( count > 0 ) {
    ...
    if( file.Read(freecheats.data(), nb * sizeof(SavedCheat)) != 0 ) break;

    for( SavedCheat &fc : freecheats ) _instance->Create(fc);
  ...
  if( _instance->_menu.GetFolder()->ItemsCount() ) {
    ...
    while( count > 0 ) {
      ...
      for( int i = 0; i < nb && index < folder->ItemsCount(); ++i, ++index ) {
        reinterpret_cast<MenuEntryFreeCheat *>(folder->_items[index])
          ->ToSavedSearch(svcheats[i]);
      }
...
void FreeCheats::_DrawBottom(void)
{
  ...
  if( _selectedFC == nullptr ) {
    ...
    Renderer::DrawSysString("\uE010\uE010  \uE00A  \uE010\uE010", 117, posY,
                            300, textcolor);
    posY += 10;
  ...
  _u32CheckBox.Draw(); */
  posY = 104;
  Renderer::DrawString("8Bits", 77, posY, textcolor);
  posY = 104;
  Renderer::DrawString("16Bits", 157, posY, textcolor);
  posY = 104;
...
void FreeCheats::_ProcessEvent(EventList &eventList)
{
  for( Event &event : eventList ) {
    ...
    if( menuevent == MenuEvent::SelectorChanged ) {
      if( _mustSave ) _CancelBtn_OnClick();
      _selectedFC = reinterpret_cast<MenuEntryFreeCheat *>(item);
      _UpdateInfos();
    }
    if( menuevent == MenuEvent::MenuClose ) Window::BottomWindow.Close();
  }
}
...
void FreeCheats::_UpdateInfos(void)
{
  if( _selectedFC != nullptr ) {
    ...
    // if (_selectedFC->Type == Type_e::Bits64)
    // _valueTextBox.SetValue(_selectedFC->Value.Bits64); if (_selectedFC->Type
    // == Type_e::Float) _valueTextBox.SetValue(_selectedFC->Value.Float); if
    // (_selectedFC->Type == Type_e::Double)
    // _valueTextBox.SetValue(_selectedFC->Value.Double);
  }
}
...
void FreeCheats::_Update(void)
{
  ...
  if( _selectedFC == nullptr ) {
    _selectedFC =
      reinterpret_cast<MenuEntryFreeCheat *>(_menu.GetSelectedItem());
    _UpdateInfos();
  }
...
{
  if( _nameChanged ) _selectedFC->name = _savedName;
  _mustSave = false;
  _nameChanged = false;
  _saveBtn.IsLocked = true;
  _cancelBtn.IsLocked = true;
...
void FreeCheats::_DeleteBtn_OnClick(void)
{
  ...
  if( !MessageBox(msg, DialogType::DialogYesNo)() ) return;
