...
extern "C" void __gsp__Update(u32 threadId, Handle eventHandle,
...
Result Initialize(void)
{
  const std::vector<u32> gspgpuRegisterInterruptPattern = {
    0xE92D4070, 0xE1A06003, 0xE59D5010, 0xEE1D3F70, 0xE2834080, 0xE59F3034,
    0xE3A0C000, 0xE584100C, 0xE9841004, 0xE5843000, 0xE5900000, 0xEF000032,
    0xE2101102, 0x4A000004, 0xE5940008, 0xE5850000, 0xE5940010, 0xE5860000,
    0xE5940004, 0xE8BD8070, 0x00130042,
  };

  const std::vector<u32> gspgpuRegisterInterruptPattern2 = {
    0xE92D4070, 0xE1A05003, 0xE59D6010, 0xEE1D4F70, 0xE59FC034,
    0xE3A03000, 0xE5A4C080, 0xE584100C, 0xE1C420F4,
    /* 0xE5900000,
      0xEF000032,
      0xE2101102,
      0x4A000004,
  ...
  u32 found = 0;
  u32 addr = Utils::Search(0x00100000, Process::GetTextSize(),
                           gspgpuRegisterInterruptPattern);
  Hook &hook = GSPRegisterInterruptReceiverHook;

  if( addr ) {
    hook.Initialize(addr + 0x2C, (u32)GSPGPU__RegisterInterruptHook)
      .SetFlags(USE_LR_TO_RETURN);
    hook.Enable();
  }
  else {
    for( addr = 0x00100000;
         !found && addr <= Process::GetTextSize() -
                             gspgpuRegisterInterruptPattern2.size(); ) {
      addr = Utils::Search(0x00100000, Process::GetTextSize(),
                           gspgpuRegisterInterruptPattern2);
      if( !addr ) return -1;
...
void Update(u32 threadId, Handle eventHandle, Handle sharedMemHandle)
{
  ...
  if( SharedMemoryBlock == nullptr ) {
    ...
    svcMapMemoryBlock(sharedMemHandle, reinterpret_cast<u32>(SharedMemoryBlock),
                      static_cast<MemPerm>(0x3),
                      static_cast<MemPerm>(0x10000000));
  }
  ...
  SharedFrameBuffers[1] =
    reinterpret_cast<FrameBufferInfoShared *>(base + 0x40);
...
void FrameBufferInfo::FillFrameBufferFrom(FrameBufferInfo &src)
{
  ...
  if( (format & 0xF) != GSP_RGB565_OES ) return;
  ...
  svcInvalidateProcessDataCache(CUR_PROCESS_HANDLE, (u32)src.framebuf0_vaddr,
                                width * src.framebuf_widthbytesize);
...
static void EnqueueEvent(s8 event, bool signal = true)
{
  ...
  if( signal ) svcSignalEvent(GSPEvent);
}
...
void SetFrameBufferInfo(FrameBufferInfoShared &src, int screen, bool convert)
{
  ...
  if( convert ) {
    ...
    fbs->fbInfo[0].framebuf0_vaddr =
      plgVAtoGameVa(src.fbInfo[0].framebuf0_vaddr);
    if( src.fbInfo[0].framebuf1_vaddr )
      fbs->fbInfo[0].framebuf1_vaddr =
        plgVAtoGameVa(src.fbInfo[0].framebuf1_vaddr);

    fbs->fbInfo[1].framebuf0_vaddr =
      plgVAtoGameVa(src.fbInfo[1].framebuf0_vaddr);
    if( src.fbInfo[1].framebuf1_vaddr )
      fbs->fbInfo[1].framebuf1_vaddr =
        plgVAtoGameVa(src.fbInfo[0].framebuf1_vaddr);
  }
