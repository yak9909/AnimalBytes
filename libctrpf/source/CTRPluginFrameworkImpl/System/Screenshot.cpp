...
bool Screenshot::OSDCallback(u32 isBottom, void *addr, void *addrB, int stride,
                             int format)
{
  ...
  if( _mode & SCREENSHOT_BOTH ) {
    ...
    if( (_mode & SCREENSHOT_TOP) && !isBottom ) {
      ...
      if( Process::CheckAddress((u32)addr) ) {
        std::copy(static_cast<u32 *>(addr), static_cast<u32 *>(addr) + fbSize,
                  reinterpret_cast<u32 *>(&ImgBuffer->top[0]));
    ...
    if( (_mode & SCREENSHOT_BOTTOM) && isBottom ) {
      if( Process::CheckAddress((u32)addr) ) {
        // Copy image data to buffer
        std::copy(static_cast<u32 *>(addr), static_cast<u32 *>(addr) + fbSize,
                  reinterpret_cast<u32 *>(&ImgBuffer->bottom[0]));
    ...
    if( (_mode & SCREENSHOT_BOTH) == 0 ) {
      ...
      __strex__(&isReady, 1);
      _task.Start();
      if( !(_mode & TIMED) ) OSDImpl::WaitingForScreenshot = false;
      _mode |= 8;
    }
  }
  ...
  if( isBottom && _display ) {
    ...
    if( _display < 120 )  ///< No problem
      Renderer::DrawString(
        (Prefix + Utils::Format(" - %03d.bmp", _filecount - 1)).c_str(), 0,
        posY, Color::White, Color::ForestGreen);
    else if( _display > 130 && _display < 250 )  ///< Error
      Renderer::DrawString("Screenshot: An error occured", 0, posY, Color::Red,
                           Color::White);
    else
      _display = -1;
...
static void ScreenToBMP_BGR8(Pixel *bmp, u32 padding, u8 *src, u32 width,
                             u32 stride)
{
  u32 height = 240;
...
static void ScreenToBMP_RGBA8(Pixel *bmp, u32 padding, u8 *src, u32 width,
                              u32 stride)
{
  u32 height = 240;
...
static void ScreenToBMP_RGB565(Pixel *bmp, u32 padding, u8 *src, u32 width,
                               u32 stride)
{
  u32 height = 240;
...
static void ScreenToBMP_RGB5A1(Pixel *bmp, u32 padding, u8 *src, u32 width,
                               u32 stride)
{
  u32 height = 240;
...
static void ScreenToBMP_RGBA4(Pixel *bmp, u32 padding, u8 *src, u32 width,
                              u32 stride)
{
  u32 height = 240;
...
static void ScreenToBMP(Pixel *bmp, const u32 padding, const u32 screen,
                        Screenshot::ImageBuffer *imgBuf)
{
  if( bmp == nullptr || imgBuf == nullptr ) return;
...
s32 Screenshot::TaskCallback(void *arg UNUSED)
{
  ...
  do {
    ...
    // Convert screens to bmp
    {
      ...
      if( screens & SCREENSHOT_TOP ) ScreenToBMP(bmp, 0, TOP, imgBuf);
    }
