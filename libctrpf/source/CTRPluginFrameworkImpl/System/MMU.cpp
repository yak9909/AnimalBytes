...
DescType L1Descriptor__GetType(u32 descriptor)
{
  ...
  if( pdesc.reserved.bits1_0 == 0b00 ) return Descriptor_TranslationFault;
  if( pdesc.reserved.bits1_0 == 0b01 ) return Descriptor_CoarsePageTable;
  if( pdesc.reserved.bits1_0 == 0b10 )
    return pdesc.section.bit18 == 0 ? Descriptor_Section
...
DescType L2Descriptor__GetType(u32 descriptor)
{
  ...
  if( pdesc.translationFault.bits1_0 == 0b01 ) return Descriptor_LargePage;
  if( pdesc.smallPage.bit1 == 1 ) return Descriptor_SmallPage;
  ...
  for( ; table != tableEnd; ++table ) {
    ...
    switch( L1Descriptor__GetType(descriptor.raw) ) {
      case Descriptor_CoarsePageTable: {
        u32 *l2table =
          (u32 *)((descriptor.coarsePageTable.addr << 10) - 0x40000000);
