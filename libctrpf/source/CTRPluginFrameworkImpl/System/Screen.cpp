...
ScreenImpl::ScreenImpl(const u32 lcdSetupInfo, const u32 fillColorAddress,
                       const bool isTopScreen)
...
{
  auto *screensFbs =
    reinterpret_cast<ScreensFramebuffers *>(FwkSettings::Header->heapVA);

  Top = new(_topBuf) ScreenImpl(0x10400400 | (1u << 31),
                                (0x10202000 | (1u << 31)) + 0x204, true);
  Bottom = new(_botBuf)
    ScreenImpl(0x10400500 | (1u << 31), (0x10202000 | (1u << 31)) + 0xA04);
...
void ScreenImpl::Fade(const float fade)
{
  ...
  for( int i = size; i > 0; --i ) {
    frameBuf = PrivColor::ToFramebuffer(
      frameBuf, PrivColor::FromFramebuffer(frameBuf).Fade(fade));
  }
}
...
u32 ScreenImpl::ImportFromGsp(void)
{
  ...
  if( !Process::CheckAddress(reinterpret_cast<u32>(
        _gameFrameBufferInfo.fbInfo[displayed].framebuf0_vaddr)) )
    return -1;
...
void ScreenImpl::Invalidate(void)
{
  ...
  svcInvalidateProcessDataCache(CUR_PROCESS_HANDLE, (u32)GetLeftFrameBuffer(),
                                size);
}
...
void ScreenImpl::Clear(bool applyFlagForCurrent)
{
  ...
  _frameBufferInfo.fbInfo[!_currentBuffer].FillFrameBufferFrom(
    _gameFrameBufferInfo.fbInfo[displayed]);

  Fade(0.3f);

  if( !applyFlagForCurrent ) return;
...
void ScreenImpl::Debug(void)
{
  ...
  if (_isTopScreen)
  {
    ...
    Renderer::DrawString(Utils::Format("FB0: %08X",
  _leftFramebuffers[0]).c_str(), 10, posY, Color::Blank, Color::Black);
    Renderer::DrawString(Utils::Format("FB1: %08X",
  _leftFramebuffers[1]).c_str(), 10, posY, Color::Blank, Color::Black);
    Renderer::DrawString(Utils::Format("Sel: %d", _originalBuffer).c_str(), 10,
  posY, Color::Blank, Color::Black);
  }
  else
  {
...
bool ScreenImpl::IsTopScreen(void) { return _isTopScreen; }

bool ScreenImpl::Is3DEnabled(void)
{
  if( !_isTopScreen ) return false;
...
u32 ScreenImpl::AcquireFromGsp(bool fade)
{
  ...
  while( (GPU_PSC0_CNT | GPU_PSC1_CNT | GPU_TRANSFER_CNT | GPU_CMDLIST_CNT) &
         1 )
    ;
...
void ScreenImpl::SwapBufferInternal(void) { _currentBuffer = !_currentBuffer; }
...
GSPGPU_FramebufferFormat ScreenImpl::GetFormat(void) const { return _format; }

u16 ScreenImpl::GetWidth(void) const { return _width; }

u32 ScreenImpl::GetStride(void) const { return _stride; }

u32 ScreenImpl::GetRowSize(void) const { return _rowSize; }

u32 ScreenImpl::GetBytesPerPixel(void) const { return _bytesPerPixel; }

u32 ScreenImpl::GetFrameBufferSize(void) const { return _stride * _width; }

u32 ScreenImpl::GetCurrentBufferID(void) const { return _currentBuffer; }

void ScreenImpl::GetFrameBufferInfos(int &rowStride, int &bpp,
                                     GSPGPU_FramebufferFormat &format) const
{
  rowStride = _stride;
  bpp = _bytesPerPixel;
  format = _format;
...
u8 *ScreenImpl::GetLeftFrameBuffer(bool current)
{
  return reinterpret_cast<u8 *>(
    _leftFrameBuffers[current ? _currentBuffer : !_currentBuffer]);
}
...
{
  if( !_isTopScreen ) return (nullptr);

  return reinterpret_cast<u8 *>(
    _rightFrameBuffers[current ? _currentBuffer : !_currentBuffer]);
}
