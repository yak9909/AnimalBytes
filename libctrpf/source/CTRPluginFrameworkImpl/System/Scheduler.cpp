...
int Scheduler::Schedule(const Task &task)
{
  ...
  if( task.context == nullptr ) return -1;
  ...
  if( ctx->affinity == -1 ) ctx->affinity = AllCores;

  // 2 threads on AppCore
  if( ctx->affinity & AppCore ) ctx->affinity |= AppCore1;
...
bool Scheduler::CurrentThreadIsTaskHandler(void)
{
  ...
  for( const Core &core : _singleton._cores )
    if( core.tlsPtr == current ) return true;
