...
namespace Kernel {
// static KRecursiveLock * CriticalSectionLock; // = (KRecursiveLock
// *)0xFFF2F0AC; static KThread  **  CurrentKThread = (KThread **)0xFFFF9000;
...
// static void    (*KRecursiveLock__Lock)(KRecursiveLock *lock); // = (void
// (*)(KRecursiveLock *))0xFFF1DC24; static void
// (*KRecursiveLock__Unlock)(KRecursiveLock *lock); // = (void
// (*)(KRecursiveLock *))0xFFF1DD64; static void
// (*KScheduler__AdjustThread)(KScheduler *scheduler, KThread *thread, u32
// oldSchedulingMask);// = (void (*)(KScheduler *, KThread*, u32))0xFFF1DD38;
...
{
  void (*K__Memcpy)(void *, void *, u32) = [](void *dst, void *src,
                                              u32 size) -> void {
...
std::string KAutoObject::GetName(void)
{
  ...
  void (*K_GetName)(KAutoObject *, char *) = [](KAutoObject *obj, char *out) {
    ...
    for( int i = 0; i < 8; ++i ) *out++ = token.name[i];
  };
...
{
  u32 (*KFunc)(KThread *) = [](KThread *thread) -> u32 { return thread->tls; };

  return (u32 *)svcCustomBackdoor((void *)KFunc, this);
}

bool KThread::IsPluginThread(void) { return *GetTls() == THREADVARS_MAGIC; }

KProcess *KThread::GetOwner(void)
{
  KProcess *(*K_GetOwner)(KThread *) =
    (KProcess * (*)(KThread *))[](KThread * thread)
  {
    return thread->owner;
  };
...
{
  u32 (*K_PatchCategory)(KProcess *, u32, u32) =
    [](KProcess *process, u32 newCategory, u32 offset) -> u32 {
  ...
  return svcCustomBackdoor((void *)K_PatchCategory, this, newCategory,
                           SystemImpl::IsNew3DS ? 0xB0 : 0xA8);
}
...
{
  u32 (*K_PatchMaxPriority)(KProcess *, u32, u32) =
    [](KProcess *process, u32 newPrio, u32 offset) -> u32 {
    KResourceLimit *resLimit =
      (KResourceLimit *)*(u32 *)((u32)process + offset);
  ...
  return svcCustomBackdoor((void *)K_PatchMaxPriority, this, newPrio,
                           SystemImpl::IsNew3DS ? 0x84 : 0x7C);
}

void KProcess::PatchMaxThreads(void)
{
  void (*K_PatchMaxThreads)(KProcess *, u32) = [](KProcess *process,
                                                  u32 offset) {
    KResourceLimit *resLimit =
      (KResourceLimit *)*(u32 *)((u32)process + offset);
  ...
  svcCustomBackdoor((void *)K_PatchMaxThreads, this,
                    SystemImpl::IsNew3DS ? 0x84 : 0x7C);
}

KAutoObject *KProcess::GetObjFromHandle(Handle handle)
{
  KAutoObject *(*K_GetObjFromHandle)(KProcess *, Handle) = [](KProcess *process,
                                                              Handle handle) {
    KProcessHandleTable *table =
      (KProcessHandleTable *)((u32)process +
                              (SystemImpl::IsNew3DS ? 0xDC : 0xD4));

    return table->handleTable[handle & 0xFFFF].obj;
  };

  if( __is_svmode() ) return K_GetObjFromHandle(this, handle);

  return (KAutoObject *)svcCustomBackdoor((void *)K_GetObjFromHandle, this,
                                          handle);
}
...
std::string KProcess::GetName(void)
{
  void (*K_GetName)(KProcess *, char *) = [](KProcess *process, char *out) {
    KCodeSet *codeset =
      (KCodeSet *)((u32)process + (SystemImpl::IsNew3DS ? 0xB8 : 0xD0));

    for( int i = 0; i < 8; ++i ) out[i] = codeset->processName[i];
  };
...
{
  void (*K_IncreaseMaxThreadLimit)(KResourceLimit *) =
    [](KResourceLimit *resLimit) { resLimit->maxThread += 4; };
