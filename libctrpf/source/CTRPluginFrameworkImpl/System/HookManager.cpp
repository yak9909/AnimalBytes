...
extern "C" void __hook__CheckTlsValue(u32 value)
{
  if( value && (value < 0x06000000 || value > 0x07000000) ) {
    ...
    const char *str =
      "Hi there! You found a game which do not match tls expected value.\n"
...
static inline u32 ARM__LDR_LR_PC(u32 offset) { return 0xE59FE000 | offset; }

static inline u32 ARM__STR_LR_PC(u32 offset) { return 0xE58FE000 | offset; }

static __attribute__((noinline)) void GenerateAsm(AsmWrapper &asmWrapper,
                                                  HookContext &ctx)
{
  u32 flags = ctx.flags;
  ...
  *code++ =
    ARMBranchLink(&asmWrapper.code[2],
                  (void *)SetHookContextInTLS);  ///< bl SetHookContextInTLS
  *code++ = 0xE8BD400F;                          ///< ldmfd sp!, {r0-r3, lr}
  ...
  if( flags & MITM_MODE ) {
    ...
    *code++ = ctx.callbackAddress;
    ...
    if( op == 0xEB )  // If instruction is branch with link
      *code = ARMBranchLink((void *)code,
                            (void *)DecodeARMBranch((vu32 *)ctx.targetAddress));
    else if( op == 0xEA )  // If instruction is branch
      *code = ARMBranch((void *)code,
                        (void *)DecodeARMBranch((vu32 *)ctx.targetAddress));
    else
  ...
  if( flags & USE_LR_TO_RETURN ) strLr = code++;

  if( flags & EXECUTE_OI_BEFORE_CB ) *code++ = ctx.overwrittenInstr;
  ...
  if( flags & EXECUTE_OI_AFTER_CB ) *code++ = ctx.overwrittenInstr;
...
HookResult HookManager::ApplyHook(HookContext &ctx)
{
  ...
  if( manager._asmWrappers.size() >= MAX_HOOKS ) {
    ...
    for( ; iter != manager._asmWrappers.end(); ++iter )
      if( iter->ctx == nullptr ) break;

    if( iter == manager._asmWrappers.end() ) return HookResult::TooManyHooks;
  }
  ...
  // Update AsmWrapper
  asmWrapper.ctx = &ctx;
  svcFlushProcessDataCache(Process::GetHandle(), (u32)&asmWrapper,
                           sizeof(AsmWrapper));
  ...
  hookWrapper.ctx = &ctx;
  svcFlushProcessDataCache(Process::GetHandle(), (u32)&hookWrapper,
                           sizeof(HookWrapper));
  // svcFlushDataCacheRange(&hookWrapper, sizeof(HookWrapper));

  // Apply hook
  svcInvalidateEntireInstructionCache();
  *(vu32 *)ctx.targetAddress =
    ARMBranch((void *)ctx.targetAddress, &hookWrapper.jumpCode);
...
HookResult HookManager::DisableHook(HookContext &ctx)
{
  ...
  if( ctx.index > MAX_HOOKS ) return HookResult::Success;
...
AsmWrapper &HookManager::GetFreeAsmWrapper(void)
{
  ...
  for( AsmWrapper &asmWrapper : _asmWrappers )
    if( asmWrapper.ctx == nullptr ) return asmWrapper;
...
HookWrapper &HookManager::GetFreeHookWrapper(s32 &index)
{
  ...
  for( ; begin != end; ++begin, ++index )
    if( begin->ctx == nullptr ) return *begin;
...
AsmWrapper &HookManager::GetAsmWrapper(HookContext *ctx)
{
  ...
  for( AsmWrapper &asmWrapper : _asmWrappers )
    if( asmWrapper.ctx == ctx ) return asmWrapper;
...
void HookManager::Lock() { _singleton._mutex.Lock(); }

void HookManager::Unlock() { _singleton._mutex.Unlock(); }
...
void HookManager::RecoverFromUnmapMemory()
{
  ...
  for( ; begin != end; ++begin ) {
    ...
    HookContext &ctx = *begin->ctx;
    *(vu32 *)ctx.targetAddress =
      ARMBranch((void *)ctx.targetAddress, &begin->jumpCode);
    svcFlushProcessDataCache(Process::GetHandle(), ctx.targetAddress, 4);
  }
}
