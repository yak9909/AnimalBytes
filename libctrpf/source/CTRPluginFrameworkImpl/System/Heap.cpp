...
{
  if( !__ctrpf_heap || !__ctrpf_heap_size ) abort();
...
void *MemAlign(size_t size, size_t alignment)
{
  ...
  for( shift = 4; shift < 32; shift++ ) {
    if( (1U << shift) == alignment ) break;
  }
  if( shift == 32 )  // Invalid alignment
    return nullptr;

  // Initialize the pool if it is not ready
  if( !g_heapPool.Ready() && !HeapInit() ) return nullptr;

  // Allocate the chunk
  MemChunk chunk;
  if( !g_heapPool.Allocate(chunk, size, shift) ) return nullptr;
...
void *Alloc(const size_t size) { return (MemAlign(size, 0x80)); }
...
void *Realloc(void *ptr, const size_t size)
{
  ...
  if( size < chunk.size ) return chunk.addr;
...
size_t SpaceFree(void) { return g_heapPool.GetFreeSpace(); }
}  // namespace Heap
}  // namespace CTRPluginFramework
