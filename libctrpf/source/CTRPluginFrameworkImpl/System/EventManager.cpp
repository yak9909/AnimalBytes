...
EventManager::EventManager(u32 captureEventsGroups)
  : _captureMask(captureEventsGroups)
{
}

bool EventManager::PollEvent(Event &event)
{
  if( PopEvent(event, false) ) return (true);
  return (false);
}

bool EventManager::WaitEvent(Event &event)
{
  if( PopEvent(event, true) ) return (true);
  return (false);
}

void EventManager::Clear()
{
  while( !_eventsQueue.empty() ) _eventsQueue.pop();
...
bool EventManager::PopEvent(Event &event, bool isBlocking)
{
  ...
  if( !_eventsQueue.empty() ) {
    ...
    _eventsQueue.pop();
    if( _eventsQueue.empty() ) _refresh = false;
    return (true);
  }
  return (false);
}
...
{
  if( _startTime.HasTimePassed(Milliseconds(50)) ) _eventsQueue.push(event);
}
...
void EventManager::ProcessEvents(void)
{
  ...
  // Touch Event
  if( (_captureMask & EventGroups::GROUP_TOUCH) ||
      (_captureMask & EventGroups::GROUP_TOUCH_AND_SWIPE) ) {
    touchPosition touchPos;

    if( Controller::IsKeyDown(Key::Touchpad) ) {
      hidTouchRead(&touchPos);
      if( touchPos.px != _lastTouch.px || touchPos.py != _lastTouch.py ||
          !_isTouching ) {
        _lastTouch = touchPos;
        if( _isTouching ) {
