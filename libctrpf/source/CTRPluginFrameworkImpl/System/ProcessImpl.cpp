...
extern "C" bool IsPaused(void) { return (ProcessImpl::IsPaused > 0); }
...
bool ProcessImpl::PatchProcess(u32 addr, u8 *patch, u32 length, u8 *original)
{
  ...
  if( !Process::CopyMemory((void *)addr, (void *)patch, length) ) goto error;
...
void ProcessImpl::GetHandleTable(
  KProcessHandleTable &table, std::vector<HandleDescriptor> &handleDescriptors)
{
  bool isNew3DS = System::IsNew3DS();

  // Copy KProcessHandleTable
  Kernel::Memcpy(&table, (void *)((u32)KProcessPtr + (isNew3DS ? 0xDC : 0xD4)),
                 sizeof(KProcessHandleTable));

  u32 count = table.handlesCount;

  handleDescriptors.resize(count);
  Kernel::Memcpy(handleDescriptors.data(), table.handleTable,
                 count * sizeof(HandleDescriptor));
}
...
void ProcessImpl::GetGameThreads(std::vector<KThread *> &threads)
{
  ...
  for( HandleDescriptor &handle : handles ) {
    if( !(handle.obj->GetType() == KType::KThread) ) continue;

    KThread *thread = reinterpret_cast<KThread *>(handle.obj);

    if( !thread->IsPluginThread() ) threads.push_back(thread);
  }
}
...
static bool ThreadPredicate(KThread *thread)
{
  ...
  if( currentThread != thread && *tls != THREADVARS_MAGIC ) return true;
  return false;
}

void ProcessImpl::LockGameThreads(void)
{
  svcControlProcess(CUR_PROCESS_HANDLE, PROCESSOP_SCHEDULE_THREADS, 1,
                    (u32)ThreadPredicate);
}

void ProcessImpl::UnlockGameThreads(void)
{
  svcControlProcess(CUR_PROCESS_HANDLE, PROCESSOP_SCHEDULE_THREADS, 0,
                    (u32)ThreadPredicate);
}
...
void ProcessImpl::UpdateMemRegions(bool ignoreLock)
{
  ...
  for( u32 addr = 0x00100000; addr < 0x40000000; ) {
    ...
    if( R_SUCCEEDED(
      ...
      if( memInfo.perm & MEMPERM_READ ) MemRegions.push_back(memInfo);
...
static bool IsRegionProhibited(const MemInfo &memInfo)
{
  ...
  if( memInfo.state == MEMSTATE_SHARED ) return true;
...
bool ProcessImpl::IsValidAddress(const u32 address)
{
  ...
  for( MemInfo &memInfo : MemRegions )
    if( IsInRegion(memInfo, address) ) return true;
...
u32 ProcessImpl::GetPAFromVA(const u32 address)
{
  ...
  svcControlProcess(ProcessImpl::ProcessHandle, PROCESSOP_GET_PA_FROM_VA,
                    (u32)&pa, address);
...
MemInfo ProcessImpl::GetMemRegion(u32 address)
{
  ...
  for( MemInfo &memInfo : MemRegions )
    if( IsInRegion(memInfo, address) ) return memInfo;
...
MemInfo ProcessImpl::GetNextRegion(const MemInfo &region)
{
  ...
  for( MemInfo &memInfo : MemRegions )
    if( memInfo > region && !IsRegionProhibited(memInfo) ) return memInfo;
...
u32 ProcessImpl::GetFreeMemRegion(const u32 size, const u32 searchStart)
{
  ...
  for( auto block : freeBlocks ) {
    if( block.size <= 0x1000 || searchStart >= block.base_addr + block.size )
      continue;
...
{
  if( MainThreadTls ) *(u32 *)(MainThreadTls + 0x40) = 0;
}
...
void ProcessImpl::ExceptionHandler(ERRF_ExceptionInfo *excep,
                                   CpuRegisters *regs)
{
  // Default exception handler, if the user didn't set an custom exception
  // handler or an exception happened in the user callback
  if( AtomicPostIncrement(&exceptionCount) || !Process::exceptionCallback ) {
    DisableExceptionHandlers();
    ReturnFromException(Process::ThrowOldExceptionOnCallbackException
                          ? &g_previousException
                          : regs);
  }
  // Lock game threads
...
void ProcessImpl::SignalExit(void) { LightEvent_Signal(&waitForExitEvent); }

void ProcessImpl::WaitForExit(void) { LightEvent_Wait(&waitForExitEvent); }
}  // namespace CTRPluginFramework
