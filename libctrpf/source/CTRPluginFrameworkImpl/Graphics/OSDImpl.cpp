...
Task OSDImpl::DrawNotifTask1(OSDImpl::DrawNotif_TaskFunc, nullptr,
                             Task::AppCores);
Task OSDImpl::DrawNotifTask2(OSDImpl::DrawNotif_TaskFunc, nullptr,
                             Task::AppCores);
...
OSDImpl::OSDMessage::OSDMessage(const std::string &str, const Color &fg,
                                const Color &bg)
{
  text = str;
...
void OSDImpl::Update(void)
{
  if( Preferences::IsEnabled(Preferences::UseFloatingBtn) ) {
    ...
    if( FloatingBtn() ) PluginMenuImpl::ForceOpen();
  }
  ...
  if( Notifications.size() && Notifications.front()->drawn )
    Notifications.erase(
      std::remove_if(Notifications.begin(), Notifications.end(),
                     [](OSDMessage *message) {
                       bool remove = message->drawn &&
                                     message->time.HasTimePassed(Seconds(5.f));
                       if( remove ) delete message;
                       return remove;
                     }),
      Notifications.end());
...
bool OSDImpl::Draw(void)
{
  ...
  for( OSDMessage *message : Notifications ) {
    ...
    Renderer::DrawString(message->text.c_str(), posX, posY, message->foreground,
                         message->background);
    posY += 5;
    if( !message->drawn ) message->time.Restart();
...
u32 OSDImpl::MainCallback(u32 isBottom, int nextBank, void *leftFb,
                          void *rightFb, int stride, int format, int swap)
{
  ...
  if( ProcessImpl::Status == Running )
    return HookContext::GetCurrent().OriginalFunction<u32>(
      isBottom, nextBank, leftFb, rightFb, stride, format, swap);
...
u32 OSDImpl::MainCallback2(u32 r0, u32 *params, u32 isBottom, u32 arg)
{
  ...
  if( params ) {
    ...
    if( leftFb ) CallbackCommon(isBottom, leftFb, nullptr, stride, format, 0);
  }

  if( ProcessImpl::Status == Running )
    return HookContext::GetCurrent().OriginalFunction<u32>(r0, params, isBottom,
                                                           arg);
...
{
  if( FramesToPlay ) --FramesToPlay;

  // If frame has to be paused
  PauseFrame();

  if( DrawSaveIcon ) Icon::DrawSave(10, 10);
...
{
  // Floating button
  if( Preferences::IsEnabled(Preferences::UseFloatingBtn) ) FloatingBtn.Draw();
  ...
  if( Touch::IsDown() ) {
    ...
    if( Preferences::IsEnabled(Preferences::DrawTouchPosition) ) {
      std::string &&str =
        Utils::Format("Touch.x: %d  Touch.y: %d", touchPos.x, touchPos.y);
      int posY = 20;
...
void OSDImpl::CallbackCommon(u32 isBottom, void *addr, void *addrB, int stride,
                             int format, int swap)
{
  if( SystemImpl::Status() ) return;
  // TODO: fully remove this, rosalina implements it now
  // Preferences::ApplyBacklight();

  bool drawFps =
    (Preferences::IsEnabled(Preferences::ShowBottomFps) && isBottom) ||
    (Preferences::IsEnabled(Preferences::ShowTopFps) && !isBottom);
  if( drawFps ) g_fpsCounter[isBottom].Update();

  // Screen shot first
  if( Screenshot::OSDCallback(isBottom, addr, addrB, stride, format) ) return;
  ...
  if( !isBottom ) addrB = (void *)GetBuffer((u32)addrB);
  ...
  if( !isBottom ) {
    ...
    if( ProcessImpl::IsPaused || res ) return;
  }
  else {
  ...
  FwkSettings &settings = FwkSettings::Get();
  if( settings.CachedDrawMode ) {
    svcInvalidateProcessDataCache(CUR_PROCESS_HANDLE,
                                  reinterpret_cast<u32>(addr),
                                  isBottom ? stride * 320 : stride * 400);
    if( !isBottom && addrB && addrB != addr )
      svcInvalidateProcessDataCache(CUR_PROCESS_HANDLE,
                                    reinterpret_cast<u32>(addrB), stride * 400);
  }
    ...
    Callbacks.erase(std::remove_if(Callbacks.begin(), Callbacks.end(),
                                   [](OSDCallback cb) {
                                     ...
                                     auto foundIter = std::find(
                                       trashbin.begin(), trashbin.end(), cb);
  ...
  // Draw fps
  if( drawFps ) g_fpsCounter[isBottom].Display();

  if( settings.CachedDrawMode ) {
    svcFlushProcessDataCache(CUR_PROCESS_HANDLE, reinterpret_cast<u32>(addr),
                             isBottom ? stride * 320 : stride * 400);
    if( !isBottom && addrB && addrB != addr )
      svcFlushProcessDataCache(CUR_PROCESS_HANDLE, reinterpret_cast<u32>(addrB),
                               stride * 400);
  }
}
...
static u8 *memsearch(u8 *startPos, const void *pattern, u32 size,
                     u32 patternSize)
{
  ...
  // Preprocessing
  for( u32 i = 0; i < 256; i++ ) table[i] = patternSize;
...
static u32 SearchOSD(u32 pattern)
{
  ...
  if( pattern == 0 ) {
    address =
      memsearch((u8 *)0x100000, g_OSDPattern, Process::GetTextSize(), 0x14);

    if( address == nullptr ) {
      address = memsearch((u8 *)0x100000, &g_OSDPattern[5],
                          Process::GetTextSize(), 0x10);
    }
  }
  else if( pattern == 3 ) {
    address =
      memsearch((u8 *)0x100000, &g_OSDPattern[9], Process::GetTextSize(), 0x14);
  }
  else if( pattern == 4 ) {
    address = memsearch((u8 *)0x100000, &g_OSDPattern[14],
                        Process::GetTextSize(), 0x10);
  }
  else if( pattern == 5 ) {
    address = memsearch((u8 *)0x100000, &g_OSDPattern[18],
                        Process::GetTextSize(), 0x10);
  }
...
void InstallOSD(void)
{
  ...
  if( !found ) {
    ...
    if( !found ) {
      ...
      OSDImpl::OSDHook.InitializeForMitm(found2, u32(OSDImpl::MainCallback2))
        .Enable();
      return;
    }
  }

  OSDImpl::OSDHook.InitializeForMitm(found, u32(OSDImpl::MainCallback))
    .Enable();
}
