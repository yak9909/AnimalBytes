...
MessageBoxImpl::MessageBoxImpl(const std::string &title,
                               const std::string &message, DialogType dType,
                               ClearScreen clear)
  : _title(title),
    _message(message),
    _dialogType(dType),
    ...
    _cursor(0)
{
  ...
  _textbox._box = _box;
  _textbox._border = IntRect(_box.leftTop.x + 2, _box.leftTop.y + 2,
                             _box.size.x - 4, _box.size.y - 4);
  _box.size.y = height;
  _textbox.Update(_title, _message);
  _textbox.Open();
  _textbox._fastscroll = _textbox._drawBox = false;
}
MessageBoxImpl::MessageBoxImpl(const std::string &message,
                               const DialogType dType, ClearScreen clear)
  : _message(message),
    _dialogType(dType),
    _clearScreen(clear),
    _exit(false),
...
bool MessageBoxImpl::operator()(void)
{
  ...
  for( int i = 0; i < 2; ++i ) {
    ...
    if( (u32)_clearScreen & (u32)ClearScreen::Top ) Window::TopWindow.Draw();
  ...
  if( menu && !menu->IsOpen() && !sleepExit ) ScreenImpl::Top->Clear(true);

  if( mustReleaseGame ) ProcessImpl::Play(false);

  if( sleepExit ) return false;
  return _cursor == 0;
}
...
void MessageBoxImpl::_Draw(void)
{
  ...
  Renderer::DrawRect2(_box, settings.BackgroundMainColor,
                      settings.BackgroundSecondaryColor);

  // Draw Text
  int posY = _box.leftTop.y + _textbox._box.size.y;
  // int posX = _box.leftTop.x + 5;
  // int maxW = _box.leftTop.x + _box.size.x - 5; ///< Should be -10, but that
  // way a letter that is slighty too big can still be drawn int maxH =
  // _box.leftTop.y + _box.size.y - 30;

  /*if (!_title.empty())
  {
    int width = Renderer::DrawSysString(_title.c_str(), posX, posY, maxW,
  _titleColor); Renderer::DrawLine(posX, posY, width - posX + 30, _titleColor);
    posY += 8;
  }
  Renderer::DrawSysStringReturn((const u8 *)_message.c_str(), posX, posY, maxW,
  settings.MainTextColor, maxH);
  */
  _textbox.Draw();
  // Draw "Buttons"
  // posY += 13;
  ...
  if( _dialogType == DialogType::DialogOk ) {
    ...
    // Ok Button
    {
      ...
      // Background
      Renderer::DrawRect(posX, posY, 80, 20, Color::DimGrey);
      Renderer::DrawRect(posX, posY, 80, 20,
                         _cursor ? Color::Gray : Color::White, false);

      const char *content =
        _dialogType == DialogType::DialogOkCancel ? "Ok" : "Yes";
      ...
      posX += ((80 - width) / 2);
      posY += 2;
      Renderer::DrawSysString(content, posX, posY, 380,
                              _cursor ? Color::Silver : Color::White);
    }
    ...
    // Cancel Button
    {
      ...
      Renderer::DrawRect(posX, posY, 80, 20, Color::DimGrey);
      Renderer::DrawRect(posX, posY, 80, 20,
                         _cursor ? Color::White : Color::Gray, false);

      const char *content =
        _dialogType == DialogType::DialogOkCancel ? "Cancel" : "No";
      // Text
      float width = Renderer::GetTextSize(content);
      ...
      posY += 2;
      Renderer::DrawSysString(content, posX, posY, 380,
                              _cursor ? Color::White : Color::Silver);
    }
  }
}
}  // namespace CTRPluginFramework
