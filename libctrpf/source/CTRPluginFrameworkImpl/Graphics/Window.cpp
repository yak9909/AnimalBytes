...
Window::Window(u32 posX, u32 posY, u32 width, u32 height, bool closeBtn,
               BMPImage *image)
    ...
    _image(image)
{
  if( closeBtn ) {
    _closeBtn =
      new Button(Button::Icon, IntRect(posX + width - 25, posY + 4, 20, 20),
                 Icon::DrawClose);
  }
...
void Window::Draw(void) const
{
  ...
  if( _image != nullptr )
    ...
    Renderer::DrawRect2(_rect, Preferences::Settings.BackgroundMainColor,
                        Preferences::Settings.BackgroundSecondaryColor);
    Renderer::DrawRect(_border, Preferences::Settings.BackgroundBorderColor,
                       false);
  }

  // Close button
  if( _closeBtn != nullptr ) _closeBtn->Draw();
}
...
{
  if( _closeBtn != nullptr ) _closeBtn->Update(isTouched, touchPos);
}

bool Window::MustClose(void) const
{
  if( _closeBtn == nullptr ) return (false);
...
const IntRect &Window::GetRect(void) const { return (_rect); }
