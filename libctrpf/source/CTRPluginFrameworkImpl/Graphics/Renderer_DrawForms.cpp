...
static void DrawLineV2(u32 fmt, const Color &color1, const Color &color2,
                       u8 *dst, u32 height)
{
  union UColor {
    u16 u;
...
void Renderer::DrawLine(int posX, int posY, int width, const Color &color,
                        int height)
{
  if( posY < 0 ) {
...
void Renderer::DrawLine(const IntVector &start, const IntVector &end,
                        const Color &color)
{
  int posX = start.x;
  int posY = start.y;
...
void Renderer::DrawStippledLine(int posX, int posY, int width,
...
void Renderer::EllipseIncomplete(int posX, int posY, float a, float b, int max,
                                 int aff, Color color)
{
  int x;
  int y;
  ...
  d2 = (float)(b * b * (x + .5) * (x + .5) + a * a * (y - 1) * (y - 1) -
               a * a * b * b);
  while( (y > 0) && (cpt < max) ) {
    cpt++;
    if( d2 < 0 ) {
...
void Renderer::RoundedRectangle(const IntRect &rect, float radius, int max,
                                Color color, bool mustFill, Color fillColor)
{
  int x;
  int y;
  float d1;
  ...
  while( (radius * radius * (radius - .5) > radius * radius * (x + 1)) &&
         (cpt < max) ) {
    cpt++;
    if( d1 < 0 ) {
  ...
  d2 = (float)(radius * radius * (x + .5) * (x + .5) +
               radius * radius * (y - 1) * (y - 1) -
  ...
  // Left line
  DrawLine(posX - x + rWidth, posY + rHeight - 1, 1, color,
           height - (rHeight * 2));
  // Right line
...
#endif
void Renderer::ComputeRoundedRectangle(std::vector<IntLine> &out,
                                       ...
                                       int max)
{
  ...
  // Left line
  // DrawLine(posX - x + rWidth, posYBak + rHeight - 1, 1, color, height -
  // (rHeight* 2));
  // Right line
  // DrawLine(posX + x + width, posYBak + rHeight, 1, color, height - (rHeight *
  // 2));
  ...
  // Bottom Line
  out.push_back(
    IntLine(posX + rWidth - 1, posYBak + height, width - rWidth + 3, 1));
  // Left Line
  out.push_back(
    IntLine(posX - x + rWidth, posYBak + rHeight, 1, height - (rHeight * 2)));
  // Right Line
  out.push_back(
    IntLine(posX + x + width, posYBak + rHeight, 1, height - (rHeight * 2)));
...
void Renderer::DrawRoundedRectangle(const IntRect &rect, const Color &border,
                                    const Color &fill)
{
  static std::vector<IntLine> _lines;

  if( _lines.empty() )
    ComputeRoundedRectangle(_lines, IntRect(0, 0, rect.size.x, rect.size.y),
                            7.f, 50);
  ...
  for( i = 0; i < bMax; ++i ) {
    ...
    // Fill line
    Renderer::DrawLine(posX + 1 + line.start.x, posY + line.start.y,
                       line.end.x - line.start.x - 1, fill);
  }

  for( ; i < _lines.size() - 1; ++i ) {
    IntLine &line = _lines[i];

    Renderer::DrawLine(posX + line.start.x, posY + line.start.y, line.end.x,
                       border, line.end.y);
  }

  IntLine &line = _lines[i];
  Renderer::DrawLine(posX + line.start.x, posY + line.start.y, line.end.x, fill,
                     line.end.y);
}
void Renderer::DrawRect(int posX, int posY, int width, int height,
                        const Color &color, bool fill, int thickness)
{
  if( fill ) {
...
void Renderer::DrawRect2(const IntRect &rect, const Color &color1,
                         const Color &color2)
{
  int height = rect.size.y;
