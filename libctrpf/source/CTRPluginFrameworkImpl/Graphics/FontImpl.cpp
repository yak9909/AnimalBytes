...
float Glyph::Width(void) const { return (xOffset + xAdvance); }
...
static void PatchGameFontMapping(void)
{
  ...
  if( addr ) *reinterpret_cast<vu32 *>(addr + 16) = 0xE3A00000;  ///< mov r0, #0
}
  ...
  if( code > 0 ) {
    ...
    if( glyphIndex == 0xFFFF )  // Glyph not found, return "?" instead
    {
      if( defaultGlyph == nullptr )
        defaultGlyph =
          CacheGlyph(fontGlyphIndexFromCodePoint(nullptr, (u32)'?'));
            ...
            for( int xx = 0; xx < 2; xx++ ) {
              ...
              for( int yyy = 0; yyy < 2; yyy++ ) {
                for( int xxx = 0; xxx < 2; xxx++ ) {
                  // If the final y value is beyond the input data's height then
                  // don't read it
                  if( tileY + y + yy + yyy >= dataHeight ) continue;
                  // Same for the x and the input data width
                  if( tileX + x + xx + xxx >= dataWidth ) continue;
                  ...
                  int dataY =
                    ((yyy * 2) + (yy * 8) + (y * 32) + (tileY * width * 8));

                  int dataPos = dataX + dataY;

                  if( pixelY >= startPy && pixelY < endPy )
                    if( pixelX >= startPx && pixelX < endPx ) {
                      u32 offset = ((pixelX - startPx) +
                                    (pixelY - startPy) * (endPx - startPx));
                      if( offset < 1000 )
                        *(glyph + offset) =
                          GetAlphaValueFromData(data, dataPos, tglp->sheetFmt);
                    }
                }
              }
            }
...
{
  constexpr const int outWidth = std::round(
    static_cast<float>(GLYPH_WIDTH) * (static_cast<float>(SHRINK_GLYPH_HEIGHT) /
                                       static_cast<float>(GLYPH_HEIGHT)));
  constexpr const float dx =
    std::round(static_cast<float>(GLYPH_WIDTH) / outWidth);
  constexpr const float dy = std::round(
    static_cast<float>(GLYPH_HEIGHT) / static_cast<float>(SHRINK_GLYPH_HEIGHT));
  ...
  for( float yt = 0.f, y = 0.f; y < SHRINK_GLYPH_HEIGHT; y++, yt += dy ) {
    ...
    for( float xt = 0.f, x = 0.f; x < outWidth; x++, xt += dx ) {
      ...
      for( i = 0; yi + i + 1 < yt + dy - 1 && yi + i + 1 < GLYPH_HEIGHT; i++ ) {
        ...
        for( ii = 0; xi + ii + 1 < xt + dx - 1 && xi + ii + 1 < GLYPH_WIDTH;
             ii++ )
      ...
      if( yi + i + 1 < GLYPH_HEIGHT && xi + ii + 1 < GLYPH_WIDTH )
        alpha +=
          xfrag2 * yfrag2 * src[((yi + i + 1) * GLYPH_WIDTH + xi + ii + 1)];

      alpha /= dx * dy;
      alpha = alpha <= 0.f ? 0.f : (alpha >= 255.f ? 255.f : alpha);
      dest[(static_cast<u32>(y) * outWidth + static_cast<u32>(x))] =
...
Glyph *Font::CacheGlyph(u32 glyphIndex)
{
  ...
  // Allocate new Glyph
  Glyph *glyph = new Glyph;  // static_cast<Glyph
                             // *>(linearAlloc(sizeof(Glyph)));
  ...
  glyph->xAdvance =
    (glyphIndex == 0) ? glyphPos.xAdvance : std::round(glyphPos.xAdvance);
