...
GSPGPU_FramebufferFormat PrivColor::GetFormat() { return (_format); }
...
u8 *PrivColor::_WriteRGBA8Clamp(u8 *dst, const Color &color)
{
  ...
  if( !_clampArea.Contains(posX, posY) ) return (dst + 4);
...
u8 *PrivColor::_WriteBGR8Clamp(u8 *dst, const Color &color)
{
  ...
  if( !_clampArea.Contains(posX, posY) ) return (dst + 3);
...
u8 *PrivColor::_WriteRGB565Clamp(u8 *dst, const Color &color)
{
  ...
  if( !_clampArea.Contains(posX, posY) ) return (dst + 2);
