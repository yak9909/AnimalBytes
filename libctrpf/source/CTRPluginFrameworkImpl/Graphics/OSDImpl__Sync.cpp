...
void OSDImpl::Lock(void) { RecursiveLock_Lock(&RecLock); }

bool OSDImpl::TryLock(void) { return (RecursiveLock_TryLock(&RecLock)); }

void OSDImpl::Unlock(void) { RecursiveLock_Unlock(&RecLock); }
  ...
  for( ; res != toRes; ) {
    if( event.state == CLEARED_STICKY ) {
      res = syncArbitrateAddressWithTimeout(
        &event.state, ARBITRATION_WAIT_IF_LESS_THAN_TIMEOUT, 0, timeOutNs);
      return res == toRes;
    }

    if( event.state != CLEARED_ONESHOT ) {
      if( event.state == SIGNALED_STICKY ) return false;
...
int OSDImpl::PauseFrame(void)
{
  ...
  if( ScreenImpl::CheckGspFrameBuffersInfo() ) return 0;

  {
    CTRPFLock lock(FrameLockingMutex);

    if( IsForced || !NeedToPauseFrame ) return 0;
...
void OSDImpl::WaitFramePaused(void)
{
  ...
  if( !isAsync ) {
    ...
    while(
      (IsForced = LightEvent__WaitTimeOut(OnFramePaused, Milliseconds(50))) &&
