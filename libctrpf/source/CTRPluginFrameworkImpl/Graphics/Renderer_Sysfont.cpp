...
void Renderer::FontCalcGlyphPos(fontGlyphPos_s *out, charWidthInfo_s **cwout,
                                int glyphIndex, float scaleX, float scaleY)
{
  ...
  *cwout = cwi;
  ...
  float ty = 1.0f - (float)((lineId + 1) * (tglp->cellHeight + 1) + 1) /
                      tglp->sheetHeight;
...
float Renderer::GetTextSize(const char *text)
{
  ...
  // Skip UTF8 sig
  if( c[0] == 0xEF && c[1] == 0xBB && c[2] == 0xBF ) c += 3;
...
int Renderer::GetLineCount(const char *text, float maxWidth)
{
  ...
  while( *c ) {
    ...
    if( *c == 0x11 ) {
      ...
      if( control & 0x8000 ) {
        u16 skipToX = control & 0x1FF;
        if( skipToX > w && skipToX < maxWidth ) w = skipToX;
      }
      continue;
    }
...
void Renderer::GetTextInfos(const char *text, int &lineCount,
                            float &lineMaxWidth, float maxWidth)
{
  lineCount = 0;
  lineMaxWidth = 0.f;
...
void Renderer::DrawSysCheckBox(const char *str, int posX, int &posY,
                               int xLimits, Color color, bool isChecked,
                               float offset)
{
...
void Renderer::DrawSysFolder(const char *str, int posX, int &posY, int xLimits,
...
int Renderer::DrawGlyph(ScreenImpl *screen, Glyph *glyph, int posX, int posY,
                        Color color, u32 flags)
{
  if( !screen || !glyph ) return posY;
  ...
  for( int i = 0; i < 208; i++ ) {
    ...
    for( int j = 0;
         (alpha > 12) && j < ((flags & Render::FontDrawMode::BOLD) ? 2 : 1);
         j++ ) {
      color.a = alpha;
      Color &&l =
        PrivColor::FromFramebuffer(fb + stride * j + italicOffset * stride);
...
int Renderer::DrawGlyph(ScreenImpl *screen, Glyph *glyph, int posX, int posY,
                        float &offset, Color color, u32 flags)
{
  if( !screen || !glyph ) return posY;
...
int Renderer::DrawSysStringReturn(const unsigned char *stri, int posX,
                                  int &posY, int xLimits, Color color, int maxY,
                                  u32 flags)
{
  // Check for a valid pointer
  if( !(stri && *stri) ) return (posX);
  int lineCount = 1;
  u8 *str = const_cast<u8 *>(stri);
  int x = posX;
  int underLineStart = -1, strikeLineStart = -1;
  ScreenImpl *screen = GetContext()->screen;
  void (*lineDrawer)(int posX, int posY, int width, const Color &color,
                     int height) = Renderer::DrawLine;

  if( !screen ) return posY;

  xLimits = std::min(xLimits, (GetContext()->target == TOP ? 400 : 320));

  // Skip UTF8 sig
  if( str[0] == 0xEF && str[1] == 0xBB && str[2] == 0xBF ) str += 3;
    ...
    if( c == 0x11 ) {
      ...
      if( control & 0x8000 ) {
        ...
        if( skipToX > x && skipToX < xLimits ) {
          if( underLineStart == posX &&
              x == posX )  // Fix for aligned text after newline
            underLineStart = skipToX;
          if( strikeLineStart == posX &&
              x == posX )  // Fix for aligned text after newline
        ...
        if( (prevFlags & Render::FontDrawMode::UNDERLINE) !=
            (flags & Render::FontDrawMode::UNDERLINE) ) {
          bool startLine = flags & Render::FontDrawMode::UNDERLINE;
          if( startLine )
            underLineStart = x;
          else {
            if( underLineStart != x )
              lineDrawer(underLineStart, posY + 15, x - underLineStart, color,
                         1);
            underLineStart = -1;
          }
        }
        if( (prevFlags & Render::FontDrawMode::STRIKETHROUGH) !=
            (flags & Render::FontDrawMode::STRIKETHROUGH) ) {
          bool startLine = flags & Render::FontDrawMode::STRIKETHROUGH;
          if( startLine )
            strikeLineStart = x;
          else {
            if( strikeLineStart != x )
              lineDrawer(strikeLineStart, posY + 9, x - strikeLineStart, color,
                         1);
            strikeLineStart = -1;
          }
        }
...
int Renderer::DrawSysString(const char *stri, int posX, int &posY, int xLimits,
                            Color color, float offset, const char *end,
                            u32 flags)
{
  Glyph *glyph;
  int x = posX;
  ...
  do {
    ...
    if( c == 0x11 ) {
      ...
      if( control & 0x8000 ) {
        u16 skipToX = control & 0x1FF;
        if( skipToX > x && skipToX < xLimits ) x = skipToX;
      }
      else if( control & 0x4000 ) {
        u32 prevFlags = flags;
        flags ^= (control & 0x7F);
