...
Button::Button(u32 type, const std::string &content, const IntRect &bounds,
               IconCallback icon)
  : _icon{(void *)icon},
    _content{content},
    _bounds{bounds}
...
{
  if( !_isEnabled || _isLocked ) return false;

  if( !_isToggleBtn ) _state = false;
...
void Button::Draw(void)
{
  ...
  if( _isIconBtn && _icon != nullptr ) {
    reinterpret_cast<IconCallbackState>(_icon)(
      _posX, _posY, _isToggleBtn ? _state : _isPressed);
    return;
  }
...
void Button::Update(const bool isTouchDown, const IntVector &touchPos)
{
  ...
  if( _isToggleBtn && !_clock.HasTimePassed(halfSecond) ) return;

  _isPressed = _bounds.Contains(touchPos) && isTouchDown;
}

void Button::Enable(bool isEnabled) { _isEnabled = isEnabled; }

void Button::Enable(void) { _isEnabled = true; }

void Button::Disable(void) { _isEnabled = false; }

void Button::Lock(void) { _isLocked = true; }

void Button::Unlock(void) { _isLocked = false; }

void Button::Execute(void) { _execute = true; }

void Button::SetState(bool state) { _state = state; }

bool Button::GetState(void) const { return _state; }
}  // namespace CTRPluginFramework
