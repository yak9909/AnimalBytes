...
** Constructor
***************/
TextBox::TextBox(const std::string &title, const std::string &text,
                 const IntRect &box)
  : _title(title),
    _text(&text),
    _box(box),
    _border(IntRect(box.leftTop.x + 2, box.leftTop.y + 2, box.size.x - 4,
                    box.size.y - 4))
{
  _currentLine = 0;
  _isOpen = false;
...
** Open
************/
void TextBox::Open(void) const { _isOpen = true; }

/*
** Close
***********/
void TextBox::Close(void) const { _isOpen = false; }

/*
** IsOpen
***********/
bool TextBox::IsOpen(void) const { return (_isOpen); }
...
{
  if( _currentLine >= _newline.size() ) _currentLine = 0;

  int max =
    std::min((int)(_currentLine + _maxLines), (int)(_newline.size() - 1));
  ...
  if( !_title.empty() ) {
    int width;
    width = Renderer::DrawSysString((const char *)_title.c_str(), posX, posY,
                                    xLimit, settings.WindowTitleColor);
    int length = width - posX + 30;
    if( posX + length > xLimit ) length = xLimit - posX;
  ...
  for( int i = _currentLine; i < max; i++ ) {
    Renderer::DrawSysString((char *)_newline[i], posX, posY, xLimit,
                            RendererPriv::g_customColor, 0,
                            (char *)_newline[i + 1]);
  }
