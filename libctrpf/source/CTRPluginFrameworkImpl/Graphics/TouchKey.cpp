...
void TouchKey::Enable(bool isEnabled) { _enabled = isEnabled; }
...
void TouchKey::Draw(void)
{
  ...
  const Color &background =
    _isPressed ? theme.KeyBackgroundPressed : theme.KeyBackground;
...
int TouchKey::operator()(std::string &str)
{
  ...
  if( _enabled && _character == 0x8 ) {
    if( _isPressed ) return (_character);
    if( _execute ) {
      _execute = false;
      return (~_character);
    }
