...
TouchKeyString::TouchKeyString(const std::string &content, IntRect ui,
                               bool isEnabled)
  : TouchKeyString(ui, isEnabled)
{
  ...
  while( (int)(_contentLength) > ui.size.x ) {
    if( _content.size() < 1 ) break;
    _content.pop_back();
    _contentLength = Renderer::GetTextSize(_content.c_str());
  }

  _posX = ((_uiProperties.size.x - (int)_contentLength) >> 1) +
          _uiProperties.leftTop.x;
}

TouchKeyString::TouchKeyString(const CustomIcon &icon, IntRect ui,
                               bool isEnabled)
  : TouchKeyString(ui, isEnabled)
{
  _icon = icon;
...
void TouchKeyString::Enable(bool isEnabled) { _enabled = isEnabled; }

void TouchKeyString::ForcePressed(bool force) { _isForcePressed = force; }

bool TouchKeyString::CanUse(void)
{
  return !(!_enabled || (_isIcon && !_icon.isEnabled) ||
           (!_isIcon && _content.empty()));
}
...
void TouchKeyString::Draw(void)
{
  ...
  const Color &background = (_isPressed || _isForcePressed)
                              ? theme.KeyBackgroundPressed
                              : theme.KeyBackground;
  const Color &text =
    (_isPressed || _isForcePressed) ? theme.KeyTextPressed : theme.KeyText;
