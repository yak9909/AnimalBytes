...
{
  return file.Read(this, sizeof(BitmapInformationHeader)) !=
         File::OPResult::SUCCESS;
}

const u32 BMPImage::HeaderSize = sizeof(BMPImage::BitmapFileHeader) +
                                 sizeof(BMPImage::BitmapInformationHeader);
    ...
    _height{static_cast<u16>(height)}
{
  ...
  // Create buffer
  _data =
    static_cast<u8 *>(::operator new(_dataSize + HeaderSize, std::nothrow));
...
bool BMPImage::IsLoaded(void) const { return _loaded; }

u32 BMPImage::Width(void) const { return _width; }

u32 BMPImage::Height(void) const { return _height; }

u32 BMPImage::BytesPerPixel(void) const { return _bytesPerPixel; }

const IntVector &BMPImage::GetDimensions(void) const { return _dimensions; }
...
{
  if( _data ) ::operator delete(_data, std::nothrow);
  _data = nullptr;
  _dataSize = 0;
  _loaded = false;
}
...
void BMPImage::Draw(int x, int y)
{
  ...
  int posX =
    scr->IsTopScreen() ? x + (340 - _width) / 2 : x + (280 - _width) / 2;
...
void BMPImage::Draw(const IntVector &point) { Draw(point.x, point.y); }
...
void BMPImage::SaveImage(const std::string &fileName) const
{
  ...
  BitmapInformationHeader *bih =
    (BitmapInformationHeader *)(_data + sizeof(BitmapFileHeader));
  ...
  bih->size_image =
    (((bih->width * _bytesPerPixel) + 3) & 0x0000FFFC) * bih->height;

  bfh->type = 19778;
  bfh->size = sizeof(BitmapFileHeader) + sizeof(BitmapInformationHeader) +
              bih->size_image;
  bfh->reserved1 = 0;
  bfh->reserved2 = 0;
...
void BMPImage::OptimiseForRendering(void)
{
  ...
  if( _channelMode == RGB565_Mode ) return;

  // Ensure we're dealing with BGR8 color format
  RGBtoBGR();

  Pixel *img = reinterpret_cast<Pixel *>(data());
  u16 *rgb565Image = static_cast<u16 *>(
    ::operator new(_width *_height * 2 + HeaderSize, std::nothrow));

  u16 *dst =
    reinterpret_cast<u16 *>(reinterpret_cast<u32>(rgb565Image) + HeaderSize);
...
void BMPImage::FillWithImg(const BMPImage &src)
{
  ...
  if( srcWidth < _width ) offsetX = (_width - srcWidth) / 2 * 3;
  if( src._height < _height ) startY = (_height - srcHeight) / 2;
...
bool BMPImage::Resample(BMPImage &dest, int newWidth, int newHeight)
{
  ...
  if( !dest._loaded ) return (false);

  unsigned char *newData =
    dest.data();  // new unsigned char [newWidth * newHeight * 3];
  ...
  for( int cy = 0; cy < newHeight; cy++ ) {
    for( int cx = 0; cx < newWidth; cx++ ) {
      ...
      int nearestMatch = (((int)(cy / scaleHeight) * (_width * 3)) +
                          ((int)(cx / scaleWidth) * 3));
  ...
  */
  ...
  for( unsigned int j = 0; j < vertical_upper; ++j ) {
    for( unsigned int i = 0; i < horizontal_upper; ++i ) {
      for( unsigned int k = 0; k < _bytesPerPixel;
           s_itr[k] += _bytesPerPixel, ++k ) {
        total = 0;
        total += *(itr1[k]);
        total += *(itr1[k]);
        total += *(itr2[k]);
...
bool BMPImage::Region(const u32 &x, const u32 &y, const u32 &width,
                      const u32 &height, BMPImage &destImage)
{
  if( (x + width) > _width ) return (false);
  if( (y + height) > _height ) return (false);
...
bool BMPImage::RoiFromCenter(const u32 cx, const u32 cy, const u32 &width,
                             const u32 &height, BMPImage &destImage)
{
  return (
    Region(cx - (width / 2), cy - (height / 2), width, height, destImage));
}
...
void BMPImage::_LoadBitmap(void)
{
  ...
  if( bfh.type != 19778 ) {
    OSD::Notify(Utils::Format("BMP Error: Unexpected type value: %d", bfh.type),
                Color::Red);
    return;
  }
  ...
  if( bih.bit_count != 24 ) {
    OSD::Notify(
      Utils::Format("BMP Error: Unexpected bit depth: %d", bih.bit_count),
      Color::Red);
    return;
  }
  ...
  if( _width > 340 || _height > 200 ) {
    OSD::Notify("BMP Error: file should not be higher than 340px * 200px",
                Color::Red);
    return;
  }
  ...
  if( _CreateBitmap() ) {
    OSD::Notify("BMP Error: Error while allocating required space.",
                Color::Red);
    return;
  }
...
void BMPImage::_LoadBitmapFromMemory(void *data)
{
  ...
  std::copy(
    (u8 *)data + sizeof(BitmapFileHeader),
    (u8 *)data + sizeof(BitmapFileHeader) + sizeof(BitmapInformationHeader),
    (u8 *)&bih);
