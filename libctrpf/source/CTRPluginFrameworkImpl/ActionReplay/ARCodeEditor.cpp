...
#define IsData                               \
  (flags &                                   \
   (u32)(ARCodeEditor::CodeLine::PatchData | \
         ARCodeEditor::CodeLine::PatternData | ARCodeEditor::CodeLine::Asm))
...
static std::string KeysToString(u32 keys)
{
  static const char *keysText[] = {
    "A",        "B",  "Select", "Start", ARROW_RIGHT, ARROW_LEFT, ARROW_UP,
    ARROW_DOWN, "R",  "L",      "X",     "Y",         "",         "",
    "ZL",       "ZR", "",       "",      "",          "",         "Touch",
    "",         "",   "",       "SR",    "SL",        "SU",       "SD",
    "CR",       "CL", "CU",     "CD"};
...
static std::string GetCond16Str(u32 left, u32 right, const char *ope)
{
  ...
  if( g_condAgainstData == CondMode::ImmAgainstVal ) {
    if( mask )
      return Utils::Format("if %04X%s[%07X+off] & %04X:", value, ope, left,
                           mask);
    else
...
static std::string CommentCodeLine(const ARCode &code)
{
  ...
  switch( code.Type ) {
    ...
    case 0x10:  ///< Write16
      ret =
        Utils::Format("[%08X + offs] = %04X", code.Left, (code.Right & 0xFFFF));
      break;
    case 0x20:  ///< Write8
      ret =
        Utils::Format("[%08X + offs] = %02X", code.Left, (code.Right & 0xFF));
      break;
    ...
    case 0xDF:  ///< Registers:
    {
      ...
      if( code.Left == 0xFFFFFE ) {
        ...
        if( conversion ) ret += ", cvt";
        break;
      }
      // Condition toggle
      if( code.Left == 0xFFFFFF ) {
        static const char *condstr[5] = {
          "cond: immediate against value", "cond. data against value",
          "cond. immediate against data", "cond. immediate against storage",
      ...
      if( to && from ) ret = Utils::Format("%s = %s", to, from);
      break;
    }
    case 0xF0:
...
ARCodeEditor::CodeLine::CodeLine(ARCode &code)
  : base(code),
    ...
    index(0)
{
  if( code.Text.empty() ) code.Text = code.ToString();
  BSet(Modified);
}
...
void ARCodeEditor::CodeLine::Edit(u32 cursor, u32 value)
{
  if( IsData ) {
    ...
    if( ctDiffer && (nextCodeType == "E" || nextCodeType == "FE" ||
      ...
      if( nextCodeType.size() > 1 ) base.Text[1] = nextCodeType[1];
    }
    // If the code type changes from E or FE to something else, ask before
    // pursue
    else if( ctDiffer && !base.Data.empty() &&
             (currentCodeType == "E" || currentCodeType == "FE" ||
              currentCodeType == "FD") ) {
    _changeCodeType:
      if( !MessageBox(
    ...
    else if( (cursor >= 8 && (base.Type == 0xE0 || base.Type == 0xFD)) ||
             (IsInRange(cursor, 2, 7) && base.Type == 0xFE) ) {
    _changeDataSize:
      ...
      if( diff > 0 ) {
        std::string body = Utils::Format(
          "You're about to delete %d line(s) of data, continue ?", diff);
        if( !MessageBox(Color::Orange << "Warning", body,
                        DialogType::DialogYesNo)() )
          return;
      }
...
void ARCodeEditor::CodeLine::Update(void)
{
  ...
  if( IsData && IsModified ) {
    ...
    display =
      ColorToString(PATCH_COLOR) + Utils::Format("%08X %08X", left, right);
    if( IsPattern )
...
#define IsCondModeToggle(code) (code.Type == 0xDF && code.Left == 0x00FFFFFF)
  ...
  if( IsModified ) {
    ...
    if( isCondModeToggle || IsCondModeToggle(base) ) g_newCondDataToggle = true;
...
static void ShowHelp(void)
{
  std::string body =
    "Controls:\n"
    "    - " FONT_B
    ": Exit editor (changes are applied directly)\n"
    "    - " FONT_Y
    ": Delete current code\n"
    "    - " FONT_L
    ": Insert a new code before current code\n"
...
ARCodeEditor::ARCodeEditor(void)
  : _submenu{{"Copy to clipboard", "Clear clipboard", "Delete all codes",
              "Converter", "Hex Editor", "Help"}}
{
  _exit = false;
  _index = _line = 0;
...
bool ARCodeEditor::operator()(EventList &eventList)
{
  ...
  for( Event &event : eventList ) {
    ...
    if( !isSubMenuOpen ) _ProcessEvent(event);
  }
  ...
  if( _submenu.IsOpen() ) {
    switch( _submenu() ) {
      ...
      {
        if( _codes.empty() || !_context ) break;
      ...
      {
        if( !(MessageBox(Color::Orange << "Warning",
                         "Do you really want to delete all codes ?",
                         DialogType::DialogYesNo)()) )
          break;
      ...
      case 4:  ///< Hex Editor
        if( static_cast<size_t>(_line) < _codes.size() &&
            _codes[_line].base.Type == 0xD3 ) {
          u32 address = _codes[_line].base.Right;
          if( address && __g_hexEditor ) __g_hexEditor->Goto(address, true);
        }
        isInHexEditor = true;
        break;
      case 5:  ///< Show help
...
void ARCodeEditor::Edit(ARCodeContext &ctx)
{
  ...
  do {
    eventList.clear();
    // Fetch all events
    while( manager.PollEvent(event) ) eventList.push_back(event);
...
void ARCodeEditor::_ProcessEvent(Event &event)
{
  ...
  if( event.type == Event::KeyDown &&
      _inputClock.HasTimePassed(Milliseconds(150)) ) {
    switch( event.key.code ) {
      case Key::DPadUp:
...
void ARCodeEditor::_RenderTop(void)
{
  ...
  int posYComment = posY;
  {
    ...
    for( ; i < max; ++i ) {
      // Draw line
      Renderer::DrawString(Utils::Format("%3d", i + 1).c_str(), posXline,
                           posYline, Color::Black);
      // Draw code
      Renderer::DrawString(_codes[i].display.c_str(), posX, posY, Color::Black);
      // Draw comment
      Renderer::DrawString(_codes[i].comment.c_str(), posXComment, posYComment,
                           Color::DimGrey);
    }
  }
...
void ARCodeEditor::_Update(void)
{
  ...
  for( CodeLine &code : _codes ) code.Update();
...
void ARCodeEditor::_ReloadCodeLines(void)
{
  ...
  for( ARCode &code : arcodes ) {
    ...
    if( code.IsCodeWithData() && !code.Data.empty() ) {
      u32 flags = code.Type == 0xFE
                    ? CodeLine::PatternData
                    : (code.Type == 0xF0 && code.Left == 0x00F00000
                         ? CodeLine::Asm
                         : CodeLine::PatchData);
      ...
      for( size_t i = 0; i < code.Data.size() - 1; i += 2 ) {
        bool error;
        std::string data =
          Utils::Format("%08X %08X", code.Data[i], code.Data[i + 1]);
  ...
  while( static_cast<size_t>(_line) >= _codes.size() && _line > 0 ) _line--;
  if( _line < 0 ) _line = 0;
}
}  // namespace CTRPluginFramework
