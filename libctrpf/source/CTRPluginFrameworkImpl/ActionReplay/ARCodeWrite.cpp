...
{
  if( !item || item->name.empty() ) return false;
...
{
  if( !item || item->note.empty() ) return false;
  writer << "{" << RemoveFormat(item->note) << "}" << endl;
  return true;
}

static bool ActionReplay_WriteCode(LineWriter &writer,
                                   MenuEntryActionReplay *entry)
{
  if( !entry || entry->context.codes.empty() ) return false;
  ...
  for( ARCode &code : entry->context.codes ) {
    ...
    if( !code.Data.empty() ) {
      for( u32 i = 0; i < code.Data.size(); i += 2 ) {
        writer << Utils::Format("%08X %08X", code.Data[i], code.Data[i + 1])
               << endl;
      }
    }
  }
...
static bool ActionReplay_WriteFolderToFile(LineWriter &writer,
                                           MenuFolderImpl &folder)
{
