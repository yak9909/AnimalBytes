...
#define TRACE                                            \
  {                                                      \
    OSDManager["trace"].SetScreen(true).SetPos(10, 50) = \
      std::string(__FUNCTION__) << ":" << __LINE__;      \
    Sleep(Seconds(0.04f));                               \
  }
#define XTRACE(str, ...)                                           \
  {                                                                \
    OSDManager["trace1"].SetScreen(true).SetPos(0, 30) =           \
      std::string(__FUNCTION__)                                    \
      << ":" << __LINE__ << Utils::Format(" " str, ##__VA_ARGS__); \
    Sleep(Seconds(0.10f));                                         \
  }
#define XTRACE2(str, ...)                                          \
  {                                                                \
    OSDManager["trace2"].SetScreen(true).SetPos(0, 40) =           \
      std::string(__FUNCTION__)                                    \
      << ":" << __LINE__ << Utils::Format(" " str, ##__VA_ARGS__); \
    Sleep(Seconds(0.10f));                                         \
  }
#define XTRACE3(str, ...)                                          \
  {                                                                \
    OSDManager["trace3"].SetScreen(true).SetPos(0, 50) =           \
      std::string(__FUNCTION__)                                    \
      << ":" << __LINE__ << Utils::Format(" " str, ##__VA_ARGS__); \
    Sleep(Seconds(0.10f));                                         \
  }
#define XTRACE4(str, ...)                                          \
  {                                                                \
    OSDManager["trace4"].SetScreen(true).SetPos(0, 60) =           \
      std::string(__FUNCTION__)                                    \
      << ":" << __LINE__ << Utils::Format(" " str, ##__VA_ARGS__); \
    Sleep(Seconds(0.10f));                                         \
  }
...
void ActionReplay_OpenCheatsFile(File &output, bool create)
{
  ...
  if( create ) flags |= File::TRUNCATE | File::CREATE;
...
bool ActionReplay_IsHexCode(const std::string &line)
{
  ...
  for( const char c : line ) {
    if( index != 8 && !std::isxdigit(c) ) return (false);
    ++index;
    if( index >= 16 ) break;
  }
...
bool ActionReplay_IsValidCode(const std::string &line)
{
  ...
  if( !ActionReplay_IsHexCode(line) ) return false;
  ...
  return !(error ||
           !ActionReplay_CheckCodeTypeValidity(left));  ///< The code is valid
}
...
void ActionReplay_ProcessString(std::string &str, bool canNewLine)
{
  ...
  for( u32 i = 0; i < strSize - 2; i++ ) {
    ...
    // Check if we found a color pattern
      ...
      const int leftSize =
        strSize - i -
        1;  ///< size left of the current string from the '~' + 1 character
      ...
      static const std::vector<PColor> colors = {
        ...
        PColor("red", Color::Red),         PColor("green", Color::Green),
        PColor("blue", Color::Blue),       PColor("cyan", Color::Cyan),
        PColor("magenta", Color::Magenta), PColor("yellow", Color::Yellow),
        PColor("gray", Color::Gray)};

      for( const PColor &color : colors ) {
        u32 colorsize = std::strlen(color.name);
        if( !std::strncmp(cstr, color.name, colorsize) &&
            *(cstr + colorsize) == '~' ) {
...
static bool ActionReplay_GetData(std::string &line, ARCodeContext *codectx,
                                 int &index)
{
  if( !codectx ) return (false);
...
void ActionReplay_LoadCodes(MenuFolderImpl *dst)
{
  ...
  while( reader(line) ) {
    ...
    // If line is empty
    if( line.empty() ) {
      if( entry ) folders.top()->Append(entry->Update());
      entry = nullptr;
      name.clear();
      error = ecode = false;
      continue;
    ...
    if( error ) XTRACE4("Error: %s", line.c_str());
