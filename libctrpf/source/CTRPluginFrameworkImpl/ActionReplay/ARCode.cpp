...
const std::vector<u8> g_codeTypes = {
  ...
  0xD3, 0xDC, 0xC0, 0xD2, 0xD1, 0xD0, 0xD4, 0xD5, 0xD6, 0xD7, 0xD8, 0xD9,
  0xDA, 0xDB, 0xE0, 0xDD, 0xDE, 0xDF, 0xF0, 0xF1, 0xF2, 0xF3, 0xF4, 0xF5,
...
{
  if( Type == 0xE0 || Type == 0xFE || Type == 0xFD ) return HasError;

  if( HasError && !Text.empty() ) Update(Text);

  if( !HasError && !Text.empty() ) Text.clear();
...
bool ARCodeContext::Update(void)
{
  ...
  for( ARCode &code : codes ) error |= code.Update();
  hasError = error;
  return hasError;
}
...
void ARCodeContext::Clear(void)
{
  ...
  if( hooks != nullptr ) {
    for( Hook &hook : *hooks ) hook.Disable();
    hooks->clear();
  }
}
}  // namespace CTRPluginFramework
