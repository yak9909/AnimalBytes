...
extern "C" void AR__ExecuteRoutine(u32 **args, const u32 *routine);
...
{
  if( CheckAddress(address, true) ) *(vu32 *)address = value;
  return (true);
}

bool Write16(u32 address, u16 value)
{
  if( CheckAddress(address, true) ) *(vu16 *)address = value;
  return (true);
}

bool Write8(u32 address, u8 value)
{
  if( CheckAddress(address, true) ) *(vu8 *)address = value;
  return (true);
}

bool Read32(u32 address, u32 &value)
{
  if( CheckAddress(address, false) ) value = *(vu32 *)address;
  return (true);
}

bool Read16(u32 address, u16 &value)
{
  if( CheckAddress(address, false) ) value = *(vu16 *)address;
  return (true);
}

bool Read8(u32 address, u8 &value)
{
  if( CheckAddress(address, false) ) value = *(vu8 *)address;
  return (true);
}
...
static bool Memcpy(u32 address, u8 *pattern, u32 size)
{
  if( CheckAddress((u32)pattern, false) && CheckAddress(address, true) )
    while( size-- ) *(vu8 *)address++ = *pattern++;
...
#define Cond16(operator)                                                      \
  if( conditionalMode == CondMode::ImmAgainstData ) {                         \
    ...
    if( (code.Right & 0xFFFF) operator((Storage[ActiveStorage] & 0xFFFF) &    \
                                       mask) )                                \
      continue;                                                               \
  }                                                                           \
  else if( conditionalMode == CondMode::DataAgainstVal ) {                    \
    if( (currentData.value & 0xFFFF) operator value16 ) continue;             \
  }                                                                           \
  else if( conditionalMode == CondMode::DataAgainstStorage ) {                \
    if( (currentData.value & 0xFFFF) operator(Storage[ActiveStorage] &        \
                                              0xFFFF) )                       \
      continue;                                                               \
  }                                                                           \
...
bool ARHandler::_Execute(const ARCodeVector &codes)
{
  ...
  for( const ARCode &code : codes ) {
    ...
    // If we must exit
    if( ExitCodeImmediately ) return true;  ///< Consider as error
    ...
    if( waitForExitCode ) {
      ...
      {
        --conditionCount;
        if( conditionCount == 0 ) waitForExitCode = false;
      }

      // Check for embricked conditions
      if( code.Type >= 0x30 && code.Type <= 0xA0 ) ++conditionCount;
    ...
    switch( code.Type ) {
      ...
      {
        ExitCodeImmediately =
          !Write32(code.Left + Offset[ActiveOffset], code.Right);
        break;
      }
      case 0x10:  ///< Write16
      {
        ExitCodeImmediately =
          !Write16(code.Left + Offset[ActiveOffset], code.Right);
        break;
      }
      case 0x20:  ///< Write8
      {
        ExitCodeImmediately =
          !Write8(code.Left + Offset[ActiveOffset], code.Right);
        break;
      }
      case 0x30:  ///< GreaterThan 32Bits
      {
        if( conditionalMode == CondMode::ImmAgainstData ||
            !((ExitCodeImmediately =
                 !Read32(code.Left + Offset[ActiveOffset], value))) ) {
      ...
      case 0x70:  ///< GreaterThan 16Bits
      {
        ...
        if( conditionalMode == CondMode::ImmAgainstData ||
            !((ExitCodeImmediately =
                 !Read16(code.Left + Offset[ActiveOffset], value16))) ) {
          value16 &= mask;
          Cond16(>);
        }
        conditionCount++;
      ...
      {
        ExitCodeImmediately =
          !Read32(code.Left + Offset[ActiveOffset], Offset[ActiveOffset]);
        break;
      }
      ...
      {
        u32 v =
          code.Left == 0 ? currentData.value : Data[(code.Left - 1) & 1].value;
      ...
      case 0xD7:  ///< Write 16Bits Data[ActiveData]
      {
        value16 =
          code.Left == 0 ? currentData.value : Data[(code.Left - 1) & 1].value;
        ExitCodeImmediately =
          !Write16(code.Right + Offset[ActiveOffset], value16);
      ...
      case 0xD8:  ///< Write 8Bits Data[ActiveData]
      {
        u8 value8 =
          code.Left == 0 ? currentData.value : Data[(code.Left - 1) & 1].value;
        ExitCodeImmediately =
          !Write8(code.Right + Offset[ActiveOffset], value8);
        Offset[ActiveOffset]++;
        break;
      }
      ...
      {
        Register &data =
          code.Left == 0 ? currentData : Data[(code.Left - 1) & 1];
        ExitCodeImmediately =
          !Read32(code.Right + Offset[ActiveOffset], data.value);
        break;
      }
      case 0xDA:  ///< Read 16Bits to Data[ActiveData]
      {
        ...
        ExitCodeImmediately =
          !Read16(code.Right + Offset[ActiveOffset], value16);
        data.value = value16;
        break;
      }
...
#define IsInRange(val, min, max) (min <= val && val <= max)
        ...
        bool isInRange = code.Left == 0 ? IsInRange(touchPos.x, min, max)
                                        : IsInRange(touchPos.y, min, max);
      ...
      case 0xE0:  ///< Patch code
      {
        ...
        if( data == nullptr || code.Data.size() == 0 || !code.Right ||
            code.Right > code.Data.size() * 4 )
          continue;  ///< An error occured

        ExitCodeImmediately =
          !Memcpy(code.Left + Offset[ActiveOffset], data, code.Right);
        break;
      }
      case 0xF0:  ///< Custom things
      {
        switch( code.Left ) {
          ...
          {
            u32 *args[] = {&Offset[0],        &Offset[1],  &Data[0].value,
                           &Data[1].value,    &Storage[0], &Storage[1],
                           (u32 *)0x01E81000, arStack};
      ...
      case 0xF1:  ///< ADD code
      {
        if( !ToggleFloat ) {
          if( Read32(code.Left + Offset[ActiveOffset], value) )
            ExitCodeImmediately =
              !Write32(code.Left + Offset[ActiveOffset], value + code.Right);
        }
        else {
          value = code.Right;
          float val = vfpval;
          if( Read32(code.Left + Offset[ActiveOffset], value) ) {
            vfpval += val;
            ExitCodeImmediately =
              !Write32(code.Left + Offset[ActiveOffset], value);
          }
        }
        break;
      }
      ...
      case 0xFC:  ///< Memcpy code
      {
        if( Offset[0] && Offset[1] ) {
          ExitCodeImmediately = !Process::CopyMemory(
            (void *)Offset[0], (void *)Offset[1], code.Right);
        }
        break;
      }
      case 0xFD:  ///< Hook
      {
        ...
        if( hook.Enable() == HookResult::Success ) hooks->push_back(hook);
      ...
      case 0xFE:  ///< Memsearch
      {
        ...
        if( !pattern.empty() && code.Right &&
            Process::CheckRegion(start, regionSize) ) {
          u32 address = 0;

          if( R_SUCCEEDED(svcQueryMemory(&mInfo, &pInfo, start)) ) {
            u32 size =
              std::min((u32)code.Right, mInfo.size - (start - mInfo.base_addr));
