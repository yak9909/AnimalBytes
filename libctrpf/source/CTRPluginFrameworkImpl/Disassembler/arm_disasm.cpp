...
static const char *cond_names[] = {"eq", "ne", "cs", "cc",      "mi", "pl",
                                   "vs", "vc", "hi", "ls",      "ge", "lt",
                                   "gt", "le", "",   "RESERVED"};

const char *opcode_names[] = {"invalid",
                              "undefined",
                              "adc",
                              "add",
                              "and",
...
static const char *cond_to_str(int cond) { return cond_names[cond]; }
...
static std::string GetRegList(u32 reglist)
{
  ...
  for( int i = 13; i < 16; ++i )
    if( reglist & (1u << i) ) blocks.emplace_back(i, i);

  const char *comma = "";

  for( RegBlock &block : blocks ) {
    if( block.begin != block.end )
      out += Utils::Format("%s%s-%s", comma, RegNames[block.begin],
                           RegNames[block.end]);
    else
...
std::string ARM_Disasm::DisassembleALU(Opcode opcode, uint32_t insn)
{
  ...
  const char *sbit_str = "";
  if( bit_s && !(flags & kNoSbit) ) sbit_str = "s";

  if( is_immed ) {
    return Utils::Format(
      "%-8s%s%s#%u  ; 0x%X",
      std::string(opname).append(cond_to_str(cond)).append(sbit_str).c_str(),
      rd_str.c_str(), rn_str.c_str(), immed, immed);
  }
  ...
  if( !shift_is_reg && shift_type == 0 && shift_amount == 0 ) {
    return Utils::Format(
      "%-8s%s",
      std::string(opname).append(cond_to_str(cond)).append(sbit_str).c_str(),
      rd_str.append(rn_str).append(RegNames[rm]).c_str());
  }
  ...
  if( shift_is_reg ) {
    return Utils::Format(
      "%-8s%s%s%s, %s %s",
      std::string(opname).append(cond_to_str(cond)).append(sbit_str).c_str(),
      rd_str.c_str(), rn_str.c_str(), RegNames[rm], shift_name, RegNames[rs]);
  }
  if( shift_amount == 0 ) {
    if( shift_type == 3 ) {
      return Utils::Format(
        "%-8s%s%s%s, RRX",
        std::string(opname).append(cond_to_str(cond)).append(sbit_str).c_str(),
        rd_str.c_str(), rn_str.c_str(), RegNames[rm]);
    }
    shift_amount = 32;
  }
  return Utils::Format(
    "%-8s%s%s%s, %s #%u",
    std::string(opname).append(cond_to_str(cond)).append(sbit_str).c_str(),
    rd_str.c_str(), rn_str.c_str(), RegNames[rm], shift_name, shift_amount);
}

std::string ARM_Disasm::DisassembleBranch(uint32_t addr, Opcode opcode,
                                          uint32_t insn)
{
  uint8_t cond = (insn >> 28) & 0xf;
  ...
  const char *opname = opcode_names[opcode];
  return Utils::Format(
    "%-8s0x%X", std::string(opname).append(cond_to_str(cond)).c_str(), addr);
}
...
std::string ARM_Disasm::DisassembleBX(uint32_t insn)
{
  ...
  return Utils::Format("%-8s%s",
                       std::string("bx").append(cond_to_str(cond)).c_str(),
                       RegNames[rn]);
}
...
std::string ARM_Disasm::DisassembleCLZ(uint32_t insn)
{
  ...
  return Utils::Format("%-8s%s, %s",
                       std::string("clz").append(cond_to_str(cond)).c_str(),
                       RegNames[rd], RegNames[rm]);
}
...
std::string ARM_Disasm::DisassembleMemblock(Opcode opcode, uint32_t insn)
{
  ...
  return Utils::Format(
    "%-8s%s%s, {%s}%s",
    std::string(opname).append(cond_to_str(cond)).append(addr_mode).c_str(),
    RegNames[rn], bang, tmp_list.c_str(), carret);
}
...
std::string ARM_Disasm::DisassembleMem(uint32_t insn)
{
  ...
  if( is_reg == 0 ) {
    if( is_pre ) {
      if( offset == 0 ) {
        return Utils::Format(
          "%-8s%s, [%s]",
          std::string(opname).append(cond_to_str(cond)).append(byte).c_str(),
          RegNames[rd], RegNames[rn]);
      }
      else {
        return Utils::Format(
          "%-8s%s, [%s, #%s%u]%s",
          std::string(opname).append(cond_to_str(cond)).append(byte).c_str(),
          RegNames[rd], RegNames[rn], minus, offset, bang);
      }
    }
    else {
      const char *transfer = "";
      if( write_back ) transfer = "t";

      return Utils::Format("%-8s%s, [%s], #%s%u",
                           std::string(opname)
                             .append(cond_to_str(cond))
                             .append(byte)
                             .append(transfer)
                             .c_str(),
                           RegNames[rd], RegNames[rn], minus, offset);
    }
  }
  ...
  if( is_pre ) {
    if( shift_amount == 0 ) {
      if( shift_type == 0 ) {
        return Utils::Format(
          "%-8s%s, [%s, %s%s]%s",
          std::string(opname).append(cond_to_str(cond)).append(byte).c_str(),
          RegNames[rd], RegNames[rn], minus, RegNames[rm], bang);
      }
      if( shift_type == 3 ) {
        return Utils::Format(
          "%-8s%s, [%s, %s%s, RRX]%s",
    ...
    return Utils::Format(
      "%-8s%s, [%s, %s%s, %s #%u]%s",
      std::string(opname).append(cond_to_str(cond)).append(byte).c_str(),
      RegNames[rd], RegNames[rn], minus, RegNames[rm], shift_name, shift_amount,
      bang);
  }
  ...
  if( shift_amount == 0 ) {
    if( shift_type == 0 ) {
      return Utils::Format(
        "%-8s%s, [%s], %s%s",
        std::string(opname).append(cond_to_str(cond)).append(byte).c_str(),
        transfer, RegNames[rd], RegNames[rn], minus, RegNames[rm]);
    }
    if( shift_type == 3 ) {
      return Utils::Format("%-8s%s, [%s], %s%s, RRX",
                           std::string(opname)
                             ...
                             .append(transfer)
                             .c_str(),
                           RegNames[rd], RegNames[rn], minus, RegNames[rm]);
    }
    shift_amount = 32;
  }
  ...
  if( is_immed ) {
    if( is_pre ) {
      if( offset == 0 ) {
        return Utils::Format(
          "%-8s%s, [%s]",
          std::string(opname).append(cond_to_str(cond)).append("h").c_str(),
          RegNames[rd], RegNames[rn]);
      }
      else {
        return Utils::Format(
...
std::string ARM_Disasm::DisassembleMCR(Opcode opcode, uint32_t insn)
{
  ...
  return Utils::Format("%-8s%d, 0, %s, cr%d, cr%d, {%d}",
                       std::string(opname).append(cond_to_str(cond)).c_str(),
                       cpnum, RegNames[crd], crn, crm, opcode2);
}
...
std::string ARM_Disasm::DisassembleMLA(Opcode opcode, uint32_t insn)
{
  ...
  return Utils::Format("%-8s%s, %s, %s, %s",
                       std::string(opname)
                         ...
                         .append(bit_s ? "s" : "")
                         .c_str(),
                       RegNames[rd], RegNames[rm], RegNames[rs], RegNames[rn]);
}
...
std::string ARM_Disasm::DisassembleUMLAL(Opcode opcode, uint32_t insn)
{
  ...
  return Utils::Format("%-8s%s, %s, %s, %s",
                       std::string(opname)
                         ...
                         .append(bit_s ? "s" : "")
                         .c_str(),
                       RegNames[rdlo], RegNames[rdhi], RegNames[rm],
                       RegNames[rs]);
}
...
std::string ARM_Disasm::DisassembleMUL(Opcode opcode, uint32_t insn)
{
  ...
  return Utils::Format("%-8s%s, %s, %s",
                       std::string(opname)
                         ...
                         .append(bit_s ? "s" : "")
                         .c_str(),
                       RegNames[rd], RegNames[rm], RegNames[rs]);
}
...
std::string ARM_Disasm::DisassembleMRS(uint32_t insn)
{
  ...
  return Utils::Format("%-8s%s, %s",
                       std::string("mrs").append(cond_to_str(cond)).c_str(),
                       RegNames[rd], ps ? "spsr" : "cpsr");
}
...
std::string ARM_Disasm::DisassembleMSR(uint32_t insn)
{
  ...
  if( mask & 1 ) flags[flag_index++] = 'c';
  if( mask & 2 ) flags[flag_index++] = 'x';
  if( mask & 4 ) flags[flag_index++] = 's';
  if( mask & 8 ) flags[flag_index++] = 'f';
  flags[flag_index] = 0;
  ...
  if( is_immed ) {
    ...
    return Utils::Format("%-8s%s_%s, #0x%X",
                         std::string("msr").append(cond_to_str(cond)).c_str(),
  ...
  return Utils::Format("%-8s%s_%s, %s",
                       std::string("mrs").append(cond_to_str(cond)).c_str(),
                       pd ? "spsr" : "cpsr", flags, RegNames[rm]);
}
...
std::string ARM_Disasm::DisassemblePLD(uint32_t insn)
{
  ...
  if( is_reg ) {
    uint8_t rm = insn & 0xf;
    return Utils::Format("%-8s[%s, %s%s]", "pld", RegNames[rn], minus,
                         RegNames[rm]);
  }
...
std::string ARM_Disasm::DisassembleSWI(uint32_t insn)
{
  ...
  return Utils::Format(
    "%-8s0x%X", std::string("svc").append(cond_to_str(cond)).c_str(), sysnum);
}
...
std::string ARM_Disasm::DisassembleSWP(Opcode opcode, uint32_t insn)
{
  ...
  return Utils::Format("%-8s%s, %s, [%s]",
                       std::string(opname).append(cond_to_str(cond)).c_str(),
                       RegNames[rd], RegNames[rm], RegNames[rn]);
}
...
Opcode ARM_Disasm::Decode01(uint32_t insn)
{
  ...
  if( is_reg == 1 && bit4 == 1 ) return OP_UNDEFINED;
...
Opcode ARM_Disasm::DecodeMUL(uint32_t insn)
{
  ...
  if( bit23 == 0 ) {
    ...
    if( bit21_A == 0 ) return OP_MUL;
    return OP_MLA;
  }
  // 64-bit multiply
  if( bit22_U == 0 ) {
    // Unsigned multiply long
    if( bit21_A == 0 ) return OP_UMULL;
    return OP_UMLAL;
  }
  // Signed multiply long
  if( bit21_A == 0 ) return OP_SMULL;
  return OP_SMLAL;
}
...
Opcode ARM_Disasm::DecodeALU(uint32_t insn)
{
  ...
  switch( opcode ) {
    ...
    case 0x7:
      return OP_RSC;
    case 0x8:
      if( bit_s ) return OP_TST;
      return OP_MRS;
    case 0x9:
      if( bit_s ) return OP_TEQ;
      return OP_MSR;
    case 0xa:
      if( bit_s ) return OP_CMP;
      return OP_MRS;
    case 0xb:
      if( bit_s ) return OP_CMN;
      return OP_MSR;
    case 0xc:
      return OP_ORR;
    case 0xd:
