...
Result plgLdrInit(void)
{
  ...
  if( R_SUCCEEDED(res) && R_SUCCEEDED((res = PLGLDR__GetArbiter())) ) {
...
void plgLdrExit(void)
{
  ...
  if( plgLdrHandle ) svcCloseHandle(plgLdrHandle);
  if( plgLdrArbiter ) svcCloseHandle(plgLdrArbiter);
...
Result PLGLDR__GetVersion(u32 *version)
{
  ...
  if( R_SUCCEEDED((res = svcSendSyncRequest(plgLdrHandle))) ) {
    if( cmdbuf[0] != IPC_MakeHeader(8, 2, 0) ) return 0xD900182F;

    res = cmdbuf[1];
    if( version ) *version = cmdbuf[2];
  }
  return res;
}
...
Result PLGLDR__SetSwapSettings(const char *swapPath, void *encFunc,
                               void *decFunc, void *args)
{
  Result res = 0;
...
void PLGLDR__SetEventCallback(OnPlgLdrEventCb_t cb) { onPlgLdrEventCb = cb; }
...
void PLGLDR__Reply(s32 event)
{
  ...
  if( event == PLG_ABOUT_TO_SWAP ) {
    ...
    svcArbitrateAddress(plgLdrArbiter, (u32)plgEvent,
                        ARBITRATION_WAIT_IF_LESS_THAN, PLG_OK, 0);
  }
