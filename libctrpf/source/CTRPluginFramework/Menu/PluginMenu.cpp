...
PluginMenu::PluginMenu(std::string name, std::string about, u32 menuType)
  : OnFirstOpening{nullptr},
    OnOpening{nullptr},
    OnNewFrame{nullptr},
    _menu(new PluginMenuImpl(name, about, menuType))
{
}
    ...
    OnNewFrame{nullptr}
{
  ...
  _menu = std::unique_ptr<PluginMenuImpl>(
    new PluginMenuImpl(name, aboutStr, menuType));
}

PluginMenu::PluginMenu(std::string name, u32 major, u32 minor, u32 revision,
                       std::string about, u32 menuType)
  : OnFirstOpening{nullptr},
    OnOpening{nullptr},
    OnNewFrame{nullptr},
    _menu(new PluginMenuImpl(name, about, menuType))
{
  u32 version =
    (major & 0xFF) | ((minor & 0xFF) << 8) | ((revision & 0xFF) << 16);
...
{
  if( callback != nullptr ) _menu->Callback(callback);
}

void PluginMenu::operator-=(CallbackPointer callback) const
{
  if( callback != nullptr ) _menu->RemoveCallback(callback);
}
...
bool PluginMenu::IsOpen(void) { return (_menu->IsOpen()); }

bool PluginMenu::WasOpened(void) { return (_menu->WasOpened()); }
...
void PluginMenu::ForceOpen(void) { PluginMenuImpl::ForceOpen(); }
...
std::string &PluginMenu::Title(void) { return _menu->_home->_root->name; }
std::string &PluginMenu::ScreenshotPath(void) { return Screenshot::Path; }
std::string &PluginMenu::ScreenshotFilePrefix(void)
{
  return Screenshot::Prefix;
}

void PluginMenu::ScreenshotUpdatePaths(void) { Screenshot::UpdateFileCount(); }
...
{
  return isTopScreen ? (Preferences::topBackgroundImage != nullptr)
                     : (Preferences::bottomBackgroundImage != nullptr);
}
}  // namespace CTRPluginFramework
