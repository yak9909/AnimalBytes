...
void Hotkey::operator=(u32 keys) { _keys = keys; }

void Hotkey::operator=(const std::string& name) { _name = name; }

bool Hotkey::IsDown(void) const { return (Controller::IsKeysDown(_keys)); }
...
u32 Hotkey::GetKeys(void) { return (_keys); }

const std::string& Hotkey::GetName() const { return _name; }

static const std::string g_keyName[16] = {
  FONT_A, FONT_B, "Select", "Start", FONT_DR, FONT_DL, FONT_DU, FONT_DD,
  FONT_R, FONT_L, FONT_X,   FONT_Y,  "",      "",      FONT_ZL, FONT_ZR};
...
{
  if( withName ) return (_name + ": " + KeysToString(_keys));
...
{
  if( index >= _hotkeys.size() ) return (g_dummy);
...
void HotkeyManager::AskForKeys(void)
{
  ...
  for( Hotkey& hotkey : _hotkeys ) hkNames.push_back(hotkey._name);
  ...
  do {
    ...
    if( ret != -1 ) {
      ...
      if( _callback != nullptr ) _callback(_owner, ret);
...
u32 HotkeyManager::Count(void) { return (_hotkeys.size()); }
}  // namespace CTRPluginFramework
