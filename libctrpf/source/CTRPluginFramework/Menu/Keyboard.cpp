...
bool stou32(std::string &input, u32 &res)
{
  ...
  while( *str ) {
    ...
    if( res > UINT32_MAX / 10 ) goto overflow;

    res *= 10;

    // Check that the addition won't overflow
    if( (res > 0) && (static_cast<u32>(val) > UINT32_MAX - res) ) goto overflow;
...
bool stou64(std::string &input, u64 &res)
{
  ...
  while( *str ) {
    ...
    // Check that the mutiplication won't overflow
    if( res > U64_MAX / 10 ) goto overflow;

    res *= 10;

    // Check that the addition won't overflow
    if( (res > 0) && (static_cast<u64>(val) > U64_MAX - res) ) goto overflow;
...
void Keyboard::IsHexadecimal(bool isHex) { _hexadecimal = isHex; }
...
int Keyboard::GetLastSelectedEntry() { return _keyboard->GetSelectedEntry(); }
...
{
  if( SystemImpl::IsSleeping() ) return (-2);

  if( !_isPopulated ) return (-1);
...
int Keyboard::Open(u8 &output) const
{
  ...
  if( ret >= 0 ) {
    ...
    output =
      *(static_cast<u8 *>(ConvertToU8(input, _keyboard->IsHexadecimal())));
  }
  return (ret);
}
...
int Keyboard::Open(u16 &output) const
{
  ...
  if( ret >= 0 ) {
    std::string &input = _keyboard->GetInput();
    output =
      *(static_cast<u16 *>(ConvertToU16(input, _keyboard->IsHexadecimal())));
  }
  return (ret);
}
...
int Keyboard::Open(u32 &output) const
{
  ...
  if( ret >= 0 ) {
    std::string &input = _keyboard->GetInput();
    output =
      *(static_cast<u32 *>(ConvertToU32(input, _keyboard->IsHexadecimal())));
  }
  return (ret);
}
...
int Keyboard::Open(u64 &output) const
{
  ...
  if( ret >= 0 ) {
    std::string &input = _keyboard->GetInput();
    output =
      *(static_cast<u64 *>(ConvertToU64(input, _keyboard->IsHexadecimal())));
  }
  return (ret);
}
...
int Keyboard::Open(float &output) const
{
  ...
  if( ret >= 0 ) {
    std::string &input = _keyboard->GetInput();
    output = *(
      static_cast<float *>(ConvertToFloat(input, _keyboard->IsHexadecimal())));
  }
  return (ret);
}
...
int Keyboard::Open(double &output) const
{
  ...
  if( ret >= 0 ) {
    ...
    output = *(static_cast<double *>(
      ConvertToDouble(input, _keyboard->IsHexadecimal())));
  }
  return (ret);
}
...
std::string &Keyboard::GetInput(void) const { return (_keyboard->GetInput()); }
...
void Keyboard::SetError(std::string error) const { _keyboard->SetError(error); }

void Keyboard::CanAbort(bool canAbort) const { _keyboard->CanAbort(canAbort); }

void Keyboard::Close(void) const { _keyboard->Close(); }
