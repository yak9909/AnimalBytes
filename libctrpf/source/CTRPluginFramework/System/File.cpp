...
int File::Create(const std::string &path)
{
  ...
  if( fsPath.Error ) return (INVALID_PATH);

  Result res;

  res = FSUSER_CreateFile(_sdmcArchive, fsPath, 0, 0);
  if( R_SUCCEEDED(res) ) return (SUCCESS);
  return (res);
}
...
int File::Rename(const std::string &oldPath, const std::string &newPath)
{
  ...
  if( oldFsPath.Error || newFsPath.Error ) return (INVALID_PATH);
...
enum PriorityForApplication {
  PRIORITY_APP_REALTIME =
    -16,  //!< Specifies that file accesses have real-time priority.
  PRIORITY_APP_NORMAL =
    -8,                 //!< Specifies that file accesses have normal priority.
...
int File::Open(File &output, const std::string &path, int mode)
{
  ...
  if( mode & TRUNCATE ) FSFILE_SetSize(handle, 0);
...
int File::Read(void *buffer, u32 length) const
{
  ...
  if( !buffer || !Process::CheckAddress((u32)buffer, MEMPERM_WRITE) )
    return (INVALID_ARG);
...
int File::Write(const void *data, u32 length)
{
  ...
  if( !data || !Process::CheckAddress((u32)data, MEMPERM_READ) )
    return (INVALID_ARG);
  if( length == 0 ) return (SUCCESS);
  ...
  if( _mode & APPEND ) res = FSFILE_GetSize(_handle, &_offset);
...
u64 File::Tell(void) const { return (_offset); }
...
void File::SetPriority(u32 priority) { FSFILE_SetPriority(_handle, priority); }
...
int File::Dump(u32 address, u32 length)
{
  ...
  if( !_isOpen ) return (NOT_OPEN);
  if( !(_mode & WRITE) ) return (INVALID_MODE);
  if( !Process::CheckAddress(address, MEMPERM_READ) ) return (INVALID_ARG);
  ...
  if( unpause ) ProcessImpl::Play(false);
...
int File::Inject(u32 address, u32 length) const
{
  ...
  if( buffer == nullptr )
    return (MAKERESULT(RL_USAGE, RS_OUTOFRESOURCE, 0,
                       RD_OUT_OF_MEMORY));  ///< I think it'll abort() so it
  ...
  do {
    ...
    // Then patch process
    if( !Process::Patch(address, buffer, size) ) res = UNEXPECTED_ERROR;
...
bool File::IsOpen(void) const { return (_isOpen); }

std::string File::GetFullName(void) const { return (_path); }

std::string File::GetName(void) const
{
  size_t pos = _path.rfind("/");

  if( pos != std::string::npos ) return (_path.substr(pos + 1));
