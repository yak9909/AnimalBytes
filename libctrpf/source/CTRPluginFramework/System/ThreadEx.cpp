...
static ThreadVars *CreateThreadVars(ThreadVars *mainThreadVars)
{
  ...
  ThreadVarsHook *tv =
    (ThreadVarsHook *)_malloc_r(mainThreadVars->reent, tvsize + tlssize);
  if( !tv ) return NULL;
  ...
  if( tlsloadsize ) memcpy(tls, __tdata_lma, tlsloadsize);
  if( tbsssize ) memset((u8 *)tls + tlsloadsize, 0, tbsssize);
...
ThreadVars *__ctrpf_getThreadVars(ThreadVars *mainThreadVars)
{
  ...
  if( ((RecursiveLock *)&__mutex)->counter > 1 ) return mainThreadVars;
  ...
  if( pair != __threads.end() ) return pair->second;
...
ThreadEx::ThreadEx(ThreadFunc entrypoint, u32 stackSize, u32 priority,
                   int affinity)
...
Result ThreadEx::Start(void *arg)
{
  ...
  if( R_SUCCEEDED(res) ) _state = RUNNING;
...
{
  if( _thread ) return ((CThread_tag *)_thread)->handle;
  return 0;
}

u32 ThreadEx::GetStatus(void) { return _state; }

void ThreadEx::Yield(void) { svcSleepThread(0); }
}  // namespace CTRPluginFramework
