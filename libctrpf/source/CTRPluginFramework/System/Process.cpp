...
Handle Process::GetHandle(void) { return ProcessImpl::ProcessHandle; }

u32 Process::GetProcessID(void) { return ProcessImpl::ProcessId; }

u64 Process::GetTitleID(void) { return ProcessImpl::TitleId; }
...
bool Process::IsPaused(void) { return ProcessImpl::IsPaused > 0; }

void Process::Pause() { ProcessImpl::Pause(false); }
...
void Process::WaitForExit() { ProcessImpl::WaitForExit(); }

bool Process::Patch(u32 addr, void *patch, u32 length, void *original)
{
  return ProcessImpl::PatchProcess(addr, static_cast<u8 *>(patch), length,
                                   static_cast<u8 *>(original));
}

bool Process::Patch(u32 addr, u32 patch, void *original)
{
  return ProcessImpl::PatchProcess(addr, reinterpret_cast<u8 *>(&patch), 4,
                                   static_cast<u8 *>(original));
}
...
bool Process::ProtectMemory(u32 addr, u32 size, int perm)
{
  ...
  // if (R_FAILED(svcControlProcessMemory(ProcessImpl::ProcessHandle, addr,
  // addr, size, 6, perm))) 	return false;
...
bool Process::ProtectRegion(u32 addr, int perm)
{
  ...
  if( R_FAILED(svcQueryProcessMemory(&minfo, &pinfo, ProcessImpl::ProcessHandle,
                                     addr)) )
    goto error;
...
bool Process::CopyMemory(void *dst, const void *src, u32 size)
{
  ...
  if( !CheckAddress(src32) ) goto error;
  if( !CheckAddress(dst32) ) goto error;
  if( !CheckAddress(dst32 + size) ) goto error;
  if( !CheckAddress(src32 + size) ) goto error;
...
static bool ConvertString(void *output, const u8 *input, u32 size,
                          StringFormat outfmt)
{
...
bool Process::ReadString(u32 address, std::string &output, u32 size,
                         StringFormat format)
{
  if( !CheckAddress(address, MEMPERM_READ) ) return (false);
...
bool Process::WriteString(u32 address, const std::string &input,
                          StringFormat outFmt)
{
...
bool Process::WriteString(u32 address, const std::string &input, u32 size,
                          StringFormat outFmt)
{
  if( !CheckAddress(address, MEMPERM_READ | MEMPERM_WRITE) || input.empty() )
    return (false);
