...
int Directory::ChangeWorkingDirectory(const std::string &path)
{
  ...
  if( path[path.size() - 1] != '/' ) FSPath::CurrentWorkingDirectory += '/';
...
int Directory::Rename(const std::string &oldPath, const std::string &newPath)
{
  ...
  if( fsNewPath.Error || fsOldPath.Error ) return (INVALID_PATH);

  Result res;

  res =
    FSUSER_RenameDirectory(_sdmcArchive, fsOldPath, _sdmcArchive, fsNewPath);
  if( R_SUCCEEDED(res) ) return (SUCCESS);
...
int Directory::Open(Directory &output, const std::string &path, bool create)
{
  ...
  if( R_FAILED(res) ) {
    if( create ) {
      if( (res = Create(path)) != 0 ) return (res);
      res = FSUSER_OpenDirectory(&handle, _sdmcArchive, fsPath);
...
int Directory::_List(void) const
{
  ...
  FS_DirectoryEntry *entry =
    (FS_DirectoryEntry *)Heap::Alloc(sizeof(FS_DirectoryEntry) * 100);
  u32 entriesNb = 0;
  ...
  std::sort(
    ...
    [](const DirectoryEntry &lhs, const DirectoryEntry &rhs) {
      ...
      do {
        ...
        if( leftUnits == -1 || rightUnits == -1 ) break;
        left += leftUnits;
        right += rightUnits;
...
** List files
***************/
int Directory::ListFiles(std::vector<std::string> &files,
                         const std::string &pattern) const
{
  ...
  for( DirectoryEntry &entry : _list ) {
    ...
    if( entry.attributes & 1 ) continue;

    std::string &fn = entry.name;
    if( patternCheck && fn.find(pattern) == std::string::npos ) continue;
...
** List folders
*****************/
int Directory::ListDirectories(std::vector<std::string> &folders,
                               const std::string &pattern) const
{
  Lock lock(_mutex);
...
std::string Directory::GetFullName(void) const { return (_path); }

bool Directory::IsOpen(void) const { return (_isOpen); }
