...
Hook& Hook::operator=(const Hook& hook)
{
  if( _ctx ) {
    ...
    if( _ctx->refcount == 0 ) delete _ctx;
  }
  _ctx = hook._ctx;
  AtomicIncrement(&_ctx->refcount);
...
Hook& Hook::InitializeForSubWrap(u32 targetAddr, u32 beforeCallback,
                                 u32 afterCallback)
{
  if( !_ctx ) goto exit;
...
{
  if( _ctx ) _ctx->flags = flags;
  return *this;
}

Hook& Hook::SetReturnAddress(u32 returnAddr)
{
  if( _ctx ) _ctx->returnAddress = returnAddr;
  return *this;
}

bool Hook::IsEnabled(void) { return _ctx && _ctx->index != 0xFFFFFFFF; }

static bool IsTargetAlreadyHooked(u32 target, u32 instruction)
{
  if( (instruction >> 24) != 0xEA ) return false;
...
static bool IsInstructionPCDependant(u32 instruction)
{
  ...
  for( u32 forbiddenInstruction : forbiddenInstructions )
    if( instruction == forbiddenInstruction ) return true;
...
{
  if( !_ctx ) return HookResult::InvalidContext;

  if( IsEnabled() ) return HookResult::Success;
...
const HookContext& Hook::GetContext(void) const { return *_ctx; }
