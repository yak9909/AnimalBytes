...
{
  if( useRightFb && (!IsTop || !Is3DEnabled) ) return (nullptr);
...
u32 Screen::Draw(const std::string &str, u32 posX, u32 posY,
                 const Color &foreground, const Color &background) const
{
...
u32 Screen::DrawSysfont(const std::string &str, u32 posX, u32 posY,
...
void Screen::DrawRect(u32 posX, u32 posY, u32 width, u32 height,
...
void OSD::Run(OSDCallback cb)
{
  ...
  if( OSDImpl::CallbacksTrashBin.size() ) {
    auto it = std::find(OSDImpl::CallbacksTrashBin.begin(),
                        OSDImpl::CallbacksTrashBin.end(), cb);
    if( it != OSDImpl::CallbacksTrashBin.end() )
      OSDImpl::CallbacksTrashBin.erase(it);
  }

  if( std::find(OSDImpl::Callbacks.begin(), OSDImpl::Callbacks.end(), cb) ==
      OSDImpl::Callbacks.end() )
...
void OSD::Stop(OSDCallback cb)
{
  ...
  if( std::find(OSDImpl::CallbacksTrashBin.begin(),
                OSDImpl::CallbacksTrashBin.end(),
                cb) == OSDImpl::CallbacksTrashBin.end() )
...
{
  if( sysfont ) return Renderer::GetTextSize(text.c_str());
  return Renderer::LinuxFontSize(text.c_str());
}

const Screen &OSD::GetTopScreen(void) { return OSDImpl::TopScreen; }

const Screen &OSD::GetBottomScreen(void) { return OSDImpl::BottomScreen; }
...
void OSD::Lock(void) { OSDImpl::Lock(); }

bool OSD::TryLock(void) { return (OSDImpl::TryLock()); }

void OSD::Unlock(void) { OSDImpl::Unlock(); }
}  // namespace CTRPluginFramework
