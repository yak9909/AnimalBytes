...
std::string ResetColor(void) { return ("\x18"); }
...
static size_t SafeStringHalve(const std::string& str)
{
  ...
  for( u32 i = 0; i < 4 && middle >= i; i++ ) {
    ...
    if( c == 0x11 || c == 0x1B ) {  // Special characters, cannot cut, max
                                    // length special character is 4.
      middle = middle - i;
      i = 0;
      if( middle == 0 ) break;
    }
...
static std::string CenterAlignImpl(const std::string& str, u16 leftBorder,
                                   u16 rightBorder, int rec)
{
...
std::string RightAlignImpl(const std::string& str, u16 leftBorder,
                           u16 rightBorder, int rec)
{
  if( rec >= 10 )  // Hacky and lazy way to prevent infinite recursion.
    return "";
