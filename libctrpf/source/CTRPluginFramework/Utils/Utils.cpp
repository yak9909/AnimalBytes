...
{
  return (fpval > 999999.f || fpval < -999999.f
            ? Format(Format("%%.%de", precision).c_str(), fpval)
            : Format(Format("%%.%df", precision).c_str(), fpval));
}
...
u32 Utils::Random(void) { return (Random(0, 0)); }
                        ...
                        bool listFiles)
{
  ...
  if( folder.ItemsCount() > 0 ) folder.Clear();
  ...
  if( path.size() > 1 && path[path.size() - 1] != '/' ) path.append("/");
...
static int SDExplorerInternal(std::string &out, const std::string &filter,
                              bool selDirectory)
{
  ...
  Clock clock;
  SubMenu submenu(
    {"Create folder", "Create file", "Rename", "Delete", "Cancel"});
  ...
  do {
    ...
    while( eventManager.PollEvent(event) ) {
      if( !submenu.IsOpen() )
        if( (menuEvent = menu.ProcessEvent(event, &item)) != Nothing ) break;
      submenu.ProcessEvent(event);

      // Check for start
      if( selDirectory && event.type == Event::KeyPressed &&
      ...
      if( menuEvent >= 0 ) {
        ...
        if( menuEvent == 3 )  // Delete
        {
          ...
          // Display a little warning
          {
            ...
            MessageBox msgBox(Color::Orange << "Warning", body,
            ...
            if( msgBox() ) {
              ...
              if( R_SUCCEEDED(res) ) {
                ...
                MessageBox(Color::Red << "Error",
                           Utils::Format("Operation failed: %08X", res))();
            }
          }
        }
    ...
    int posY = 55;
    Renderer::DrawSysStringReturn(footer_cstr, 50, posY, 300,
                                  Preferences::Settings.MainTextColor);
  ...
  if( !selDirectory ) {
    ...
    if( entry ) out.append(entry->name);
  }
...
u32 Utils::OpenInHexEditor(u32 address, HexEditorView view)
{
  ...
  while( true ) {
    if( SystemImpl::WantsToSleep() ) break;
    eventList.clear();
  ...
  if( SystemImpl::WantsToSleep() ) SystemImpl::ReadyToSleep();
