...
Result OnSharedMemMap(Handle handle, void *sharedMem, MemPerm myPerm,
                      MemPerm otherPerm)
{
  Result res = svcMapMemoryBlock(handle, (u32)sharedMem, myPerm, otherPerm);
  if( R_SUCCEEDED(res) && sharedMem &&
      CTRPluginFramework::FwkSettings::Get().UseGameHidMemory ) {
    ...
    // Probably breaks if hid thread is paused just here, but the likelyness of
    // this happening is very low, so I guess it's good enough...
    if( llabs(currSysTick - firstSysTick) < 1000000000ULL &&
        llabs(currSysTick - firstSysTickTS) < 1000000000ULL && arrayIndex < 8 &&
        arrayIndexTS < 8 )
...
void KeepThreadMain(void *arg UNUSED)
{
  ...
  // Install CRO hook
  {
    const std::vector<u32> LoadCroPattern = {
      0xE92D5FFF, 0xE28D4038, 0xE89407E0, 0xE28D4054, 0xE8944800, 0xEE1D4F70,
      0xE59FC058, 0xE3A00000, 0xE5A4C080, 0xE284C028, 0xE584500C, 0xE584A020};
    ...
    u32 loadCroAddress =
      Utils::Search<u32>(0x00100000, Process::GetTextSize(), LoadCroPattern);
  ...
  // Install svcMapMemoryBlock hook
  {
    ...
    do {
      svcMapMemoryBlockAddr = Utils::Search<u32>(
        svcMapMemoryBlockAddr + 4, Process::GetTextSize(), {0xEF00001F});
      if( svcMapMemoryBlockAddr ) {
        for( int i = 0; i > -10; i-- ) {
          if( reinterpret_cast<u32 *>(svcMapMemoryBlockAddr)[i] ==
              0xE3A03201 )  // MOV R3, #0x10000000
          {
            g_onSharedMemMapHook.Initialize(svcMapMemoryBlockAddr,
                                            (u32)OnSharedMemMap);
  ...
  if( settings.UseGameHidMemory ) {
    ...
    if( !g_onSharedMemMapHook.IsEnabled() ) abort();
  }
  else {
    hidExitFake();
    hidInit();
  ...
  // Reduce priority
  while( R_FAILED(
    svcSetThreadPriority(g_keepThreadHandle, settings.ThreadPriority - 1)) )
    ;
  ...
  svcControlProcess(CUR_PROCESS_HANDLE, PROCESSOP_GET_ON_MEMORY_CHANGE_EVENT,
                    (u32)&memLayoutChanged, 0);
  while( true ) {
    if( svcWaitSynchronization(memLayoutChanged, 100000000ULL) ==
        0x09401BFE )  // 0.1s
    {
...
void Initialize(void)
{
  ...
  Font::Initialize();
  {
    ...
    if( !Directory::IsExists(dirpath) ) Directory::Create(dirpath);
  }
  ...
  if( !File::Exists(Preferences::CheatsFile) )
    Preferences::CheatsFile =
      Utils::Format("/cheats/%016llX.txt", Process::GetTitleID());

  {
    ...
    Screenshot::Prefix +=
      Utils::Format(" - %08X] - Screenshot", (u32)Process::GetTitleID());
...
void ThreadInit(void *arg)
{
  ...
  svcSetThreadPriority(threadGetHandle(threadGetCurrent()),
                       FwkSettings::Get().ThreadPriority);
...
void ThreadExit(void)
{
  ...
  if( reinterpret_cast<u32>(((CThread_tag *)threadGetCurrent())->stacktop) <
      0x07000000 ) {
    // ## Main Thread ##
...
extern "C" int __entrypoint(int arg)
{
  // Call early callback, with pointer to the 2 saved instructions
  if( EarlyCallback ) EarlyCallback((u32 *)arg);
  ...
  svcCreateThread(&g_keepThreadHandle, KeepThreadMain, arg,
                  (u32 *)&keepThreadStack[0x1000], 0x1A, 0);
