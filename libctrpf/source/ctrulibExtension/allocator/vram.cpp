...
void* vramMemAlign(size_t size, size_t alignment)
{
  ...
  if( !sVramPool.Ready() && !vramInit() ) return nullptr;

  // Allocate the chunk
  MemChunk chunk;
  if( !sVramPool.Allocate(chunk, size, shift) ) return nullptr;
...
void* vramAlloc(size_t size) { return vramMemAlign(size, 0x80); }
...
u32 vramSpaceFree() { return sVramPool.GetFreeSpace(); }
