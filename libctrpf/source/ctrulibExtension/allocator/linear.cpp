...
void* linearMemAlign(size_t size, size_t alignment)
{
  ...
  if( !sLinearPool.Ready() && !linearInit() ) return nullptr;

  // Allocate the chunk
  MemChunk chunk;
  if( !sLinearPool.Allocate(chunk, size, shift) ) return nullptr;
...
void* linearAlloc(size_t size) { return linearMemAlign(size, 0x80); }
