...
bool MemPool::Allocate(MemChunk& chunk, u32 size, int align)
{
  ...
  if( align >= 32 || align < 0 ) return false;
  ...
  if( size & alignMask ) {
    ...
    if( size > UINT32_MAX - alignMask ) return false;
    ...
    if( !begWaste ) {
      ...
      if( nSize ) {
        ...
        if( n )
          ...
          chunk.size += nSize;  // we have no choice but to waste the space.
      }
    }
    return true;
  }
...
{
  u32 acc = 0;
  for( auto b = first; b; b = b->next ) acc += b->size;
  return acc;
}
