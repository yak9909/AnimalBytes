...
//---------------------------------------------------------------------------------
int __SYSCALL(gettod_r)(struct _reent *ptr, struct timeval *tp,
                        struct timezone *tz)
{
  //---------------------------------------------------------------------------------
  if( tp != NULL ) {
  ...
  if( tz != NULL ) {
    // Provide dummy information, as the 3DS does not have the concept of
    // timezones
    tz->tz_minuteswest = 0;
    tz->tz_dsttime = 0;
  }
...
void __SYSCALL(exit)(int rc) { __ctru_exit(rc); }
...
void __system_initSyscalls(void)
{
  ...
  if( tls_size ) memcpy(__tls_start, __tdata_lma, tls_size);
}
