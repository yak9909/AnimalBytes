...
static void _thread_begin(void* arg)
{
  ...
  tv->thread_ptr = t;
  tv->tls_tp =
    (u8*)t->stacktop - 8;  // ARM ELF TLS ABI mandates an 8-byte header
  t->ep(t->arg);
  threadExit(0);
}
...
Thread __createThread(ThreadFunc entrypoint, u32 stackSize, bool detached)
{
  ...
  if( tlsloadsize ) memcpy(t->stacktop, __tdata_lma, tlsloadsize);
  if( tbsssize ) memset((u8*)t->stacktop + tlsloadsize, 0, tbsssize);
...
Result __startThread(Thread t, int prio, int affinity)
{
  ...
  if( prio < 0x18 ) oldPrio = KProcess__PatchMaxPriority(0);

  rc = svcCreateThread(&t->handle, _thread_begin, (u32)t, (u32*)t->stacktop,
                       prio, affinity);

  if( oldAppType != -1 ) KProcess__PatchCategory(oldAppType);

  if( oldPrio != -1 ) KProcess__PatchMaxPriority(oldPrio);

  return rc;
}

Thread threadCreate(ThreadFunc entrypoint, void* arg, size_t stack_size,
