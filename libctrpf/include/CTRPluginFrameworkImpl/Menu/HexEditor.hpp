...
class HexEditor {
  ...
  enum {
    ...
    DirtyMemoryCache =
      1 << 2,  ///< Set if memory cache is different than real memory
    DirtyCursor = 1 << 3,  ///< Trigger a general cursor update based on current
                           ///< cursor address

    DirtyViewCache = 1 << 4,  ///< Trigger view display update
    DirtyCursorAddress =
      1 << 5,  ///< Trigger cursor address update based on current position
