...
namespace Services {
namespace GSP {
enum { TOP_SCREEN = 0, BOTTOM_SCREEN = 1 };
...
struct FrameBufferInfo {
  ...
  u32* framebuf0_vaddr;  ///< Framebuffer virtual address, for the main screen
                         ///< this is the 3D left framebuffer.
  u32* framebuf1_vaddr;  ///< For the main screen: 3D right framebuffer address.
  u32 framebuf_widthbytesize;  ///< Value for 0x1EF00X90, controls framebuffer
                               ///< width.
  u32 format;  ///< Framebuffer format, this u16 is written to the low u16 for
               ///< LCD register 0x1EF00X70.
  u32 framebuf_dispselect;  ///< Value for 0x1EF00X78, controls which
