...
class ScreenImpl {
 public:
  enum LCDSetup {
    WidthHeight = 0x5C,  ///< Framebuffer width & height  Lower 16 bits: width,
                         ///< upper 16 bits: height
    FramebufferA1 = 0x68,  ///< Framebuffer A first address For top screen, this
                           ///< is the left eye 3D framebuffer.
    FramebufferA2 = 0x6C,  ///< Framebuffer A second address    For top screen,
                           ///< this is the left eye 3D framebuffer.
    Format = 0x70,         ///< Framebuffer format  Bit0-15: framebuffer format,
                           ///< bit16-31: unknown
    Select = 0x78,  ///< Framebuffer select  Bit0: which framebuffer to display,
                    ///< bit1-7: unknown
    Stride = 0x90,  ///< Framebuffer stride  Distance in bytes between the start
                    ///< of two framebuffer rows (must be a multiple of 8).
    FramebufferB1 =
      0x94,  ///< Framebuffer B first address For top screen, this is the right
             ///< eye 3D framebuffer. Unused for bottom screen.
    FramebufferB2 =
      0x98  ///< Framebuffer B second address    For top screen, this is the
            ///< right eye 3D framebuffer. Unused for bottom screen.
  };
  ...
  void GetFrameBufferInfos(int &rowstride, int &bpp,
 ...
 private:
  ...
  friend void CTRPluginFrameworkImpl::Services::GSP::Update(
    u32 threadId, Handle eventHandle, Handle sharedMemHandle);
