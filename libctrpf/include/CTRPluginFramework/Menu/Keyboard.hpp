...
class KeyboardEvent {
 public:
  ...
  u32 codepoint{
    0};  ///< The codepoint of the character that thrown the event (used for
         ///< CharacterAdded and CharacterRemoved, 0 otherwise)
  u32 selectedIndex{0};  ///< The entry index in a custom keyboard being
                         ///< selected (used for SelectionChanged, 0 otherwise)
  Key affectedKey{
    (Key)0};  //< Button affected not mapped to any keyboard feature (used for
              // ButtonPressed, ButtonHold and ButtonReleased, 0 otherwise)
};
...
class Keyboard {
   ...
   * \param input     A void pointer to the input, must be casted to the correct
   * type (as passed to Keyboard::Open) \param error     A reference to the
   * string that hold the error message. You can edit it according to what you
  ...
  /**
   * \brief The signature of the callback called when the input change (user
   * enter / delete a character) \param keyboard  A reference to the Keyboard
   * object that called the callback \event event     The event that caused the
   * input to change
   */
 ...
 public:
   ...
   * \param canAbort  Whether the user can press B to close the keyboard and
   ...
   * \n Note that if a CompareCallback is set, CompareCallback is called before
   * OnKeyboardEvent \n See OnEventCallback's description for more infos \param
   * callback
   */
  ...
  /**
   * \brief Selects and scroll to the specified entry, must be called after
   * Populate \param entry Entry index to select, -1 to deselect all.
   */
   ...
   * \param resetScroll  Set to true to reset the scroll position if the list is
   * the same size
   */
   ...
   * \param input A std::vector that contains a list of CustomIcons (icon size
   * must be 30x30 pixels, otherwise a red cross will be displayed) \param
   * resetScroll  Set to true to reset the scroll position if the list is the
   * same size
   */
