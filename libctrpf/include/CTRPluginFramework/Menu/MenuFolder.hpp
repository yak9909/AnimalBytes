...
class MenuFolder {
 public:
   ...
   * Upon destruction all MenuEntry and MenuFolder objects contained by this
   ...
   * Will disable every MenuEntry objects contained in this folder and
   * subfolders
   */
  ...
  /**
   * \brief Get the number of items that his folder contains (not counting
   * subfolders's content) \return The count of items
   */
   ...
   * \param destroy If the elements must be destroyed (invalidate pointers and
   * release ressources)
   */
   ...
   * \return Whether the folder can be opened (on ActionType::Opening only,
   * ignored for ActionType::Closing)
   */
  using MenuFolder_OnActionFunc = bool (*)(MenuFolder &, ActionType);

  /**
   * \brief This callback is called when the folder is about to be opened or
   * closed
   */
