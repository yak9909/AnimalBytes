...
enum class Separator { None, Filled, Stippled };
#endif
...
class MenuEntry {
 ...
 public:
  ...
  MenuEntry(const std::string &name, FuncPointer gameFunc,
            const std::string &note = "");
  MenuEntry(const std::string &name, FuncPointer gameFunc, FuncPointer menuFunc,
            const std::string &note = "");
  MenuEntry(int radioGroup, const std::string &name, FuncPointer gameFunc,
            const std::string &note = "");
  MenuEntry(int radioGroup, const std::string &name, FuncPointer gameFunc,
  ...
  /**
   * \brief Check if this entry was just activated (first gamefunction
   * execution) \return true if the entry was just activated
   */
  ...
  /**
   * \brief Set if this entry must display a separator at the bottom of the
   * entry \param type Type of separator to display
   */
  void UseBottomSeparator(Separator type = Separator::Filled) const;

  /**
   * \brief Set if the entry can be selected in the menu or not.\n
   * If the entry is Activated and the state is set to unselectable, the entry
   * will be disabled \param canBeSelected
   */
  ...
  /**
   * \brief The hotkey manager of this entry. See MenuEntryHotkeys.hpp for more
   * infos
   */
