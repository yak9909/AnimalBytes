...
class Hotkey {
 public:
   ...
   * Appear on the note and on the Hotkey selector when the user want to edit an
   * hotkey
   */
   ...
   * \param name The new name you want to appear on the note / the
   * HotkeyModifier selector
   */
   ...
   * if withName is false then the output will simply be the Key's value: "A +
   * B" if withName is true the output will be: "Hotkey's name : A + B"
   */
...
class HotkeyManager {
 public:
  ...
  /**
   * \brief Display a control that allows the user to select which Hotkey he
   * wants to edit
   */
  void AskForKeys(void);
