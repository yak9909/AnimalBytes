...
class PluginMenu {
  ...
  ******************************/
  ...
  PluginMenu(std::string name, u32 major, u32 minor, u32 revision,
             std::string about = "", u32 menuType = 0);
  ...
  ******************************/
  ...
  /*
  ** Run the menu
  ** This call is blocking so be sure to do everyting before calling this
  *function
  ******************************/
  int Run(void) const;
  ...
  /**
   * \brief If set to true, the plugin's loop will only be executed 1 per top
   * screen's frame \param useSync Wheter to wait for the top screen's frame or
   * not
   */
  void SynchronizeWithFrame(const bool useSync);

  /**
   * \brief If a callback is set, the callback will be called - Must be set
   * before calling Run when the menu is opened for the first time
   */
  CallbackPointer OnFirstOpening;

  /**
   * \brief If a callback is set, the callback will be called  - Must be set
   * before calling Run when the menu is opened. Ideal to put the code that
   * refresh the UI. ;) Return true from the callback to proceed with menu
   * opening, return false otherwise.
   */
  OnOpeningCallback OnOpening;

  /**
   * \brief The callback set will be called at each frame rendered while the
   * menu is open Ideal to put some UI effect The function will receive the Time
   * elapsed since last frame Must be set before calling Run
   */
  ...
  /**
   * \brief Sets the screenshot callback, called whenever an screenshot is
   * taken. If the callback function returns false, the screenshot will be
   * aborted. \param callback The callback function
   */
   ...
   * \param isTopScreen Set to true to check the top screen background, set to
   * false to check the bottom screen. \return Wether a background image was
   * loaded for the specified screen.
   */
