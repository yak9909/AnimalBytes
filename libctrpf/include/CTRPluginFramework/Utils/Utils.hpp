...
enum class HexEditorView { Byte = 0, Integer = 1, Disassembler = 2 };
...
class Utils {
 public:
   ...
   * \param filter If the files must be filtered (can be an extension or a
   * pattern that must be present in the filename) \return -1 if the user
   * aborted the operation, 0 on success
   */
  ...
  /**
   * \brief Open the HexEditor menu at a specified address within the specified
   * view \param address The address to jump to in the HexEditor \param view The
   * type of view (can be changed when open) \return The address where the
   * cursor was when the HexEditor is closed
   */
  static u32 OpenInHexEditor(u32 address,
                             HexEditorView view = HexEditorView::Byte);
  ...
  template <typename T>
  static u32 Search(const u32 start, const u32 size,
                    const std::vector<T> &pattern)
  {
    if( !start || !size || pattern.empty() ) return (0);
    ...
    while( j <= size - patternSize ) {
      ...
      if( patternc[patternSize - 1] == c &&
          std::memcmp(patternc, startPos + j, patternSize - 1) == 0 )
  ...
  template <typename T>
  static u32 Rsearch(const u32 start, const u32 size,
                     const std::vector<T> &pattern)
  {
    if( !start || !size || pattern.empty() ) return (0);
