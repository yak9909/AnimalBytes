...
class Screen {
 public:
   ...
   * \param useRightFb If the function must return a pointer to the right
   * framebuffer \return A pointer to the screen's framebuffer at the desired
   * position
   */
   ...
   */
  u32 Draw(const std::string &str, u32 posX, u32 posY,
           const Color &foreground = Color::White,
           const Color &background = Color::Black) const;

  /**
   * \brief Draw a string using system font (support utf8 strings with special
   * chars & unicode) \param str The string to draw \param posX The position on
   * screen to draw the string to \param posY The position on screen to draw the
   * string to \param foreground The color of the characters \return posY + 16
   * (line feed)
   */
  u32 DrawSysfont(const std::string &str, u32 posX, u32 posY,
                  const Color &foreground = Color::White) const;
   ...
   * \param filled If the rectangle is filled or if the function should only
   * draw the border
   */
  void DrawRect(u32 posX, u32 posY, u32 width, u32 height, const Color &color,
   ...
   */
  void ReadPixel(u32 posX, u32 posY, Color &pixel,
                 bool fromRightFb = false) const;
...
class OSD {
 public:
  /**
   * \brief Send a notification on the top screen, a notification duration is 5
   * seconds\n A maximum of 50 notifications can be queued \param str Text of
   * the notification \param foreground The color of the text (Default: blank)
  ...
  /**
   * \brief Add a callback to the OSD system which will be called at each game's
   * frame \param cb The callback to add
   */
