...
enum {
  USE_LR_TO_RETURN =
    1 << 0,  ///< Enable the use of bx lr to return from callback. LR will be
             ///< properly restored after the callback is executed
  EXECUTE_OI_BEFORE_CB =
    1 << 1,  ///< If the instruction overwritten by the hook (target) must be
             ///< executed before the callback
  EXECUTE_OI_AFTER_CB =
    1 << 2,  ///< If the instruction overwritten by the hook (target) must be
...
enum class HookResult {
  Success,
  InvalidContext,  ///< The HookContext obj linked to the Hook obj is invalid
                   ///< (Should not happen)
  InvalidAddress,  ///< The target address was not reachable
  AddressAlreadyHooked,  ///< A hook is already enabled to the same address
  TooManyHooks,  ///< You attained the maximum of enabled hooks (current limit:
                 ///< 91)
  HookParamsError,  ///< The parameters of your hook seems off
  TargetInstructionCannotBeHandledAutomatically,  ///< The target's instruction
                                                  ///< is position dependent
                                                  ///< (PC) hence using
                                                  ///< ExecuteOverwrittenInstructionBeforeCallback
                                                  ///< or
                                                  ///< ExecuteOverwrittenInstructionAfterCallback
                                                  ///< is impossible.
};
...
struct Hook {
  ...
  /**
   * \brief Initialize hook target and callback. Hook's return address is set to
   * targetAddr + 4 \param targetAddr The address to hook to \param callbackAddr
   * The callback to be called by the hook \return A reference to Hook obj
   */
   ...
   * \param beforeCallback The callback to be called before the subroutine (can
   * be null) \param afterCallback The callback to be called after the
   * subroutine (can be null) \return A reference to Hook obj
   */
  Hook& InitializeForSubWrap(u32 targetAddr, u32 beforeCallback,
                             u32 afterCallback);

  /**
   * \brief Apply the specified flags to the hook. Must be done before enabling
   * the hook otherwise nothing happens \param flags The new hook settings
