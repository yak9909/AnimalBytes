...
class Process {
 public:
  ...
  /**
   * \brief Safely patch the current process (MemPerm check and Invalidate
   * cache) \param addr      The address to start the patch \param patch     The
   * patch values \param length    The length of the patch values in bytes
   * \param original  If defined, the original values will be copied to this
   * buffer (size must be enough) \return  If the patch was successful or not
   */
  static bool Patch(u32 addr, void *patch, u32 length,
                    void *original = nullptr);

  /**
   * \brief Safely patch the current process (MemPerm check and Invalidate
   * cache) \param addr      The address to start the patch \param patch     The
   * patch value \param original  If defined, the original values will be copied
   * to this buffer (size must be enough) \return  If the patch was successful
   * or not
   */
   ...
   */
  static bool ProtectMemory(u32 addr, u32 size,
                            int perm = (MEMPERM_READ | MEMPERM_WRITE |
                                        MEMPERM_EXECUTE));
   ...
   * \return  If the protection was successful or not
   */
  static bool ProtectRegion(u32 addr, int perm = (MEMPERM_READ | MEMPERM_WRITE |
                                                  MEMPERM_EXECUTE));
   ...
   */
  static void ProtectRegionInRange(u32 startAddress, u32 endAddress,
                                   int perm = (MEMPERM_READ | MEMPERM_WRITE |
                                               MEMPERM_EXECUTE));

  /**
   * \brief Safely copy the current process memory (MemPerm check) \n Like a
   * memcpy but safe \param dst   The destination to copy to \param src   The
   * source to copy from \param size  The size to copy in bytes \return
   */
   ...
   * \n If the address exists but don't have the specified permissions, it tries
   * to change the permissions \param address   The address to check \param perm
   * The permissions that the address must have \return If the address is valid
   * and have the specified permissions or not
   */
  static bool CheckAddress(u32 address,
                           u32 perm = MEMPERM_READ | MEMPERM_WRITE);

  /**
   * \brief Check if the region which the address is from is available and have
   * the specified permissions \n If the region exists but don't have the
   * specified permissions, it tries to change the permissions \param address
   * The address to check \param size      A reference to get the region's size
   ...
   */
  static bool CheckRegion(u32 address, u32 &size,
                          u32 perm = MEMPERM_READ | MEMPERM_WRITE);

  /**
   * \brief Gets the address of the best fitting memory region bigger or equal
   * than the specified size \n and past the starting address. Doesn't guarantee
   * that the process will try to use the same region later. \param size Minimum
   * size of the memory region \param startAddress   Starting address to search
   * from \return Adress of the best fitting memory region bigger than size and
   * past startAddress
   */
   ...
   */
  static bool ReadString(u32 address, std::string &output, u32 size,
                         StringFormat format);
   ...
   */
  static bool WriteString(u32 address, const std::string &input,
                          StringFormat outFmt = StringFormat::Utf8);
   ...
   * \return true if the write is successful, false otherwise
   */
  static bool WriteString(u32 address, const std::string &input, u32 size,
                          StringFormat outFmt = StringFormat::Utf8);
  ...
  using ExceptionCallback =
    ExceptionCallbackState (*)(ERRF_ExceptionInfo *excep, CpuRegisters *regs);
  /**
  * \brief A callback that will be executed if an exception is triggered.
  Overrides Luma exception handler. \n If this callback causes a new exception,
  the default exception handler will be executed instead. \n You can use OSD to
  get the screens and draw to them, followed to swap framebuffers.
  * \return Action to perform after each callback execution. LOOP makes the
  callback be executed again.
  */
  static ExceptionCallback exceptionCallback;

  /**
   *  \brief Determines the behaviour if the exception handler callback causes
   * another exception. If set to true, \n the first exception is thrown with
   * the default handler (luma). If set to false, the new exception is thrown
