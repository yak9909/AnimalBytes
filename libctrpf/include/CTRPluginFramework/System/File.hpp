...
class File {
 public:
  ...
  enum Mode {
    ...
    APPEND =
      1
      << 3,  ///< You'll be unable to overwrite the file, only append data to it
  ...
  enum OPResult {
    ...
    INVALID_MODE = -3,  ///< The mode passed when opened the file doesn't allow
                        ///< this operation
    INVALID_ARG = -4,   ///< One of the args passed to the operation is invalid
   ...
   * Either a value in \ref OPResult or an error code from FS service
   */
  static int Open(File &output, const std::string &path,
                  int mode = READ | WRITE | SYNC);
