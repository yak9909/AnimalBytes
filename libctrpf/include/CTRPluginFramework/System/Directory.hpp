...
class Directory {
 public:
  enum OPResult {
    ...
    INVALID_ARG = -3,   ///< One of the args passed to the operation is invalid
                        ///< (nullptr, address unreachable, etc)
   ...
   * \return Either a value from \ref OPResult or a FS return value
   */
  static int Open(Directory &output, const std::string &path,
                  bool create = false);
   ...
   * \return Either a value from \ref OPResult or a FS return value
   */
  int OpenFile(File &output, const std::string &path,
               int mode = File::RW) const;
   ...
   * \param pattern If specified, only the files that contain the pattern will
   * be returned \return The count of files found, OPResult::NOT_OPEN if the
   ...
   * Beware that the container is not emptied and results directories are
   * appended \param pattern If specified, only the directories that contain the
   * pattern will be returned \return The count of directories found,
   * OPResult::NOT_OPEN if the Directory is not opened
   */
  int ListDirectories(std::vector<std::string> &directories,
                      const std::string &pattern = "") const;
