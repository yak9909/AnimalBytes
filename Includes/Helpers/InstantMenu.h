...
class InstantMenuImpl {
  ...
  void draw(Screen const& screen)
  {
    ...
    // background
    screen.DrawRect(menu_draw_x, menu_draw_y, menu_width, menu_height,
                    background_color);

    // draw title
    screen.Draw(this->title, menu_draw_x + 2, menu_draw_y + 1);
    screen.DrawRect(menu_draw_x + 2, menu_draw_y + item_height + 1,
                    (this->title.length() + 2) * 9, 1, Color::White);
    ...
    // draw items
    {
      ...
      for( ; begin < end; begin++, dy += item_height ) {
        if constexpr( conv_fn == nullptr ) {
          if( index == begin ) {
            screen.Draw(items[begin], menu_draw_x + 2, dy, Color::White,
                        Color::Blue);
          }
          else {
 ...
 public:
  ...
  void set_title(std::string const& title) { this->title = title; }
  ...
  void set_item(int index, T const& value) { items[index] = value; }

  void append_item(T const& val) { items.emplace_back(val); }

  void append_item(T&& val) { items.emplace_back(std::move(val)); }

  std::vector<T> const& get_items() const { return this->items; }
