...
TextEditorImpl::SubmenuEntry::SubmenuEntry(std::string const& name,
                                           TextEditor::EventFuncPointer func)
  : name(name),
    func(func)
{
}
...
void TextEditorImpl::submenu_draw(Screen const& screen)
{
  ...
  // entries
  {
    ...
    static auto const draw_entry_name = [&](std::string const& name) -> u32 {
      return screen.Draw(
        name, x + 4, dy, begin == submenu_index ? Color::Black : Color::White,
    ...
    if( begin < end && !submenu_user_entries.empty() ) {
        ...
        std::min<int>(entries.size() + submenu_user_entries.size(), begin + 24);
...
void TextEditorImpl::submenu_update()
{
  ...
  static constexpr int mvCursor_RepeatReset =
    mvCursor_WaitToRepeat + mvCursor_RepeatTick;
    ...
    switch( submenu_index ) {
      ...
      case 0: {
        if( file.IsOpen() && !saved ) {
          auto warn = MessageBox(
            "Warning",
            "You are editing the file but it is not saved yet. Do you save ?",
      ...
      case 2: {
        if( !saved ) {
          if( !file.IsOpen() ) {
            auto res = Keyboard(
                         "You don't have opened any file. Please enter file "
                         "name to create and save.")
                         .Open(path);
            OSD::SwapBuffers();
            ...
            if( File::Exists(path) ) {
              if( MessageBox("The file \"" + path +
                               "\" is already exists, Do you want to "
                               "overwrite to it?",
                             DialogType::DialogYesNo)() == 0 )
                break;
            }
  ...
  if( keys ) {
    if( tick == 0 || tick == mvCursor_WaitToRepeat ||
        tick == mvCursor_RepeatReset ) {
