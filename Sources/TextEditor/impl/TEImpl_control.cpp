...
{
#define adjustX \
  if( cursor.x > data[cursor.y].length() ) cursor.x = data[cursor.y].length()
  ...
  if( type == KeyboardEvent::EventType::KeyPressed ||
      (clock.HasTimePassed(Milliseconds(keyRepeatWait)) &&
       clock2.HasTimePassed(Milliseconds(waitRepeatMilli))) ) {
    ...
    if( key & Hotkeys::CursorRight ) {
      if( ++cursor.x > data[cursor.y].length() ) {
        cursor.x = cursor.y < (signed)data.size() - 1 ? cursor.y++,
        0                                             : data[cursor.y].length();
      }
    }
