...
void TextEditorImpl::draw(Screen const& screen)
{
  ...
  // background
  screen.DrawRect(linenum_width + 1, 0, 400 - linenum_width - 1, 240,
                  Color(30, 30, 30));
  ...
  // lines
  {
    ...
    for( ; begin < end; begin++, dy += 10 ) {
      ...
      screen.Draw(Utils::Format(fmt, begin + 1), 0, dy, Color::White,
                  linenum_background);
  ...
  // cursor
  if( scroll_pos.y <= cursor.y && cursor.y < scroll_pos.y + height_max &&
      (forceDrawCursor || cursor_clock.HasTimePassed(Milliseconds(600))) ) {
    u32 cy = cursor.y - scroll_pos.y;
    screen.DrawRect(linenum_width + cursor.x * char_width + 3, cy * 10, 1, 10,
                    Color::Yellow);
