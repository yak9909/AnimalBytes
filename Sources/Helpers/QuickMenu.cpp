...
QuickMenuEntry::QuickMenuEntry(const std::string &name, ArgMethod method,
                               void *arg)
  : QuickMenuItem(name, ItemType::Entry)
{
...
QuickMenuSubMenu::QuickMenuSubMenu(const std::string &name,
                                   const std::vector<QuickMenuItem *> &items_)
  : QuickMenuItem(name, ItemType::SubMenu)
{
  for( QuickMenuItem *item : items_ ) items.push_back(item);
}

QuickMenuSubMenu::~QuickMenuSubMenu()
{
  for( QuickMenuItem *item : items ) delete item;
...
QuickMenu::~QuickMenu()
{
  for( QuickMenuItem *item : _root ) delete item;
}

QuickMenu &QuickMenu::GetInstance(void) { return (_instance); }

void QuickMenu::operator+=(QuickMenuItem *item) { _root.push_back(item); }
...
void QuickMenu::operator()(void)
{
  ...
  for( auto *item : _root ) options.push_back(item->name);
  ...
  while( true ) {
    ...
    if( userChoice >= 0 ) {
      QuickMenuItem *selected =
        _subMenuOpened ? _subMenuOpened->items[userChoice] : _root[userChoice];
      ...
      // If it's a submenu, open it
        ...
        if( _subMenuOpened != nullptr ) _submenus.push(_subMenuOpened);
...
void QuickMenu::ChangeHotkey(u32 newHotkey) { _hotkey = newHotkey; }
}  // namespace CTRPluginFramework
