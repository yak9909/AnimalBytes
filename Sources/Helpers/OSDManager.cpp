...
{
  if( _singleton == nullptr ) _singleton = new _OSDManager;
  return (_singleton);
}

void _OSDManager::Lock(void) { LightLock_Lock(&_lock); }

void _OSDManager::Unlock(void) { LightLock_Unlock(&_lock); }
...
bool _OSDManager::OSDCallback(const Screen& screen)
{
  ...
  for( auto item : manager._items ) {
    ...
    if( !std::get<4>(t) || str.empty() ) continue;
